...
/* 
 * Enumeration of different bond options across chips.
 *
 * This list should be a union
 * of all bond info features across all chips and only
 * related bond features should be enabled for a given chip.
 *
 * For new chips, the list will be appended with new feature names.
 * The feature enable/disable specific enums and table size changes related
 * enums should be grouped separately.
 * Similarly, chip-specific or component-specific options should be groupd
 * separately - See iProc specific and Apache specific groups below as an example.
 *
 * A previously defined bond feature should be re-used only if the feature is
 * exactly similar to the one that is already defined for an existing chip.
 *
 * */
