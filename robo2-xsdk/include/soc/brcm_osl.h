...
#define	OSL_ROBO_PCI_READ_CONFIG(dev, offset, size) \
        soc_pci_conf_read((signed long )dev, offset)
#define	OSL_ROBO_PCI_WRITE_CONFIG(dev, offset, size, val) \
        soc_pci_conf_write((signed long)dev, offset, val)

#define osl_init()  /* dummy func */

/* register access macros */
#define	R_REG(u, r)             CMREAD((signed long)u, (unsigned long)r)
#define	W_REG(u, r, v)          CMWRITE((signed long)u, (unsigned long)r, v)
#define	AND_REG(u, r, v)        W_REG((signed long)u, (unsigned long)(r), \
                                R_REG((signed long)u, (unsigned long)r) & (v))
#define	OR_REG(u, r, v)         W_REG((signed long)u, (uint32)(r), \
                                R_REG((signed long)u, (unsigned long)r) | (v))
