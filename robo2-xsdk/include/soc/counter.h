...
typedef struct soc_counter_non_dma_s {
    ...
    /* Eviction counter's rollover indication count */
    uint32 counter_rollover_evt[SOC_MAX_NUM_PIPES];
    uint32 counter_eviction_ct[SOC_MAX_NUM_PIPES];
...
typedef struct soc_counter_control_s {
    ...
    uint32                          total_dma_entries; /* total num of non stat
                                                        * dma entries */
#ifdef VENDOR_BROADCOM
    int                             counter_debug_lvl;
    int                             total_eviction_ct;
    int                             counter_rollover_ct;
    int                             counter_intr_ct;
    int                             counter_hm_process_ct;
    int                             hostmem_err;     /* Timeout/Overflow */
    int                             hostmem_overflow_err; /* Overflow */
    int                             slowdma_enabled; /* Residual collection
                                                        status. 1-Enable,
                                                        0-Disable */
#endif /* VENDOR_BROADCOM */
...
#ifdef VENDOR_BROADCOM
extern void soc_counter_debug_show(int unit);
extern void soc_counter_debug_clear(int unit);
#endif /* VENDOR_BROADCOM */
...
extern int soc_port_stat_dump(uint32 portid);
extern int
soc_robo2_counter_set_by_port(int unit, uint32 port, uint64 val);
extern int soc_robo2_counter_start(int unit, uint32 flags, int interval, pbmp_t pbmp);
extern int soc_robo2_counter_sync(int unit);
extern int soc_robo2_counter_stop(int unit);
