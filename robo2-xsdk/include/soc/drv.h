...
#define SOC_IS_ROBO(unit)        ((SOC_INFO(unit).spi_device) && \
                ( SOC_IS_ROBO_ARCH(unit) || SOC_IS_AVENGER(unit)))
...
#define IS_EXTERNAL_CPU_PORT(unit,port)          \
        (SOC_PBMP_MEMBER(PBMP_EXTERNAL_CPU(unit), port))
...
#define SOC_APB2PBUS_ACCESS_LOCK(unit) \
        sal_mutex_take(SOC_CONTROL(unit)->apb2pbus_access_mutex, sal_mutex_FOREVER)
#define SOC_APB2PBUS_ACCESS_UNLOCK(unit) \
        sal_mutex_give(SOC_CONTROL(unit)->apb2pbus_access_mutex)
#endif
/*
...
typedef struct {
   ...
   pbmp_t           pp_port_bitmap[SOC_MAX_NUM_PORTS];  /* Free PP Port Bitmap for physical port */
    ...
    int             cascade_ports; /* Bitmap of cascaded ports */
    int             trap_dest;     /* PGID of trap destination */
#endif
} soc_info_t;
...
#if defined(BCM_AVENGER_SUPPORT)
    sal_mutex_t  apb2pbus_access_mutex;
#endif
} soc_control_t;
...
extern int  soc_robo2_init(int unit);
extern int  soc_robo2_reset_init(int unit);
extern int  soc_robo2_detach(int unit);
extern int _soc_robo2_device_created(int unit);
extern int  soc_robo2_is_1g_disabled(int unit, soc_port_t port);
...
extern int soc_robo2_chip_reset(int unit);
#endif /* BCM_ROBO_SUPPORT */
