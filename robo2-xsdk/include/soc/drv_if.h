...
#define DRV_TRUNK_INIT(unit)\
	 _DRV_CALL(unit, trunk_init,(unit)) 
#define DRV_TRUNK_CREATE(unit, flags, tid)\
	 _DRV_CALL(unit, trunk_create,(unit, flags, tid)) 
#define DRV_TRUNK_DESTROY(unit, tid)\
	 _DRV_CALL(unit, trunk_destroy,(unit, tid)) 
#define DRV_TRUNK_MODIFY(unit, tid, gport, op)\
	 _DRV_CALL(unit, trunk_modify,(unit, tid, gport, op)) 
...
#define DRV_TRUNK_PSC_GET(unit, tid, psc)\
	 _DRV_CALL(unit, trunk_psc_get,(unit, tid, psc)) 
#define DRV_TRUNK_PSC_SET(unit, tid, psc)\
	 _DRV_CALL(unit, trunk_psc_set,(unit, tid, psc)) 
...
#define DRV_MIRROR_INIT(unit)\
	 _DRV_CALL(unit, mirror_init,(unit)) 
#define DRV_MIRROR_DEST_CREATE(unit, mirror_dest)\
	 _DRV_CALL(unit, mirror_dest_create,(unit, mirror_dest)) 
#define DRV_MIRROR_DEST_DESTROY(unit, destid)\
	 _DRV_CALL(unit, mirror_dest_destroy,(unit, destid)) 
#define DRV_MIRROR_PORT_ADD(unit, port, flags, destid)\
	 _DRV_CALL(unit, mirror_port_add,(unit, port, flags, destid)) 
#define DRV_MIRROR_PORT_DELETE(unit, port, flags, destid)\
	 _DRV_CALL(unit, mirror_port_delete,(unit, port, flags, destid)) 
#define DRV_MIRROR_VLAN_ADD(unit, vlan, flags, destid)\
	 _DRV_CALL(unit, mirror_vlan_add,(unit, vlan, flags, destid)) 
#define DRV_MIRROR_VLAN_DELETE(unit, vlan, flags, destid)\
	 _DRV_CALL(unit, mirror_vlan_delete,(unit, vlan, flags, destid)) 
#define DRV_LINKSCAN_INIT(unit)\
	 _DRV_CALL(unit, linkscan_init,(unit)) 
#define DRV_LINKSCAN_DETACH(unit)\
	 _DRV_CALL(unit, linkscan_detach,(unit)) 
#define DRV_LINKSCAN_REGISTER(unit, cb)\
	 _DRV_CALL(unit, linkscan_register,(unit, cb)) 
#define DRV_LINKSCAN_UNREGISTER(unit, cb)\
	 _DRV_CALL(unit, linkscan_unregister,(unit, cb)) 
#define DRV_LINKSCAN_ENABLE_GET(unit, us)\
	 _DRV_CALL(unit, linkscan_enable_get,(unit, us)) 
#define DRV_LINKSCAN_ENABLE_SET(unit, us)\
	 _DRV_CALL(unit, linkscan_enable_set,(unit, us)) 
#define DRV_LINKSCAN_DUMP(unit)\
	 _DRV_CALL(unit, linkscan_dump,(unit)) 
#define DRV_PORT_INIT(unit)\
	 _DRV_CALL(unit, port_init,(unit)) 
...
#define DRV_EAV_SRP_MAC_ETHERTYPE_SET(unit, mac, ethertype)\
	 _DRV_CALL(unit, eav_srp_mac_ethertype_set,(unit, mac, ethertype)) 
#define DRV_EAV_SRP_MAC_ETHERTYPE_GET(unit, mac, ethertype)\
	 _DRV_CALL(unit, eav_srp_mac_ethertype_get,(unit, mac, ethertype)) 
#define DRV_EAV_PCP_MAPPING_SET(unit, type, pcp, remapped_pcp)\
	 _DRV_CALL(unit, eav_pcp_mapping_set,(unit, type, pcp, remapped_pcp)) 
#define DRV_EAV_PCP_MAPPING_GET(unit, type, pcp, remapped_pcp)\
	 _DRV_CALL(unit, eav_pcp_mapping_get,(unit, type, pcp, remapped_pcp)) 
#define DRV_EAV_BANDWIDTH_SET(unit, port, type, bytes_sec, bytes_burst)\
	 _DRV_CALL(unit, eav_bandwidth_set,(unit, port, type, bytes_sec, bytes_burst)) 
#define DRV_EAV_BANDWIDTH_GET(unit, port, type, bytes_sec, bytes_burst)\
	 _DRV_CALL(unit, eav_bandwidth_get,(unit, port, type, bytes_sec, bytes_burst)) 
...
#define DRV_VLAN_INIT(unit)\
	 _DRV_CALL(unit, vlan_init,(unit)) 
#define DRV_VLAN_CREATE(unit, vlan_id)\
	 _DRV_CALL(unit, vlan_create,(unit, vlan_id)) 
#define DRV_VLAN_DESTROY(unit, vlan_id)\
	 _DRV_CALL(unit, vlan_destroy,(unit, vlan_id)) 
#define DRV_VLAN_DESTROY_ALL(unit)\
	 _DRV_CALL(unit, vlan_destroy_all,(unit)) 
#define DRV_VLAN_PORT_ADD(unit, vlan_id, port, flags)\
	 _DRV_CALL(unit, vlan_port_add,(unit, vlan_id, port, flags)) 
#define DRV_VLAN_PORT_REMOVE(unit, vlan_id, port)\
	 _DRV_CALL(unit, vlan_port_remove,(unit, vlan_id, port)) 
#define DRV_VLAN_GPORT_GET_ALL(unit, vlan_id, array_max, gport_array, flags_array, array_size)\
	 _DRV_CALL(unit, vlan_gport_get_all,(unit, vlan_id, array_max, gport_array, flags_array, array_size)) 
#define DRV_VLAN_STG_GET(unit, vlan_id, stg_get)\
	 _DRV_CALL(unit, vlan_stg_get,(unit, vlan_id, stg_get)) 
#define DRV_VLAN_STG_SET(unit, vlan_id, stg_id)\
	 _DRV_CALL(unit, vlan_stg_set,(unit, vlan_id, stg_id)) 
#define DRV_VLAN_CONTROL_VLAN_GET(unit, vlan_id, vlan_index, flags, learnmode, mcastid, ucastid)\
	 _DRV_CALL(unit, vlan_control_vlan_get,(unit, vlan_id, vlan_index, flags, learnmode, mcastid, ucastid)) 
#define DRV_VLAN_CONTROL_VLAN_SET(unit, vlan_id, vlan_index, flags, learnmode, mcastid, ucastid)\
	 _DRV_CALL(unit, vlan_control_vlan_set,(unit, vlan_id, vlan_index, flags, learnmode, mcastid, ucastid)) 
#define DRV_FSAL_INIT(unit)\
	 _DRV_CALL(unit, fsal_init,(unit)) 
#define DRV_L2_INIT(unit)\
	 _DRV_CALL(unit, l2_init,(unit)) 
#define DRV_L2_ADD(unit, l2addr)\
	 _DRV_CALL(unit, l2_add,(unit, l2addr)) 
#define DRV_L2_REMOVE(unit, l2addr)\
	 _DRV_CALL(unit, l2_remove,(unit, l2addr)) 
#define DRV_L2_LOOKUP(unit, l2addr)\
	 _DRV_CALL(unit, l2_lookup,(unit, l2addr)) 
#define DRV_L2_LOOKUP_NEXT(unit, l2addr, l2addr_next)\
	 _DRV_CALL(unit, l2_lookup_next,(unit, l2addr, l2addr_next)) 
#define DRV_L2_AGE_TIMER_GET(unit, age_seconds)\
	 _DRV_CALL(unit, l2_age_timer_get,(unit, age_seconds)) 
#define DRV_L2_AGE_TIMER_SET(unit, age_seconds)\
	 _DRV_CALL(unit, l2_age_timer_set,(unit, age_seconds)) 
#define DRV_L2_FLUSH_BY_VLAN(unit, vlan_id)\
	 _DRV_CALL(unit, l2_flush_by_vlan,(unit, vlan_id)) 
#define DRV_L2_FLUSH_BY_PORT(unit, port_id)\
	 _DRV_CALL(unit, l2_flush_by_port,(unit, port_id)) 
#define DRV_L2_RESEED(unit)\
	 _DRV_CALL(unit, l2_reseed,(unit)) 
#define DRV_PKT_RX_REGISTER(unit, cb)\
	 _DRV_CALL(unit, pkt_rx_register,(unit, cb)) 
#define DRV_PKT_TX(unit, pkt, count)\
	 _DRV_CALL(unit, pkt_tx,(unit, pkt, count)) 
#define DRV_MCAST_CREATE(unit, mcaddr)\
	 _DRV_CALL(unit, mcast_create,(unit, mcaddr)) 
#define DRV_MCAST_DESTROY(unit, mcaddr)\
	 _DRV_CALL(unit, mcast_destroy,(unit, mcaddr)) 
#define DRV_MCAST_ADD(unit, mcaddr, portid)\
	 _DRV_CALL(unit, mcast_add,(unit, mcaddr, portid)) 
#define DRV_MCAST_REMOVE(unit, mcaddr, portid)\
	 _DRV_CALL(unit, mcast_remove,(unit, mcaddr, portid)) 
#define DRV_MCAST_INFO_GET(unit, mcaddr)\
	 _DRV_CALL(unit, mcast_info_get,(unit, mcaddr)) 
#define DRV_CUSTOM_FIELD_ACTION_T_INIT(unit, action)\
	 _DRV_CALL(unit, custom_field_action_t_init,(unit, action)) 
#define DRV_CUSTOM_FIELD_INIT(unit, field_params, field_id)\
	 _DRV_CALL(unit, custom_field_init,(unit, field_params, field_id)) 
#define DRV_CUSTOM_FIELD_DETACH(unit, field_id)\
	 _DRV_CALL(unit, custom_field_detach,(unit, field_id)) 
#define DRV_CUSTOM_FIELD_RULE_T_INIT(unit, pkt_type, rule)\
	 _DRV_CALL(unit, custom_field_rule_t_init,(unit, pkt_type, rule)) 
#define DRV_CUSTOM_FIELD_RULE_SET(unit, field_id, rule, index)\
	 _DRV_CALL(unit, custom_field_rule_set,(unit, field_id, rule, index)) 
#define DRV_CUSTOM_FIELD_RULE_CLEAR(unit, field_id, index)\
	 _DRV_CALL(unit, custom_field_rule_clear,(unit, field_id, index)) 
#define DRV_CUSTOM_FIELD_ACTION_SET(unit, field_id, action, index)\
	 _DRV_CALL(unit, custom_field_action_set,(unit, field_id, action, index)) 
#define DRV_CUSTOM_FIELD_ACTION_CLEAR(unit, field_id, index)\
	 _DRV_CALL(unit, custom_field_action_clear,(unit, field_id, index)) 
#define DRV_CUSTOM_FIELD_RULE_DUMP(unit, field_id, index)\
	 _DRV_CALL(unit, custom_field_rule_dump,(unit, field_id, index)) 
#define DRV_CUSTOM_FIELD_ACTION_DUMP(unit, field_id, index)\
	 _DRV_CALL(unit, custom_field_action_dump,(unit, field_id, index)) 
#define DRV_CUSTOM_FIELD_STAT_MULTI_GET(unit, field_id, index, nstat, stat_arr, value_arr)\
	 _DRV_CALL(unit, custom_field_stat_multi_get,(unit, field_id, index, nstat, stat_arr, value_arr)) 
#define DRV_CUSTOM_FIELD_STAT_MULTI_SET(unit, field_id, index, nstat, stat_arr, value_arr)\
	 _DRV_CALL(unit, custom_field_stat_multi_set,(unit, field_id, index, nstat, stat_arr, value_arr)) 
#define DRV_STG_INIT(unit)\
	 _DRV_CALL(unit, stg_init,(unit)) 
#define DRV_STG_CREATE(unit, stgid)\
	 _DRV_CALL(unit, stg_create,(unit, stgid)) 
#define DRV_STG_DESTROY(unit, stgid)\
	 _DRV_CALL(unit, stg_destroy,(unit, stgid)) 
#define DRV_STG_VLAN_ADD(unit, stgid, vlanid)\
	 _DRV_CALL(unit, stg_vlan_add,(unit, stgid, vlanid)) 
#define DRV_STG_VLAN_REMOVE(unit, stgid, vlanid)\
	 _DRV_CALL(unit, stg_vlan_remove,(unit, stgid, vlanid)) 
#define DRV_STG_VLAN_REMOVE_ALL(unit, stgid)\
	 _DRV_CALL(unit, stg_vlan_remove_all,(unit, stgid)) 
#define DRV_STG_STP_GET(unit, stgid, port, stp_state)\
	 _DRV_CALL(unit, stg_stp_get,(unit, stgid, port, stp_state)) 
#define DRV_STG_STP_SET(unit, stgid, port, stp_state)\
	 _DRV_CALL(unit, stg_stp_set,(unit, stgid, port, stp_state)) 
#define DRV_CUSTOM_METER_INIT(unit)\
	 _DRV_CALL(unit, custom_meter_init,(unit)) 
#define DRV_CUSTOM_METER_PROFILE_PARAMS_T_INIT(unit, params)\
	 _DRV_CALL(unit, custom_meter_profile_params_t_init,(unit, params)) 
#define DRV_CUSTOM_METER_PROFILE_CREATE(unit, profile_params, profile_id)\
	 _DRV_CALL(unit, custom_meter_profile_create,(unit, profile_params, profile_id)) 
#define DRV_CUSTOM_METER_PROFILE_DESTROY(unit, profile_id)\
	 _DRV_CALL(unit, custom_meter_profile_destroy,(unit, profile_id)) 
#define DRV_CUSTOM_METER_PROFILE_GET(unit, profile_id, profile_params)\
	 _DRV_CALL(unit, custom_meter_profile_get,(unit, profile_id, profile_params)) 
#define DRV_CUSTOM_METER_PROFILE_SET(unit, profile_id, profile_params)\
	 _DRV_CALL(unit, custom_meter_profile_set,(unit, profile_id, profile_params)) 
#define DRV_CUSTOM_METER_PARAMS_T_INIT(unit, params)\
	 _DRV_CALL(unit, custom_meter_params_t_init,(unit, params)) 
#define DRV_CUSTOM_METER_CREATE(unit, profile_id, meter_params, meter_id)\
	 _DRV_CALL(unit, custom_meter_create,(unit, profile_id, meter_params, meter_id)) 
#define DRV_CUSTOM_METER_DESTROY(unit, meter_id)\
	 _DRV_CALL(unit, custom_meter_destroy,(unit, meter_id)) 
#define DRV_CUSTOM_METER_PM_PARAMS_T_INIT(unit, params)\
	 _DRV_CALL(unit, custom_meter_pm_params_t_init,(unit, params)) 
#define DRV_CUSTOM_METER_PM_ENABLE(unit, meter_id)\
	 _DRV_CALL(unit, custom_meter_pm_enable,(unit, meter_id)) 
#define DRV_CUSTOM_METER_PM_DISABLE(unit, meter_id)\
	 _DRV_CALL(unit, custom_meter_pm_disable,(unit, meter_id)) 
#define DRV_CUSTOM_METER_PM_GET(unit, meter_id, pm_params)\
	 _DRV_CALL(unit, custom_meter_pm_get,(unit, meter_id, pm_params)) 
#define DRV_CUSTOM_COSQ_INIT(unit)\
	 _DRV_CALL(unit, custom_cosq_init,(unit)) 
#define DRV_CUSTOM_FLOWCONTROL_PARAMS_T_INIT(unit, portid, flowcontrol_params)\
	 _DRV_CALL(unit, custom_flowcontrol_params_t_init,(unit, portid, flowcontrol_params)) 
#define DRV_CUSTOM_COSQ_SHAPER_PARAMS_T_INIT(unit, portid, shaper_params)\
	 _DRV_CALL(unit, custom_cosq_shaper_params_t_init,(unit, portid, shaper_params)) 
#define DRV_CUSTOM_COSQ_TSN_PARAMS_T_INIT(unit, tsn_params)\
	 _DRV_CALL(unit, custom_cosq_tsn_params_t_init,(unit, tsn_params)) 
#define DRV_CUSTOM_COSQ_PORT_DEFAULT_TC_SET(unit, portid, tc)\
	 _DRV_CALL(unit, custom_cosq_port_default_tc_set,(unit, portid, tc)) 
#define DRV_CUSTOM_COSQ_PORT_DEFAULT_TC_GET(unit, portid, tc)\
	 _DRV_CALL(unit, custom_cosq_port_default_tc_get,(unit, portid, tc)) 
#define DRV_CUSTOM_COSQ_PORT_DEFAULT_PCP_SET(unit, portid, pcp, dei)\
	 _DRV_CALL(unit, custom_cosq_port_default_pcp_set,(unit, portid, pcp, dei)) 
#define DRV_CUSTOM_COSQ_PORT_DEFAULT_PCP_GET(unit, portid, pcp, dei)\
	 _DRV_CALL(unit, custom_cosq_port_default_pcp_get,(unit, portid, pcp, dei)) 
#define DRV_CUSTOM_COSQ_PORT_PCP2TC_SET(unit, portid, pcp, dei, tc)\
	 _DRV_CALL(unit, custom_cosq_port_pcp2tc_set,(unit, portid, pcp, dei, tc)) 
#define DRV_CUSTOM_COSQ_PORT_PCP2TC_GET(unit, portid, pcp, dei, tc)\
	 _DRV_CALL(unit, custom_cosq_port_pcp2tc_get,(unit, portid, pcp, dei, tc)) 
#define DRV_CUSTOM_COSQ_PORT_TC2PCP_SET(unit, portid, tc, pcp, dei)\
	 _DRV_CALL(unit, custom_cosq_port_tc2pcp_set,(unit, portid, tc, pcp, dei)) 
#define DRV_CUSTOM_COSQ_PORT_TC2PCP_GET(unit, portid, tc, pcp, dei)\
	 _DRV_CALL(unit, custom_cosq_port_tc2pcp_get,(unit, portid, tc, pcp, dei)) 
#define DRV_CUSTOM_COSQ_DSCP2TC_SET(unit, dscp, tc)\
	 _DRV_CALL(unit, custom_cosq_dscp2tc_set,(unit, dscp, tc)) 
#define DRV_CUSTOM_COSQ_DSCP2TC_GET(unit, dscp, tc)\
	 _DRV_CALL(unit, custom_cosq_dscp2tc_get,(unit, dscp, tc)) 
#define DRV_CUSTOM_COSQ_TC2DSCP_SET(unit, tc, dscp)\
	 _DRV_CALL(unit, custom_cosq_tc2dscp_set,(unit, tc, dscp)) 
#define DRV_CUSTOM_COSQ_TC2DSCP_GET(unit, tc, dscp)\
	 _DRV_CALL(unit, custom_cosq_tc2dscp_get,(unit, tc, dscp)) 
#define DRV_CUSTOM_COSQ_PORT_SCHED_GET(unit, portid, sched_mode, weights)\
	 _DRV_CALL(unit, custom_cosq_port_sched_get,(unit, portid, sched_mode, weights)) 
#define DRV_CUSTOM_COSQ_PORT_SCHED_SET(unit, portid, sched_mode, weights)\
	 _DRV_CALL(unit, custom_cosq_port_sched_set,(unit, portid, sched_mode, weights)) 
#define DRV_CUSTOM_COSQ_FLOWCONTROL_GET(unit, flowcontrol_mode)\
	 _DRV_CALL(unit, custom_cosq_flowcontrol_get,(unit, flowcontrol_mode)) 
#define DRV_CUSTOM_COSQ_FLOWCONTROL_SET(unit, flowcontrol_mode)\
	 _DRV_CALL(unit, custom_cosq_flowcontrol_set,(unit, flowcontrol_mode)) 
#define DRV_CUSTOM_COSQ_PORT_FLOWCONTROL_GET(unit, portid, fc_params)\
	 _DRV_CALL(unit, custom_cosq_port_flowcontrol_get,(unit, portid, fc_params)) 
#define DRV_CUSTOM_COSQ_PORT_FLOWCONTROL_SET(unit, portid, fc_params)\
	 _DRV_CALL(unit, custom_cosq_port_flowcontrol_set,(unit, portid, fc_params)) 
#define DRV_CUSTOM_COSQ_PFC_FLOWCONTROL_GET(unit, int_pri, fc_params)\
	 _DRV_CALL(unit, custom_cosq_pfc_flowcontrol_get,(unit, int_pri, fc_params)) 
#define DRV_CUSTOM_COSQ_PFC_FLOWCONTROL_SET(unit, int_pri, fc_params)\
	 _DRV_CALL(unit, custom_cosq_pfc_flowcontrol_set,(unit, int_pri, fc_params)) 
#define DRV_CUSTOM_COSQ_PORT_SHAPER_GET(unit, portid, shaper_params)\
	 _DRV_CALL(unit, custom_cosq_port_shaper_get,(unit, portid, shaper_params)) 
#define DRV_CUSTOM_COSQ_PORT_SHAPER_SET(unit, portid, shaper_params)\
	 _DRV_CALL(unit, custom_cosq_port_shaper_set,(unit, portid, shaper_params)) 
#define DRV_CUSTOM_COSQ_QUEUE_SHAPER_GET(unit, portid, int_pri, shaper_params)\
	 _DRV_CALL(unit, custom_cosq_queue_shaper_get,(unit, portid, int_pri, shaper_params)) 
#define DRV_CUSTOM_COSQ_QUEUE_SHAPER_SET(unit, portid, int_pri, shaper_params)\
	 _DRV_CALL(unit, custom_cosq_queue_shaper_set,(unit, portid, int_pri, shaper_params)) 
#define DRV_CUSTOM_COSQ_QUEUE_TSN_GET(unit, portid, int_pri, tsn_params)\
	 _DRV_CALL(unit, custom_cosq_queue_tsn_get,(unit, portid, int_pri, tsn_params)) 
#define DRV_CUSTOM_COSQ_QUEUE_TSN_SET(unit, portid, int_pri, tsn_params)\
	 _DRV_CALL(unit, custom_cosq_queue_tsn_set,(unit, portid, int_pri, tsn_params)) 
#define DRV_CUSTOM_AUTH_PARAMS_T_INIT(unit, auth_params)\
	 _DRV_CALL(unit, custom_auth_params_t_init,(unit, auth_params)) 
#define DRV_CUSTOM_AUTH_PORT_MODE_T_INIT(unit, auth_port_mode)\
	 _DRV_CALL(unit, custom_auth_port_mode_t_init,(unit, auth_port_mode)) 
#define DRV_CUSTOM_AUTH_INIT(unit, auth_params)\
	 _DRV_CALL(unit, custom_auth_init,(unit, auth_params)) 
#define DRV_CUSTOM_AUTH_DETACH(unit, auth_params)\
	 _DRV_CALL(unit, custom_auth_detach,(unit, auth_params)) 
#define DRV_CUSTOM_AUTH_MODE_GET(unit, auth_params)\
	 _DRV_CALL(unit, custom_auth_mode_get,(unit, auth_params)) 
#define DRV_CUSTOM_AUTH_MODE_SET(unit, auth_params)\
	 _DRV_CALL(unit, custom_auth_mode_set,(unit, auth_params)) 
#define DRV_CUSTOM_AUTH_PORT_GET(unit, portid, port_mode)\
	 _DRV_CALL(unit, custom_auth_port_get,(unit, portid, port_mode)) 
#define DRV_CUSTOM_AUTH_PORT_SET(unit, portid, port_mode)\
	 _DRV_CALL(unit, custom_auth_port_set,(unit, portid, port_mode)) 
#define DRV_CUSTOM_TS_INIT(unit)\
	 _DRV_CALL(unit, custom_ts_init,(unit)) 
#define DRV_CUSTOM_TS_GEN_CONFIG_SET(unit, generator_id, config)\
	 _DRV_CALL(unit, custom_ts_gen_config_set,(unit, generator_id, config)) 
#define DRV_CUSTOM_TS_EVENT_CONFIG_SET(unit, event_id, event_source, config)\
	 _DRV_CALL(unit, custom_ts_event_config_set,(unit, event_id, event_source, config)) 
#define DRV_CUSTOM_TS_TOD_GET(unit, time, timestamp)\
	 _DRV_CALL(unit, custom_ts_tod_get,(unit, time, timestamp)) 
#define DRV_CUSTOM_TS_TOD_SET(unit, time, timestamp)\
	 _DRV_CALL(unit, custom_ts_tod_set,(unit, time, timestamp)) 
#define DRV_PORT_PKT_FLOW_ENABLE(unit)\
	 _DRV_CALL(unit, port_pkt_flow_enable,(unit)) 
...
typedef struct{ 
	...
	int (*trunk_init)(int unit);
	int (*trunk_create)(int unit,uint32 flags,int *tid);
	int (*trunk_destroy)(int unit,int tid);
	int (*trunk_modify)(int unit,int tid,int gport,int op);
	...
	int (*trunk_psc_get)(int unit,int tid,int *psc);
	int (*trunk_psc_set)(int unit,int tid,int psc);
	...
	int (*mirror_init)(int unit);
	int (*mirror_dest_create)(int unit,drv_mirror_dest_t *mirror_dest);
	int (*mirror_dest_destroy)(int unit,int destid);
	int (*mirror_port_add)(int unit,uint32 port,uint32 flags,int destid);
	int (*mirror_port_delete)(int unit,uint32 port,uint32 flags,int destid);
	int (*mirror_vlan_add)(int unit,uint32 vlan,uint32 flags,int destid);
	int (*mirror_vlan_delete)(int unit,uint32 vlan,uint32 flags,int destid);
	int (*linkscan_init)(int unit);
	int (*linkscan_detach)(int unit);
	int (*linkscan_register)(int unit,drv_link_scan_callback_t cb);
	int (*linkscan_unregister)(int unit,drv_link_scan_callback_t cb);
	int (*linkscan_enable_get)(int unit,int *us);
	int (*linkscan_enable_set)(int unit,int us);
	int (*linkscan_dump)(int unit);
	int (*port_init)(int unit);
	...
	int (*eav_srp_mac_ethertype_set)(int unit,uint8 *mac,uint16 ethertype);
	int (*eav_srp_mac_ethertype_get)(int unit,uint8 *mac,uint16 *ethertype);
	int (*eav_pcp_mapping_set)(int unit,uint32 type,int pcp,int remapped_pcp);
	int (*eav_pcp_mapping_get)(int unit,uint32 type,int *pcp,int *remapped_pcp);
	int (*eav_bandwidth_set)(int unit,uint32 port,uint32 type,uint32 bytes_sec,uint32 bytes_burst);
	int (*eav_bandwidth_get)(int unit,uint32 port,uint32 type,uint32 *bytes_sec,uint32 *bytes_burst);
	...
	int (*vlan_init)(int unit);
	int (*vlan_create)(int unit,uint32 vlan_id);
	int (*vlan_destroy)(int unit,uint32 vlan_id);
	int (*vlan_destroy_all)(int unit);
	int (*vlan_port_add)(int unit,uint32 vlan_id,int port,int flags);
	int (*vlan_port_remove)(int unit,uint32 vlan_id,int port);
	int (*vlan_gport_get_all)(int unit,uint32 vlan_id,int array_max,int *gport_array,int *flags_array,int *array_size);
	int (*vlan_stg_get)(int unit,uint32 vlan_id,int *stg_get);
	int (*vlan_stg_set)(int unit,uint32 vlan_id,uint32 stg_id);
	int (*vlan_control_vlan_get)(int unit,uint32 vlan_id,uint32 *vlan_index,uint32 *flags,uint32 *learnmode,uint32 *mcastid,uint32 *ucastid);
	int (*vlan_control_vlan_set)(int unit,uint32 vlan_id,uint32 vlan_index,uint32 flags,uint32 learnmode,uint32 mcastid,uint32 ucastid);
	int (*fsal_init)(int unit);
	int (*l2_init)(int unit);
	int (*l2_add)(int unit,bcm_l2_addr_t *l2addr);
	int (*l2_remove)(int unit,bcm_l2_addr_t *l2addr);
	int (*l2_lookup)(int unit,bcm_l2_addr_t *l2addr);
	int (*l2_lookup_next)(int unit,bcm_l2_addr_t *l2addr,bcm_l2_addr_t *l2addr_next);
	int (*l2_age_timer_get)(int unit,uint32 *age_seconds);
	int (*l2_age_timer_set)(int unit,uint32 age_seconds);
	int (*l2_flush_by_vlan)(int unit,uint32 vlan_id);
	int (*l2_flush_by_port)(int unit,uint32 port_id);
	int (*l2_reseed)(int unit);
	int (*pkt_rx_register)(int unit,drv_packet_io_callback_t cb);
	int (*pkt_tx)(int unit,bcm_pkt_t **pkt,int count);
	int (*mcast_create)(int unit,bcm_mcast_addr_t *mcaddr);
	int (*mcast_destroy)(int unit,bcm_mcast_addr_t *mcaddr);
	int (*mcast_add)(int unit,bcm_mcast_addr_t *mcaddr,uint32 portid);
	int (*mcast_remove)(int unit,bcm_mcast_addr_t *mcaddr,uint32 portid);
	int (*mcast_info_get)(int unit,bcm_mcast_addr_t *mcaddr);
	int (*custom_field_action_t_init)(int unit,bcm_custom_field_action_t *action);
	int (*custom_field_init)(int unit,bcm_custom_field_params_t *field_params,bcm_custom_field_id_t *field_id);
	int (*custom_field_detach)(int unit,bcm_custom_field_id_t field_id);
	int (*custom_field_rule_t_init)(int unit,bcm_custom_field_l3_l4_pkt_type_t pkt_type,bcm_custom_field_rule_t *rule);
	int (*custom_field_rule_set)(int unit,bcm_custom_field_id_t *field_id,bcm_custom_field_rule_t *rule,uint32 index);
	int (*custom_field_rule_clear)(int unit,bcm_custom_field_id_t *field_id,uint32 index);
	int (*custom_field_action_set)(int unit,bcm_custom_field_id_t *field_id,bcm_custom_field_action_t *action,uint32 index);
	int (*custom_field_action_clear)(int unit,bcm_custom_field_id_t *field_id,uint32 index);
	int (*custom_field_rule_dump)(int unit,bcm_custom_field_id_t *field_id,uint32 index);
	int (*custom_field_action_dump)(int unit,bcm_custom_field_id_t *field_id,uint32 index);
	int (*custom_field_stat_multi_get)(int unit,bcm_custom_field_id_t *field_id,uint32 index,int nstat,bcm_field_stat_t *stat_arr,uint64 *value_arr);
	int (*custom_field_stat_multi_set)(int unit,bcm_custom_field_id_t *field_id,uint32 index,int nstat,bcm_field_stat_t *stat_arr,uint64 *value_arr);
	int (*stg_init)(int unit);
	int (*stg_create)(int unit,int *stgid);
	int (*stg_destroy)(int unit,int stgid);
	int (*stg_vlan_add)(int unit,int stgid,uint32 vlanid);
	int (*stg_vlan_remove)(int unit,int stgid,uint32 vlanid);
	int (*stg_vlan_remove_all)(int unit,int stgid);
	int (*stg_stp_get)(int unit,int stgid,int port,int *stp_state);
	int (*stg_stp_set)(int unit,int stgid,int port,int stp_state);
	int (*custom_meter_init)(int unit);
	int (*custom_meter_profile_params_t_init)(int unit,bcm_custom_meter_profile_params_t *params);
	int (*custom_meter_profile_create)(int unit,bcm_custom_meter_profile_params_t *profile_params,bcm_custom_meter_profile_t *profile_id);
	int (*custom_meter_profile_destroy)(int unit,bcm_custom_meter_profile_t profile_id);
	int (*custom_meter_profile_get)(int unit,bcm_custom_meter_profile_t profile_id,bcm_custom_meter_profile_params_t *profile_params);
	int (*custom_meter_profile_set)(int unit,bcm_custom_meter_profile_t profile_id,bcm_custom_meter_profile_params_t *profile_params);
	int (*custom_meter_params_t_init)(int unit,bcm_custom_meter_params_t *params);
	int (*custom_meter_create)(int unit,bcm_custom_meter_profile_t profile_id,bcm_custom_meter_params_t *meter_params,bcm_custom_meterid_t *meter_id);
	int (*custom_meter_destroy)(int unit,bcm_custom_meterid_t meter_id);
	int (*custom_meter_pm_params_t_init)(int unit,bcm_custom_meter_pm_params_t *params);
	int (*custom_meter_pm_enable)(int unit,bcm_custom_meterid_t meter_id);
	int (*custom_meter_pm_disable)(int unit,bcm_custom_meterid_t meter_id);
	int (*custom_meter_pm_get)(int unit,bcm_custom_meterid_t meter_id,bcm_custom_meter_pm_params_t *pm_params);
	int (*custom_cosq_init)(int unit);
	int (*custom_flowcontrol_params_t_init)(int unit,bcm_gport_t portid,bcm_custom_flowcontrol_params_t *flowcontrol_params);
	int (*custom_cosq_shaper_params_t_init)(int unit,bcm_gport_t portid,bcm_custom_cosq_shaper_params_t *shaper_params);
	int (*custom_cosq_tsn_params_t_init)(int unit,bcm_custom_cosq_tsn_params_t *tsn_params);
	int (*custom_cosq_port_default_tc_set)(int unit,bcm_gport_t portid,bcm_custom_tc_t tc);
	int (*custom_cosq_port_default_tc_get)(int unit,bcm_gport_t portid,bcm_custom_tc_t *tc);
	int (*custom_cosq_port_default_pcp_set)(int unit,bcm_gport_t portid,int pcp,int dei);
	int (*custom_cosq_port_default_pcp_get)(int unit,bcm_gport_t portid,int *pcp,int *dei);
	int (*custom_cosq_port_pcp2tc_set)(int unit,bcm_gport_t portid,int pcp,int dei,bcm_custom_tc_t tc);
	int (*custom_cosq_port_pcp2tc_get)(int unit,bcm_gport_t portid,int pcp,int dei,bcm_custom_tc_t *tc);
	int (*custom_cosq_port_tc2pcp_set)(int unit,bcm_gport_t portid,bcm_custom_tc_t tc,int pcp,int dei);
	int (*custom_cosq_port_tc2pcp_get)(int unit,bcm_gport_t portid,bcm_custom_tc_t tc,int *pcp,int *dei);
	int (*custom_cosq_dscp2tc_set)(int unit,bcm_custom_dscp_t dscp,bcm_custom_tc_t tc);
	int (*custom_cosq_dscp2tc_get)(int unit,bcm_custom_dscp_t dscp,bcm_custom_tc_t *tc);
	int (*custom_cosq_tc2dscp_set)(int unit,bcm_custom_tc_t tc,bcm_custom_dscp_t dscp);
	int (*custom_cosq_tc2dscp_get)(int unit,bcm_custom_tc_t tc,bcm_custom_dscp_t *dscp);
	int (*custom_cosq_port_sched_get)(int unit,bcm_gport_t portid,bcm_custom_schedule_mode_t *sched_mode,int *weights);
	int (*custom_cosq_port_sched_set)(int unit,bcm_gport_t portid,bcm_custom_schedule_mode_t sched_mode,int *weights);
	int (*custom_cosq_flowcontrol_get)(int unit,bcm_custom_flowcontrol_t *flowcontrol_mode);
	int (*custom_cosq_flowcontrol_set)(int unit,bcm_custom_flowcontrol_t flowcontrol_mode);
	int (*custom_cosq_port_flowcontrol_get)(int unit,bcm_gport_t portid,bcm_custom_flowcontrol_params_t *fc_params);
	int (*custom_cosq_port_flowcontrol_set)(int unit,bcm_gport_t portid,bcm_custom_flowcontrol_params_t fc_params);
	int (*custom_cosq_pfc_flowcontrol_get)(int unit,int int_pri,bcm_custom_flowcontrol_params_t *fc_params);
	int (*custom_cosq_pfc_flowcontrol_set)(int unit,int int_pri,bcm_custom_flowcontrol_params_t fc_params);
	int (*custom_cosq_port_shaper_get)(int unit,bcm_gport_t portid,bcm_custom_cosq_shaper_params_t *shaper_params);
	int (*custom_cosq_port_shaper_set)(int unit,bcm_gport_t portid,bcm_custom_cosq_shaper_params_t *shaper_params);
	int (*custom_cosq_queue_shaper_get)(int unit,bcm_gport_t portid,int int_pri,bcm_custom_cosq_shaper_params_t *shaper_params);
	int (*custom_cosq_queue_shaper_set)(int unit,bcm_gport_t portid,int int_pri,bcm_custom_cosq_shaper_params_t *shaper_params);
	int (*custom_cosq_queue_tsn_get)(int unit,bcm_gport_t portid,int int_pri,bcm_custom_cosq_tsn_params_t *tsn_params);
	int (*custom_cosq_queue_tsn_set)(int unit,bcm_gport_t portid,int int_pri,bcm_custom_cosq_tsn_params_t *tsn_params);
	int (*custom_auth_params_t_init)(int unit,bcm_custom_auth_params_t *auth_params);
	int (*custom_auth_port_mode_t_init)(int unit,bcm_custom_auth_port_mode_t *auth_port_mode);
	int (*custom_auth_init)(int unit,bcm_custom_auth_params_t *auth_params);
	int (*custom_auth_detach)(int unit,bcm_custom_auth_params_t *auth_params);
	int (*custom_auth_mode_get)(int unit,bcm_custom_auth_params_t *auth_params);
	int (*custom_auth_mode_set)(int unit,bcm_custom_auth_params_t *auth_params);
	int (*custom_auth_port_get)(int unit,bcm_gport_t portid,bcm_custom_auth_port_mode_t *port_mode);
	int (*custom_auth_port_set)(int unit,bcm_gport_t portid,bcm_custom_auth_port_mode_t *port_mode);
	int (*custom_ts_init)(int unit);
	int (*custom_ts_gen_config_set)(int unit,int generator_id,bcm_custom_ts_gen_config_t *config);
	int (*custom_ts_event_config_set)(int unit,int event_id,int event_source,bcm_custom_ts_event_config_t *config);
	int (*custom_ts_tod_get)(int unit,bcm_custom_ts_time_t *time,uint64 *timestamp);
	int (*custom_ts_tod_set)(int unit,bcm_custom_ts_time_t time,uint64 timestamp);
	int (*port_pkt_flow_enable)(int unit);
} drv_if_t;
...
extern int drv_trunk_init(int unit);
extern int drv_trunk_create(int unit,uint32 flags,int *tid);
extern int drv_trunk_destroy(int unit,int tid);
extern int drv_trunk_modify(int unit,int tid,int gport,int op);
...
extern int drv_trunk_psc_get(int unit,int tid,int *psc);
extern int drv_trunk_psc_set(int unit,int tid,int psc);
...
extern int drv_mirror_init(int unit);
extern int drv_mirror_dest_create(int unit,drv_mirror_dest_t *mirror_dest);
extern int drv_mirror_dest_destroy(int unit,int destid);
extern int drv_mirror_port_add(int unit,uint32 port,uint32 flags,int destid);
extern int drv_mirror_port_delete(int unit,uint32 port,uint32 flags,int destid);
extern int drv_mirror_vlan_add(int unit,uint32 vlan,uint32 flags,int destid);
extern int drv_mirror_vlan_delete(int unit,uint32 vlan,uint32 flags,int destid);
extern int drv_linkscan_init(int unit);
extern int drv_linkscan_detach(int unit);
extern int drv_linkscan_register(int unit,drv_link_scan_callback_t cb);
extern int drv_linkscan_unregister(int unit,drv_link_scan_callback_t cb);
extern int drv_linkscan_enable_get(int unit,int *us);
extern int drv_linkscan_enable_set(int unit,int us);
extern int drv_linkscan_dump(int unit);
extern int drv_port_init(int unit);
...
extern int drv_eav_srp_mac_ethertype_set(int unit,uint8 *mac,uint16 ethertype);
extern int drv_eav_srp_mac_ethertype_get(int unit,uint8 *mac,uint16 *ethertype);
extern int drv_eav_pcp_mapping_set(int unit,uint32 type,int pcp,int remapped_pcp);
extern int drv_eav_pcp_mapping_get(int unit,uint32 type,int *pcp,int *remapped_pcp);
extern int drv_eav_bandwidth_set(int unit,uint32 port,uint32 type,uint32 bytes_sec,uint32 bytes_burst);
extern int drv_eav_bandwidth_get(int unit,uint32 port,uint32 type,uint32 *bytes_sec,uint32 *bytes_burst);
...
extern int drv_vlan_init(int unit);
extern int drv_vlan_create(int unit,uint32 vlan_id);
extern int drv_vlan_destroy(int unit,uint32 vlan_id);
extern int drv_vlan_destroy_all(int unit);
extern int drv_vlan_port_add(int unit,uint32 vlan_id,int port,int flags);
extern int drv_vlan_port_remove(int unit,uint32 vlan_id,int port);
extern int drv_vlan_gport_get_all(int unit,uint32 vlan_id,int array_max,int *gport_array,int *flags_array,int *array_size);
extern int drv_vlan_stg_get(int unit,uint32 vlan_id,int *stg_get);
extern int drv_vlan_stg_set(int unit,uint32 vlan_id,uint32 stg_id);
extern int drv_vlan_control_vlan_get(int unit,uint32 vlan_id,uint32 *vlan_index,uint32 *flags,uint32 *learnmode,uint32 *mcastid,uint32 *ucastid);
extern int drv_vlan_control_vlan_set(int unit,uint32 vlan_id,uint32 vlan_index,uint32 flags,uint32 learnmode,uint32 mcastid,uint32 ucastid);
extern int drv_fsal_init(int unit);
extern int drv_l2_init(int unit);
extern int drv_l2_add(int unit,bcm_l2_addr_t *l2addr);
extern int drv_l2_remove(int unit,bcm_l2_addr_t *l2addr);
extern int drv_l2_lookup(int unit,bcm_l2_addr_t *l2addr);
extern int drv_l2_lookup_next(int unit,bcm_l2_addr_t *l2addr,bcm_l2_addr_t *l2addr_next);
extern int drv_l2_age_timer_get(int unit,uint32 *age_seconds);
extern int drv_l2_age_timer_set(int unit,uint32 age_seconds);
extern int drv_l2_flush_by_vlan(int unit,uint32 vlan_id);
extern int drv_l2_flush_by_port(int unit,uint32 port_id);
extern int drv_l2_reseed(int unit);
extern int drv_pkt_rx_register(int unit,drv_packet_io_callback_t cb);
extern int drv_pkt_tx(int unit,bcm_pkt_t **pkt,int count);
extern int drv_mcast_create(int unit,bcm_mcast_addr_t *mcaddr);
extern int drv_mcast_destroy(int unit,bcm_mcast_addr_t *mcaddr);
extern int drv_mcast_add(int unit,bcm_mcast_addr_t *mcaddr,uint32 portid);
extern int drv_mcast_remove(int unit,bcm_mcast_addr_t *mcaddr,uint32 portid);
extern int drv_mcast_info_get(int unit,bcm_mcast_addr_t *mcaddr);
extern int drv_custom_field_action_t_init(int unit,bcm_custom_field_action_t *action);
extern int drv_custom_field_init(int unit,bcm_custom_field_params_t *field_params,bcm_custom_field_id_t *field_id);
extern int drv_custom_field_detach(int unit,bcm_custom_field_id_t field_id);
extern int drv_custom_field_rule_t_init(int unit,bcm_custom_field_l3_l4_pkt_type_t pkt_type,bcm_custom_field_rule_t *rule);
extern int drv_custom_field_rule_set(int unit,bcm_custom_field_id_t *field_id,bcm_custom_field_rule_t *rule,uint32 index);
extern int drv_custom_field_rule_clear(int unit,bcm_custom_field_id_t *field_id,uint32 index);
extern int drv_custom_field_action_set(int unit,bcm_custom_field_id_t *field_id,bcm_custom_field_action_t *action,uint32 index);
extern int drv_custom_field_action_clear(int unit,bcm_custom_field_id_t *field_id,uint32 index);
extern int drv_custom_field_rule_dump(int unit,bcm_custom_field_id_t *field_id,uint32 index);
extern int drv_custom_field_action_dump(int unit,bcm_custom_field_id_t *field_id,uint32 index);
extern int drv_custom_field_stat_multi_get(int unit,bcm_custom_field_id_t *field_id,uint32 index,int nstat,bcm_field_stat_t *stat_arr,uint64 *value_arr);
extern int drv_custom_field_stat_multi_set(int unit,bcm_custom_field_id_t *field_id,uint32 index,int nstat,bcm_field_stat_t *stat_arr,uint64 *value_arr);
extern int drv_stg_init(int unit);
extern int drv_stg_create(int unit,int *stgid);
extern int drv_stg_destroy(int unit,int stgid);
extern int drv_stg_vlan_add(int unit,int stgid,uint32 vlanid);
extern int drv_stg_vlan_remove(int unit,int stgid,uint32 vlanid);
extern int drv_stg_vlan_remove_all(int unit,int stgid);
extern int drv_stg_stp_get(int unit,int stgid,int port,int *stp_state);
extern int drv_stg_stp_set(int unit,int stgid,int port,int stp_state);
extern int drv_custom_meter_init(int unit);
extern int drv_custom_meter_profile_params_t_init(int unit,bcm_custom_meter_profile_params_t *params);
extern int drv_custom_meter_profile_create(int unit,bcm_custom_meter_profile_params_t *profile_params,bcm_custom_meter_profile_t *profile_id);
extern int drv_custom_meter_profile_destroy(int unit,bcm_custom_meter_profile_t profile_id);
extern int drv_custom_meter_profile_get(int unit,bcm_custom_meter_profile_t profile_id,bcm_custom_meter_profile_params_t *profile_params);
extern int drv_custom_meter_profile_set(int unit,bcm_custom_meter_profile_t profile_id,bcm_custom_meter_profile_params_t *profile_params);
extern int drv_custom_meter_params_t_init(int unit,bcm_custom_meter_params_t *params);
extern int drv_custom_meter_create(int unit,bcm_custom_meter_profile_t profile_id,bcm_custom_meter_params_t *meter_params,bcm_custom_meterid_t *meter_id);
extern int drv_custom_meter_destroy(int unit,bcm_custom_meterid_t meter_id);
extern int drv_custom_meter_pm_params_t_init(int unit,bcm_custom_meter_pm_params_t *params);
extern int drv_custom_meter_pm_enable(int unit,bcm_custom_meterid_t meter_id);
extern int drv_custom_meter_pm_disable(int unit,bcm_custom_meterid_t meter_id);
extern int drv_custom_meter_pm_get(int unit,bcm_custom_meterid_t meter_id,bcm_custom_meter_pm_params_t *pm_params);
extern int drv_custom_cosq_init(int unit);
extern int drv_custom_flowcontrol_params_t_init(int unit,bcm_gport_t portid,bcm_custom_flowcontrol_params_t *flowcontrol_params);
extern int drv_custom_cosq_shaper_params_t_init(int unit,bcm_gport_t portid,bcm_custom_cosq_shaper_params_t *shaper_params);
extern int drv_custom_cosq_tsn_params_t_init(int unit,bcm_custom_cosq_tsn_params_t *tsn_params);
extern int drv_custom_cosq_port_default_tc_set(int unit,bcm_gport_t portid,bcm_custom_tc_t tc);
extern int drv_custom_cosq_port_default_tc_get(int unit,bcm_gport_t portid,bcm_custom_tc_t *tc);
extern int drv_custom_cosq_port_default_pcp_set(int unit,bcm_gport_t portid,int pcp,int dei);
extern int drv_custom_cosq_port_default_pcp_get(int unit,bcm_gport_t portid,int *pcp,int *dei);
extern int drv_custom_cosq_port_pcp2tc_set(int unit,bcm_gport_t portid,int pcp,int dei,bcm_custom_tc_t tc);
extern int drv_custom_cosq_port_pcp2tc_get(int unit,bcm_gport_t portid,int pcp,int dei,bcm_custom_tc_t *tc);
extern int drv_custom_cosq_port_tc2pcp_set(int unit,bcm_gport_t portid,bcm_custom_tc_t tc,int pcp,int dei);
extern int drv_custom_cosq_port_tc2pcp_get(int unit,bcm_gport_t portid,bcm_custom_tc_t tc,int *pcp,int *dei);
extern int drv_custom_cosq_dscp2tc_set(int unit,bcm_custom_dscp_t dscp,bcm_custom_tc_t tc);
extern int drv_custom_cosq_dscp2tc_get(int unit,bcm_custom_dscp_t dscp,bcm_custom_tc_t *tc);
extern int drv_custom_cosq_tc2dscp_set(int unit,bcm_custom_tc_t tc,bcm_custom_dscp_t dscp);
extern int drv_custom_cosq_tc2dscp_get(int unit,bcm_custom_tc_t tc,bcm_custom_dscp_t *dscp);
extern int drv_custom_cosq_port_sched_get(int unit,bcm_gport_t portid,bcm_custom_schedule_mode_t *sched_mode,int *weights);
extern int drv_custom_cosq_port_sched_set(int unit,bcm_gport_t portid,bcm_custom_schedule_mode_t sched_mode,int *weights);
extern int drv_custom_cosq_flowcontrol_get(int unit,bcm_custom_flowcontrol_t *flowcontrol_mode);
extern int drv_custom_cosq_flowcontrol_set(int unit,bcm_custom_flowcontrol_t flowcontrol_mode);
extern int drv_custom_cosq_port_flowcontrol_get(int unit,bcm_gport_t portid,bcm_custom_flowcontrol_params_t *fc_params);
extern int drv_custom_cosq_port_flowcontrol_set(int unit,bcm_gport_t portid,bcm_custom_flowcontrol_params_t fc_params);
extern int drv_custom_cosq_pfc_flowcontrol_get(int unit,int int_pri,bcm_custom_flowcontrol_params_t *fc_params);
extern int drv_custom_cosq_pfc_flowcontrol_set(int unit,int int_pri,bcm_custom_flowcontrol_params_t fc_params);
extern int drv_custom_cosq_port_shaper_get(int unit,bcm_gport_t portid,bcm_custom_cosq_shaper_params_t *shaper_params);
extern int drv_custom_cosq_port_shaper_set(int unit,bcm_gport_t portid,bcm_custom_cosq_shaper_params_t *shaper_params);
extern int drv_custom_cosq_queue_shaper_get(int unit,bcm_gport_t portid,int int_pri,bcm_custom_cosq_shaper_params_t *shaper_params);
extern int drv_custom_cosq_queue_shaper_set(int unit,bcm_gport_t portid,int int_pri,bcm_custom_cosq_shaper_params_t *shaper_params);
extern int drv_custom_cosq_queue_tsn_get(int unit,bcm_gport_t portid,int int_pri,bcm_custom_cosq_tsn_params_t *tsn_params);
extern int drv_custom_cosq_queue_tsn_set(int unit,bcm_gport_t portid,int int_pri,bcm_custom_cosq_tsn_params_t *tsn_params);
extern int drv_custom_auth_params_t_init(int unit,bcm_custom_auth_params_t *auth_params);
extern int drv_custom_auth_port_mode_t_init(int unit,bcm_custom_auth_port_mode_t *auth_port_mode);
extern int drv_custom_auth_init(int unit,bcm_custom_auth_params_t *auth_params);
extern int drv_custom_auth_detach(int unit,bcm_custom_auth_params_t *auth_params);
extern int drv_custom_auth_mode_get(int unit,bcm_custom_auth_params_t *auth_params);
extern int drv_custom_auth_mode_set(int unit,bcm_custom_auth_params_t *auth_params);
extern int drv_custom_auth_port_get(int unit,bcm_gport_t portid,bcm_custom_auth_port_mode_t *port_mode);
extern int drv_custom_auth_port_set(int unit,bcm_gport_t portid,bcm_custom_auth_port_mode_t *port_mode);
extern int drv_custom_ts_init(int unit);
extern int drv_custom_ts_gen_config_set(int unit,int generator_id,bcm_custom_ts_gen_config_t *config);
extern int drv_custom_ts_event_config_set(int unit,int event_id,int event_source,bcm_custom_ts_event_config_t *config);
extern int drv_custom_ts_tod_get(int unit,bcm_custom_ts_time_t *time,uint64 *timestamp);
extern int drv_custom_ts_tod_set(int unit,bcm_custom_ts_time_t time,uint64 timestamp);
extern int drv_port_pkt_flow_enable(int unit);
#endif /* _DRV_IF_H */
