...
typedef enum drv_port_prop_e {
    ...
    DRV_PORT_PROP_MCAST_PFM /* Configure multicast pfm mode */
#else
    DRV_PORT_PROP_PPPOE_PARSE_EN /* Enable to parse PPPoE session packets */
...
typedef enum drv_port_mcast_pfm_type_e {
    DRV_PORT_PROP_MCAST_FLOOD_UNKNOWN = 1,
    DRV_PORT_PROP_MCAST_FLOOD_NONE
} drv_port_mcast_pfm_type_t;

typedef enum drv_port_trust_mode_e {
    DRV_PORT_PROP_TRUST_MODE_UNTRUSTED = 1,
    DRV_PORT_PROP_TRUST_MODE_DOT1P,
    DRV_PORT_PROP_TRUST_MODE_IP_DSCP
} drv_port_trust_mode_t;
...
/* Ethernet AV stream classes */
typedef enum drv_eav_stream_class_e {
    DRV_EAV_STREAM_CLASSA,
    DRV_EAV_STREAM_CLASSB
} drv_eav_stream_class_t;
 ...
 */
typedef struct drv_mirror_dest_t {
    uint32           mirror_destid;  /* Mirror destination ID */
    uint32           flags;          /* Flags:  DRV_MIRROR_DEST_XX */
    uint32           port;           /* Mirror destination. */
    sal_mac_addr_t   src_mac;        /* encap L2 source MAC address. */
    sal_mac_addr_t   dst_mac;        /* encap L2 destination MAC address. */
    uint32           vlan_id;        /* encap VLAN ID. */
    uint32           encap;          /* L3/SPAN/ERSPAN encapsulation */
} drv_mirror_dest_t;
...
typedef enum drv_dp_s {
    drvDpGreen,         /*(0): Indicates a packet with low drop precedence. */
    drvDpYellow,        /*(1): Indicates a packet of medium drop precedence. */
    drvDpRed,           /*(2): The highest drop precedence. */
    drvDpDrop,          /*(3): Used as a drop indicator. */
} drv_dp_t;


typedef struct drv_tc_s {
    int int_pri;    /* Internal Priority */
    int int_dp;     /* Internal Drop Precedence */
} drv_tc_t;

typedef struct drv_packet_cb_info_s {
...
} drv_packet_cb_info_t;

/* Packet attribute structure */
typedef struct drv_packet_attr_s {
    uint32 flags;        /* Flags:  CBX_PACKET_XX */
    uint32 vlanid;       /* Vlan ID on which the packet was received */
    drv_tc_t tc;         /* traffic class priority and drop precedence */
    int source_index;    /* port index */
    int dest_index;      /* port index */
    uint32 source_port;  /* The port on which the packet arrived */
    uint32 dest_port;    /* RX: The port to which the packet was mapped,
                                           TX: The destination port */
    uint32 mcastid;      /* Destination multicast group */
    uint32 trapid;       /* Trap ID */
    uint32 trap_data;    /* additional trap information */
    uint32 ing_ptp_ts;   /* Ingress PTP timestamp */
    uint32 unit;
} drv_packet_attr_t;

typedef int ( *drv_packet_io_callback_t ) (void *buffer, int buffer_size,
                      drv_packet_attr_t    *packet_attr,
                      drv_packet_cb_info_t *cb_info );

/* Linkscan call back */
typedef void ( *drv_link_scan_callback_t )(uint32 portid, uint32 value);
