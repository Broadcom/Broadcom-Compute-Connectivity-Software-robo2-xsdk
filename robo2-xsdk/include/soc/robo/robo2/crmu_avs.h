...
#define iproc_reg32_read(addr)      (*((volatile uint32_t*)(addr)))
#define iproc_reg32_write(addr, data)   \
    do {\
...
#define IPROC_REG32_BIT_GET(addr, bitno) (iproc_reg32_read(addr)&(1<<(bitno))?1:0)
#define IPROC_REG32_BIT_SET(addr, bitno) iproc_reg32_write(addr, iproc_reg32_read(addr)|(1<<bitno))
#define IPROC_REG32_BIT_CLEAR(addr, bitno) iproc_reg32_write(addr, iproc_reg32_read(addr)&(~(1<<bitno)))

int avs3_bsti_get_core_voltage(uint32_t *core_voltage);
int avs3_bsti_set_core_voltage(uint32_t core_voltage);
...
#define AVS_AVENGER_NUMBER_OF_CENTRALS     24   /* there are 36 rosc in Avenger, but only 20(0~7, 12~23) used, others skipped */
...
#define AVS_AVENGER_CENTRAL_XBMP1          0xFFFFFFFe   /* SKIP 33~35 */
#define AVS_AVENGER_PVT_MNTR_XBMP1         0xFFFFFFF6   /* SKIP all except Temperature and 1V_0 */
...
#define AVS_AVENGER_VMAX_AVS_A0            AVS_DEFAULT_VMAX_AVS
#define AVS_AVENGER_VMARGIN_HIGH_A0        AVS_DEFAULT_VMARGIN_HIGH
#define AVS_AVENGER_VMARGIN_LOW_A0         AVS_DEFAULT_VMARGIN_LOW
#define AVS_AVENGER_VMAX_AVS_B0            1.05
#define AVS_AVENGER_VMARGIN_HIGH_B0        0.091
#define AVS_AVENGER_VMARGIN_LOW_B0         0.038
...
extern int avs_init(int unit);
extern int avs_start(int unit);
/* ------------------------------------------------------------------------- */
