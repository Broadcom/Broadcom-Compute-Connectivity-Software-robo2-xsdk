...
*/
err_code_t eagle_phy_2_pmd_mwr_reg(const phymod_access_t *pa, uint16_t address, uint16_t mask, uint8_t lsb, uint16_t val) { 
    ...
    for(i=1; i <= 0x8; i = i << 1) {
        if ( i & pa->lane_mask ) {
            pa_copy.lane_mask = i;
            error_code+=phymod_tsc_iblk_write(&pa_copy, (PHYMOD_REG_ACC_RAW | 0x8000000 | (uint32_t) address), data);
        }
    }
    if(error_code)
...
err_code_t eagle_phy_2_pmd_mwr_reg_addr32(const phymod_access_t *pa, uint32_t address, uint16_t mask, uint8_t lsb, uint16_t val) { 
    ...
    for(i=1; i <= 0x8; i = i << 1) {
        if ( i & pa->lane_mask ) {
            pa_copy.lane_mask = i;
            error_code+=phymod_tsc_iblk_write(&pa_copy, (0x10000 | (uint32_t) address), data);
        }
    }
    if(error_code)
      return  ERR_CODE_DATA_NOTAVAIL;
...
uint16_t eagle_phy_2_pmd_rd_reg(const phymod_access_t *pa, uint16_t address){

    uint32_t data;
    phymod_tsc_iblk_read(pa, (PHYMOD_REG_ACC_RAW | 0x8000000 | (uint32_t) address), &data);
    data = data & 0xffff; 
...
/* phymod_tsc_iblk_read(const phymod_access_t *pa, uint32_t addr, uint32_t *data) */
uint16_t eagle_phy_2_pmd_rd_reg_addr32(const phymod_access_t *pa, uint32_t address){

    uint32_t data;
    phymod_tsc_iblk_read(pa, (0x10000 | (uint32_t) address), &data);
    data = data & 0xffff; 
    return ( (uint16_t)data );
}
...
err_code_t eagle_phy_2_pmd_wr_reg(const phymod_access_t *pa, uint16_t address, uint16_t val){
    ...
    error_code = phymod_tsc_iblk_write(pa, (PHYMOD_REG_ACC_RAW | 0x8000000 | (uint32_t) address), data);
    if(error_code)
      return  ERR_CODE_DATA_NOTAVAIL;
    return  ERR_CODE_NONE;
}
...
uint8_t eagle_phy_2_get_core(void) {
    return(0);
}

err_code_t eagle_phy_2_uc_lane_idx_to_system_id(char *string , uint8_t uc_lane_idx) {
    static char info[256];
   /* Indicates Eagle Core */
    PHYMOD_SPRINTF(info, "%s_%x", "FC_", uc_lane_idx);
