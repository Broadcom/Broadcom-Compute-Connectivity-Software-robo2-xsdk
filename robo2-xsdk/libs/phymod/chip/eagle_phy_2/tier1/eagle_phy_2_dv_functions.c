/**************************************************************************************
 **************************************************************************************
 *  File Name     :  eagle_phy_2_dv_functions.c                                        *
 *  Created On    :  22/05/2014                                                       *
 *  Created By    :  Kiran Divakar                                                    *
 *  Description   :  APIs for Serdes IPs ported over from DV                          *
 *  Revision      :  $Id: eagle_phy_2_dv_functions.c 1490 2016-08-11 22:26:06Z cvazquez $ *
 *                                                                                    *
 * 
...
/** @file eagle_phy_2_dv_functions.c
...
err_code_t eagle_phy_2_isolate_lane_ctrl_pins( const phymod_access_t *pa, uint8_t enable);
err_code_t eagle_phy_2_isolate_core_ctrl_pins( const phymod_access_t *pa, uint8_t enable);
...
err_code_t eagle_phy_2_config_shared_tx_pattern( const phymod_access_t *pa, uint8_t patt_length, const char pattern[]) {
  ...
  if ((str_len > 2) && ((USR_STRNCMP(pattern, "0x", 2)) == 0)) {
    ...
    for (i=2; i < str_len; i++) {
      ...
      if (pattern[i] != '_') {
        ...
        if (actual_patt_len > 240) {
          ...
          return (_error(RR_CODE_CFG_PATT_PATTERN_BIGGER_THAN_MAXLEN));
        }
      }
    }
    ...
    if ((offset_len > 3)  || (actual_patt_len < patt_length)) {
      ...
      return (_error(RR_CODE_CFG_PATT_LEN_MISMATCH));
    }
    else if (offset_len) {
    ...
    for (i=0; i < str_len; i++) {
      if ((pattern[i] == '0') || (pattern[i] == '1')) {
        ...
        return (_error(RR_CODE_CFG_PATT_INVALID_PATTERN));
      }
    }
  ...
  for (i=0; i < 15; i++) {
    ...
    switch (i) {
      ...
      default: EFUN_PRINTF(("ERROR: Invalid write to patt_gen_seq register\n"));
               return (_error(RR_CODE_CFG_PATT_INVALID_SEQ_WRITE));
               break;
    }
  }
...
err_code_t eagle_phy_2_tx_shared_patt_gen_en( const phymod_access_t *pa, uint8_t enable, uint8_t patt_length) {
  ...
  if (enable) {
      if ((mode_sel < 1) || (mode_sel > 6)) {
        return (_error(RR_CODE_PATT_GEN_INVALID_MODE_SEL));
      }
...
err_code_t eagle_phy_2_pmd_lock_status( const phymod_access_t *pa, uint8_t *pmd_rx_lock) {
    if(!pmd_rx_lock) {
        return(_error(RR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
...
err_code_t eagle_phy_2_apply_txfir_cfg( const phymod_access_t *pa, int8_t pre, int8_t main, int8_t post1, int8_t post2, int8_t post3) {

  err_code_t failcode = eagle_phy_2_validate_txfir_cfg( pa, pre, main, post1, post2, post3);
...
err_code_t eagle_phy_2_prbs_err_count_ll( const phymod_access_t *pa, uint32_t *prbs_err_cnt) {
    ...
    EFUN(eagle_phy_2_pmd_rdt_reg(pa,TLB_RX_PRBS_CHK_ERR_CNT_MSB_STATUS, &rddata));
...
err_code_t eagle_phy_2_prbs_err_count_state( const phymod_access_t *pa, uint32_t *prbs_err_cnt, uint8_t *lock_lost) {
    ...
    EFUN(eagle_phy_2_prbs_err_count_ll( pa, prbs_err_cnt));
...
err_code_t eagle_phy_2_loop_timing( const phymod_access_t *pa, uint8_t enable) {
    ...
    if (enable) {
        ...
        EFUN(eagle_phy_2_delay_us(25));               /* Wait for tclk to lock to CDR */
    } else {
...
err_code_t eagle_phy_2_rmt_lpbk( const phymod_access_t *pa, uint8_t enable) {
    if (enable) {
        EFUN(eagle_phy_2_loop_timing( pa, enable));
        ...
        EFUN(eagle_phy_2_delay_us(50));        /* Wait for rclk and tclk phase lock before expecing good data from rmt loopback */
...
err_code_t eagle_phy_2_tx_pi_jitt_gen( const phymod_access_t *pa, uint8_t enable, int16_t freq_override_val, enum srds_tx_pi_freq_jit_gen_enum jit_type, uint8_t tx_pi_jit_freq_idx, uint8_t tx_pi_jit_amp) {
    ...
    EFUN(eagle_phy_2_tx_pi_freq_override( pa, enable, freq_override_val));
...
err_code_t eagle_phy_2_uc_reset( const phymod_access_t *pa, uint8_t enable) {
  if (enable) {
    ...
    EFUN(eagle_phy_2_pmd_wr_reg(pa,0xD200, 0x0000));
    EFUN(eagle_phy_2_pmd_wr_reg(pa,0xD201, 0x0000));
    EFUN(eagle_phy_2_pmd_wr_reg(pa,0xD202, 0x0000));
    EFUN(eagle_phy_2_pmd_wr_reg(pa,0xD203, 0x0000));
    EFUN(eagle_phy_2_pmd_wr_reg(pa,0xD207, 0x0000));
    EFUN(eagle_phy_2_pmd_wr_reg(pa,0xD208, 0x0000));
    EFUN(eagle_phy_2_pmd_wr_reg(pa,0xD20A, 0x080f));
    EFUN(eagle_phy_2_pmd_wr_reg(pa,0xD20C, 0x0002));
    EFUN(eagle_phy_2_pmd_wr_reg(pa,0xD20D, 0x0000));
  }
  else {
    /* De-assert micro reset - Start executing code */
      // Clock enable and system_rest_n is already asserted while loading uCode
...
err_code_t eagle_phy_2_core_pwrdn( const phymod_access_t *pa, enum srds_core_pwrdn_mode_enum mode) {
    switch(mode) {
    case PWR_ON:
        EFUN(_eagle_phy_2_core_clkgate( pa, 0));
        EFUN(wrc_afe_s_pll_pwrdn(0x0));
        EFUN(wrc_core_dp_s_rstb(0x1));
        break;
    case PWRDN:
        EFUN(wrc_core_dp_s_rstb(0x0));
        EFUN(eagle_phy_2_delay_ns(   500)); /* wait >50 comclk cycles  */
    ...
    case PWRDN_DEEP:
        ...
        EFUN(eagle_phy_2_delay_ns(   500)); /* wait >50 comclk cycles  */
                EFUN(wrc_afe_s_pll_pwrdn(0x1));
        EFUN(_eagle_phy_2_core_clkgate( pa, 1));
        break;
    default:
        EFUN(_error(RR_CODE_BAD_PTR_OR_INVALID_INPUT));
        break;
    }
    return ERR_CODE_NONE;
}
...
err_code_t eagle_phy_2_lane_pwrdn( const phymod_access_t *pa, enum srds_core_pwrdn_mode_enum mode) {
    ...
    case PWR_ON:
        ...
        EFUN(_eagle_phy_2_lane_clkgate( pa, 0));
        break;
    case PWRDN:
    ...
    case PWRDN_DEEP:
        ...
        EFUN(wr_ln_rx_s_pwrdn(0x1));
        EFUN(wr_ln_tx_s_pwrdn(0x1));
        EFUN(_eagle_phy_2_lane_clkgate( pa, 1));
...
err_code_t eagle_phy_2_isolate_ctrl_pins( const phymod_access_t *pa, uint8_t enable) {

    EFUN(eagle_phy_2_isolate_lane_ctrl_pins( pa, enable));
    EFUN(eagle_phy_2_isolate_core_ctrl_pins( pa, enable));
...
err_code_t eagle_phy_2_ucode_mdio_load( const phymod_access_t *pa, uint8_t *ucode_image, uint16_t ucode_len) {
    ...
    if (ucode_len > UCODE_MAX_SIZE) {                     /* uCode size should be less than UCODE_MAX_SIZE  */
        return (_error(RR_CODE_INVALID_UCODE_LEN));
    }
    EFUN(wrc_pmd_core_dp_s_rstb(0x1));                    /* Core dp_s_rstb out of reset */
    ...
    EFUN(eagle_phy_2_delay_us(300));                      /* Wait for Initialization to finish */
#if 0
    //EFUN(wrc_micro_disable_ecc(0x1));                     /* Disabling ECC for Simulation backdoor load */
#endif
#ifndef ATE_LOG
    ESTM(result = !rdc_micro_init_done());
    if (result) {                                         /* Check if initialization done within 300us time interval */
        return (_error(RR_CODE_MICRO_INIT_NOT_DONE));    /* Else issue error code */
    }
#endif
    EFUN(wrc_micro_init_cmd(0x0));                        /* Clear initialization command */
    ucode_len_padded = ((ucode_len + 7) & 0xFFF8);        /* Aligning ucode size to 8-byte boundary */
    
    //ucode_len_padded = ((16 + 7) & 0xFFF8);               /* Aligning ucode size to 8-byte boundary */

    /* Code to Load microcode */
    EFUN(wrc_micro_ram_count(ucode_len_padded - 1));      /* Set number of bytes of ucode to be written */
    //EFUN(wrc_micro_ram_count(ucode_len_padded));          /* Set number of bytes of ucode to be written */
    ...
    //for (i = 0 ; i < 2048 ; i++) {
    //        eagle_phy_2_wrb_uc_ram(pa, i, 0x0);
    //}
#ifndef ATE_LOG
    ...
    if (result > 0) {            /* Look for errors in micro load FSM */
        ...
        return (_error(RR_CODE_UCODE_LOAD_FAIL));        /* Return 1 : ERROR while loading microcode (uCode Load FAIL) */
    }
    else {
        EFUN(wrc_micro_stop(0x0));                        /* Clear stop field */
...
err_code_t eagle_phy_2_ucode_load_verify( const phymod_access_t *pa, uint8_t *ucode_image, uint16_t ucode_len) {
    ...
    do {                                                  /* ucode_image read 16bits at a time */
        ...
        if (ram_data != rddata) {                                   /* Compare Program RAM ucode to ucode_image (Read to ram_rddata reg auto-increments the ram_address) */
            ...
            return (_error(RR_CODE_UCODE_VERIFY_FAIL));            /* Verify uCode FAIL */
        }
    }   while (count < ucode_len_padded);                 /* Loop repeated till entire image loaded (upto the 8byte boundary) */
...
err_code_t eagle_phy_2_wrw_uc_ram( const phymod_access_t *pa, uint16_t addr, uint16_t wr_val) {
    ...
    EFUN(eagle_phy_2_delay_ns(80));                          /* wait for 10 comclk cycles/ 80ns  */
    EFUN(wrc_micro_ram_wrdata(wr_val));                 /* RAM Write value */
    EFUN(eagle_phy_2_delay_ns(80));                          /* Wait for Data RAM to be written */
    return (ERR_CODE_NONE);
}

/* Micro RAM Byte Write */
err_code_t eagle_phy_2_wrb_uc_ram( const phymod_access_t *pa, uint16_t addr, uint8_t wr_val) {
    EFUN(wrc_micro_mdio_ram_access_mode(0x2));          /* Select Data RAM access through MDIO Register interface mode */
...
/* Micro RAM Word Read */
uint16_t eagle_phy_2_rdw_uc_ram( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
    uint16_t rddata;
    ...
    *err_code_p = ERR_CODE_NONE;
    ...
    EPFUN(eagle_phy_2_delay_ns(80));                         /* Wait for the RAM read  */
...
/* Micro RAM Byte Read */
uint8_t eagle_phy_2_rdb_uc_ram( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
    uint8_t rddata;
...
err_code_t eagle_phy_2_wrws_uc_ram( const phymod_access_t *pa, uint16_t addr, int16_t wr_val) {
    return (eagle_phy_2_wrw_uc_ram( pa, addr, wr_val));
}

/* Micro RAM Byte Signed Write */
err_code_t eagle_phy_2_wrbs_uc_ram( const phymod_access_t *pa, uint16_t addr, int8_t wr_val) {
    return (eagle_phy_2_wrb_uc_ram( pa, addr, wr_val));
}

/* Micro RAM Word Signed Read */
int16_t eagle_phy_2_rdws_uc_ram( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
    return ((int16_t)eagle_phy_2_rdw_uc_ram( pa, err_code_p, addr));
}

/* Micro RAM Byte Signed Read */
int8_t eagle_phy_2_rdbs_uc_ram( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
    return ((int8_t)eagle_phy_2_rdb_uc_ram( pa, err_code_p, addr));
}


/* Micro RAM Byte Read */
err_code_t eagle_phy_2_rdblk_uc_ram( const phymod_access_t *pa, uint8_t *mem, uint16_t addr, uint16_t cnt) {
    if(!mem) {
        return(_error(RR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    while (cnt--) {
        err_code_t error_code;
        uint8_t    tmp = eagle_phy_2_rdb_uc_ram( pa, &error_code, addr++); /* Value read from the required Data RAM Address */
        if (error_code) {
