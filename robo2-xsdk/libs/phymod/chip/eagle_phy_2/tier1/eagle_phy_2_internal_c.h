/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  eagle_phy_2_internal.c                                         *
 *  Created On    :  13/02/2014                                                    *
 *  Created By    :  Justin Gaither                                                *
 *  Description   :  APIs for Serdes IPs                                           *
 *  Revision      :  $Id: eagle_phy_2_internal.c 1490 2016-08-11 22:26:06Z cvazquez $ *
 *                                                                                 *
 * 
...
/** @file eagle_phy_2_internal.c
 * Implementation of API functions
 */

#include "eagle_phy_2_internal.h"
SDK_STATIC uint32_t _float12_to_uint32_3arg( const phymod_access_t *pa, uint8_t byte, uint8_t multi);
...
SDK_STATIC err_code_t _compute_bin( const phymod_access_t *pa, char var, char bin[]) {
  ...
  switch (var) {
    ...
    default :  ENULL_STRCPY(bin,"");
               ...
               return (_error(RR_CODE_CFG_PATT_INVALID_HEX));
               break;
  }
  return (ERR_CODE_NONE);
}
...
SDK_STATIC err_code_t _compute_hex( const phymod_access_t *pa, char bin[],uint8_t *hex) {
  ...
  if (!USR_STRCMP(bin,"0000")) {
    ...
    *hex = 0x0;
        return (_error(RR_CODE_CFG_PATT_INVALID_BIN2HEX));
...
SDK_STATIC uint8_t _eagle_phy_2_rdb_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
    uint8_t rddata;
    ...
    EPSTM(rddata = eagle_phy_2_rdb_uc_ram( pa, err_code_p, addr)); /* Use Micro register interface for reading RAM */

    return (rddata);
}

/* Micro Global RAM Word Read */
SDK_STATIC uint16_t _eagle_phy_2_rdw_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
  uint16_t rddata;
...
#endif
  EPSTM(rddata = eagle_phy_2_rdw_uc_ram( pa, err_code_p, (addr))); /* Use Micro register interface for reading RAM */

  return (rddata);
}

/* Micro Global RAM Byte Write  */
SDK_STATIC err_code_t _eagle_phy_2_wrb_uc_var( const phymod_access_t *pa, uint16_t addr, uint8_t wr_val) {

    return (eagle_phy_2_wrb_uc_ram( pa, addr, wr_val));                                /* Use Micro register interface for writing RAM */
}
/* Micro Global RAM Word Write  */
SDK_STATIC err_code_t _eagle_phy_2_wrw_uc_var( const phymod_access_t *pa, uint16_t addr, uint16_t wr_val) {
    if (addr%2 != 0) {                                                                /* Validate even address */
        return (_error(RR_CODE_INVALID_RAM_ADDR));
    }
    return (eagle_phy_2_wrw_uc_ram( pa, addr, wr_val));                                 /* Use Micro register interface for writing RAM */
}


SDK_STATIC err_code_t _eagle_display_event(const phymod_access_t *pa,uint8_t event_id,
                          uint8_t entry_len,
                          uint8_t prev_cursor,
                          uint8_t curr_cursor,
                          uint8_t post_cursor,
...
SDK_STATIC err_code_t _set_rx_pf_main( const phymod_access_t *pa, uint8_t val) {
        if (val > 15) {
                return (_error(RR_CODE_PF_INVALID));
        }
        EFUN(wr_pf_ctrl(val));
        return(ERR_CODE_NONE);
}
...
SDK_STATIC err_code_t _set_rx_vga( const phymod_access_t *pa, uint8_t val) {
    ...
    EFUN(_check_uc_lane_stopped( pa ));                     /* make sure uC is stopped to avoid race conditions */
        ...
        if (val > 45) {
                return (_error(RR_CODE_VGA_INVALID));
        }
...
SDK_STATIC err_code_t _set_rx_dfe1( const phymod_access_t *pa, int8_t val) {
...

    {
        ...
        if (val > 63) {
            return (_error(RR_CODE_DFE1_INVALID));
        }
...
SDK_STATIC err_code_t _set_rx_dfe2( const phymod_access_t *pa, int8_t val) {
    ...
    if ((val > 31) || (val < -31)) {
        return (_error(RR_CODE_DFE2_INVALID));
    }
...
SDK_STATIC err_code_t _set_rx_dfe3( const phymod_access_t *pa, int8_t val) {
    if ((val > 31) || (val < -31)) {
        return (_error(RR_CODE_DFE3_INVALID));
    }
...
SDK_STATIC err_code_t _set_rx_dfe4( const phymod_access_t *pa, int8_t val) {
    if ((val > 15) || (val < -15)) {
            return (_error(RR_CODE_DFE4_INVALID));
    }
...
SDK_STATIC err_code_t _set_rx_dfe5( const phymod_access_t *pa, int8_t val) {
    if ((val > 15) || (val < -15)) {
        return (_error(RR_CODE_DFE5_INVALID));
    }
...
SDK_STATIC err_code_t _set_tx_post1( const phymod_access_t *pa, uint8_t val) {
    if (val > 63) {
        return (_error(RR_CODE_TXFIR_POST1_INVALID));
    }
...
SDK_STATIC err_code_t _set_tx_pre( const phymod_access_t *pa, uint8_t val) {
    if (val > 31) {
        return (_error(RR_CODE_TXFIR_PRE_INVALID));
    }
...
SDK_STATIC err_code_t _set_tx_main( const phymod_access_t *pa, uint8_t val) {
    if (val > 112) {
        return (_error(RR_CODE_TXFIR_MAIN_INVALID));
    }
    EFUN(wr_txfir_main_override(val));
    return(ERR_CODE_NONE);
}

SDK_STATIC err_code_t _set_tx_post2( const phymod_access_t *pa, int8_t val) {
    if ((val > 15) || (val < -15)) {
        return (_error(RR_CODE_TXFIR_POST2_INVALID));
    }
    /* sign? */
...
SDK_STATIC err_code_t _set_tx_post3( const phymod_access_t *pa, int8_t val) {
    if ((val > 7) || (val < -7)) {
        return (_error(RR_CODE_TXFIR_POST3_INVALID));
    }
    /* sign? */
...
SDK_STATIC err_code_t _get_tx_post3( const phymod_access_t *pa, int8_t *val) {
    ESTM(*val = rd_txfir_post3_adjusted());
    return(ERR_CODE_NONE);
}


SDK_STATIC err_code_t _set_tx_amp( const phymod_access_t *pa, int8_t val) {
    if (val > 15) {
        return (_error(RR_CODE_TX_AMP_CTRL_INVALID));
    }
...
SDK_STATIC err_code_t _get_tx_amp( const phymod_access_t *pa, int8_t *val) {
    ESTM(*val = rd_ams_tx_amp_ctl());
    return(ERR_CODE_NONE);
}

SDK_STATIC err_code_t _get_tx_driver_mode( const phymod_access_t *pa, int8_t *val) {
    ESTM(*val = rd_ams_tx_drivermode());
    return(ERR_CODE_NONE);
}


SDK_STATIC err_code_t _eagle_phy_2_core_clkgate( const phymod_access_t *pa, uint8_t enable) {
...
SDK_STATIC err_code_t _eagle_phy_2_lane_clkgate( const phymod_access_t *pa, uint8_t enable) {
...
SDK_STATIC uint16_t _eye_to_mV(uint8_t var, uint8_t ladder_range)
{
    ...
    uint16_t vl = _ladder_setting_to_mV(NULL,var/8, ladder_range);
    uint16_t vu = _ladder_setting_to_mV(NULL,_min(31,var/8+1), ladder_range);
    return (vl + (vu-vl)*(var&7)/8);
}

SDK_STATIC err_code_t _eagle_phy_2_get_osr_mode( const phymod_access_t *pa, eagle_phy_2_osr_mode_st *imode) {
    eagle_phy_2_osr_mode_st mode;

     ENULL_MEMSET(&mode, 0, sizeof(eagle_phy_2_osr_mode_st));
...
SDK_STATIC err_code_t _eagle_phy_2_read_lane_state( const phymod_access_t *pa, eagle_phy_2_lane_state_st *istate) {

  eagle_phy_2_lane_state_st state;
  uint8_t ladder_range = 0;

  ENULL_MEMSET(&state, 0, sizeof(eagle_phy_2_lane_state_st));

  if(!istate)
          return(_error(RR_CODE_BAD_PTR_OR_INVALID_INPUT));

  EFUN(_eagle_phy_2_pmd_lock_status( pa, &state.rx_lock, &state.rx_lock_chg));
  ...
  EFUN(_eagle_phy_2_sigdet_status( pa, &state.sig_det, &state.sig_det_chg));
  ESTM(state.rx_ppm = rd_cdr_integ_reg()/84);
  EFUN(eagle_phy_2_get_clk90_offset( pa, &state.clk90));
...
SDK_STATIC err_code_t _eagle_phy_2_display_lane_state_no_newline( const phymod_access_t *pa ) {
  uint16_t lane_idx;
  eagle_phy_2_lane_state_st state;
  ...
  EFUN(_eagle_phy_2_read_lane_state( pa, &state));

  lane_idx = eagle_phy_2_get_lane(pa);
...
SDK_STATIC err_code_t _eagle_phy_2_read_core_state( const phymod_access_t *pa, eagle_phy_2_core_state_st *istate) {
  eagle_phy_2_core_state_st state;
  struct eagle_phy_2_uc_core_config_st core_cfg;

  ENULL_MEMSET(&state, 0, sizeof(eagle_phy_2_core_state_st));
  ENULL_MEMSET(&core_cfg, 0, sizeof(struct eagle_phy_2_uc_core_config_st));
  ...
  {  int16_t                           die_temp = 0;
     EFUN(eagle_phy_2_read_die_temperature( pa, &die_temp));
  ...
  EFUN(_eagle_phy_2_pll_lock_status( pa, &state.pll_lock, &state.pll_lock_chg));
...
SDK_STATIC err_code_t _eagle_phy_2_display_core_state_no_newline( const phymod_access_t *pa ) {
    eagle_phy_2_core_state_st state;
    int i,j;
    ENULL_MEMSET(&state     , 0, sizeof(state     ));
        EFUN(_eagle_phy_2_read_core_state( pa, &state));
    EFUN_PRINTF(("%02d  "              ,  eagle_phy_2_get_core()));
    ...
    if ( state.comclk_mhz > 4 ) {
    i = state.comclk_mhz/4;
    j = (state.comclk_mhz%4)*25;
    EFUN_PRINTF((" %3d.%02dMHz"        , i,j));
    } else {
    EFUN_PRINTF((" 000.00MHz"        ));    
    }
    ...
    if ( state.vco_rate_mhz > 1000 ) {
    i = state.vco_rate_mhz/1000;
    j = state.vco_rate_mhz % 1000;
    EFUN_PRINTF(("  %2d.%03dGHz ",i, j));
    } else {
    EFUN_PRINTF(("  00.000GHz "));
    }
    EFUN_PRINTF(("    %03d        "    ,  state.analog_vco_range));
    EFUN_PRINTF(("(%02d) "             ,  state.pll_div));
    EFUN(_eagle_phy_2_display_pll_to_divider( pa,   state.pll_div));
...
SDK_STATIC err_code_t _eagle_phy_2_pll_lock_status( const phymod_access_t *pa, uint8_t *pll_lock, uint8_t *pll_lock_chg) {

    uint16_t rddata;
    EFUN(eagle_phy_2_pmd_rdt_reg(pa,PLL_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _eagle_phy_2_pmd_lock_status( const phymod_access_t *pa, uint8_t *pmd_lock, uint8_t *pmd_lock_chg) {

    uint16_t rddata;
    EFUN(eagle_phy_2_pmd_rdt_reg(pa,PMD_LOCK_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _eagle_phy_2_sigdet_status( const phymod_access_t *pa, uint8_t *sig_det, uint8_t *sig_det_chg) {
    uint16_t rddata;
    EFUN(eagle_phy_2_pmd_rdt_reg(pa,SIGDET_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _update_uc_lane_config_st( const phymod_access_t *pa, struct  eagle_phy_2_uc_lane_config_st *st) {
  uint16_t in = st->word;
...
SDK_STATIC err_code_t _update_usr_ctrl_disable_functions_st( const phymod_access_t *pa, struct eagle_phy_2_usr_ctrl_disable_functions_st *st) {
  uint8_t in = st->byte;
...
SDK_STATIC err_code_t _update_usr_ctrl_disable_dfe_functions_st( const phymod_access_t *pa, struct eagle_phy_2_usr_ctrl_disable_dfe_functions_st *st) {
  uint8_t in = st->byte;
...
SDK_STATIC err_code_t _update_uc_core_config_st( const phymod_access_t *pa, struct eagle_phy_2_uc_core_config_st *st) {
  uint16_t in = st->word;
...
SDK_STATIC err_code_t _update_uc_core_config_word( const phymod_access_t *pa, struct eagle_phy_2_uc_core_config_st *st) {
  uint16_t in = 0;
...
SDK_STATIC err_code_t _update_uc_lane_config_word( const phymod_access_t *pa, struct eagle_phy_2_uc_lane_config_st *st) {
  uint16_t in = 0;
...
SDK_STATIC err_code_t _update_usr_ctrl_disable_functions_byte( const phymod_access_t *pa, struct eagle_phy_2_usr_ctrl_disable_functions_st *st) {
  uint8_t in = 0;
...
SDK_STATIC err_code_t _update_usr_ctrl_disable_dfe_functions_byte( const phymod_access_t *pa, struct  eagle_phy_2_usr_ctrl_disable_dfe_functions_st *st) {
  uint8_t in = 0;
...
SDK_STATIC err_code_t _check_uc_lane_stopped( const phymod_access_t *pa ) {
  ...
  if (!is_micro_stopped) {
      return(_error(RR_CODE_UC_NOT_STOPPED));
  } else {
      return(ERR_CODE_NONE);
  }
}
...
SDK_STATIC err_code_t _calc_patt_gen_mode_sel( const phymod_access_t *pa, uint8_t *mode_sel, uint8_t *zero_pad_len, uint8_t patt_length) {
  ...
  if (!(140 % patt_length)) {
    ...
    *zero_pad_len = 0;
    ...
    return (_error(RR_CODE_CFG_PATT_INVALID_PATT_LENGTH));
  }
  return(ERR_CODE_NONE);
}

SDK_STATIC err_code_t _eagle_phy_2_display_pll_to_divider( const phymod_access_t *pa, uint8_t val) {
...
SDK_STATIC err_code_t _eagle_phy_2_display_ber_scan_data( const phymod_access_t *pa, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    uint8_t range250;
    eagle_phy_2_osr_mode_st osr_mode;
    struct eagle_phy_2_uc_core_config_st core_config;

    ENULL_MEMSET(&core_config,0,sizeof(core_config));
    ENULL_MEMSET(&osr_mode,0,sizeof(osr_mode));

    EFUN(eagle_phy_2_get_uc_core_config( pa, &core_config));
    EFUN(_eagle_phy_2_get_osr_mode( pa, &osr_mode));
    ...
    ESTM(sts = rdv_usr_diag_status());
    if((sts & 0x8000) == 0) {
        return(_error(RR_CODE_DATA_NOTAVAIL));
    }
    cnt = (sts & 0x00FF)/3;
    for(i=0;i < cnt;i++) {
        /* Read 2 bytes of data */
        EFUN(eagle_phy_2_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_WORD, 0, 100));
        ...
        EFUN(eagle_phy_2_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_BYTE, 0, 100));
        ESTM(prbs_byte = (uint8_t)rd_uc_dsc_data());
        errors = _float12_to_uint32_3arg( pa, prbs_byte,prbs_multi); /* convert 12bits to uint32 */
        timer_values = (_float12_to_uint32_3arg( pa, time_byte,time_multi)<<3);
        if(verbose < 5) {
            if (!(i % 4))  {
                EFUN_PRINTF(("\n"));
            }
    ...
    EFUN_PRINTF(("\n"));
    EFUN(eagle_phy_2_pmd_uc_cmd( pa, CMD_CAPTURE_BER_END,0x00,2000));
