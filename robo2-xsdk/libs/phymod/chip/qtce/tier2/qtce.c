...
int qtce_core_identify(const phymod_core_access_t* core, uint32_t core_id, uint32_t* is_identified)
{
    ...
    *is_identified = 0;
    ...
    PHYMOD_VDBG(DBG_CFG, pm_acc,("%-22s: core_id=%0d adr=%0x lane_mask=%0x\n", 
                           __func__, core_id, pm_acc->addr, pm_acc->lane_mask));
#endif
    if(core_id == 0){
...
#ifdef VENDOR_BROADCOM
	PHYMOD_VDBG(QMOD_DBG_CFG, pm_acc,("%-22s: core_id=%0d identified=%0d rv=%0d adr=%0x lmask=%0x\n", 
              __func__, core_id, *is_identified, rv, pm_acc->addr, pm_acc->lane_mask));
#endif
    return rv ;
}
...
int qtce_core_lane_map_set(const phymod_core_access_t* core, const phymod_lane_map_t* lane_map)
{
    ...
    PHYMOD_VDBG(DBG_TOP, pm_acc,
       ("%-22s: p=%p adr=%0x lmask=%0x rx_lane_map=%0x%0x%0x%0x tx_lane_map=%0x%0x%0x%0x\n", 
...
#ifdef VENDOR_BROADCOM
    PHYMOD_VDBG(QMOD_DBG_TOP, pm_acc,
       ("%-22s: p=%p adr=%0x lmask=%0x pcs_swap=%0x addr_index_swap=%0x tx_map=%0x\n", 
          __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask,
...
int _qtce_core_firmware_load(const phymod_core_access_t* core, phymod_firmware_load_method_t load_method, phymod_firmware_loader_f fw_loader)
{
    ...
    PHYMOD_VDBG(DBG_CFG, pm_acc,
       ("%-22s: p=%p adr=%0x lmask=%0x load_meth=%0d",
...
{  
    ...
    const phymod_access_t *pm_acc;
    pm_acc = &phy->access;
    PHYMOD_VDBG(DBG_SPD, pm_acc, ("%-22s: p=%p adr=%0x lmask=%0x speed=%0d intf=%0d(%s) flags=%0x\n",
...
#ifdef VENDOR_BROADCOM
    PHYMOD_VDBG(QMOD_DBG_SPD, pm_acc, ("%-22s: p=%p adr=%0x lmask=%0x spd=%0d(%s) media=%0d\n",
       __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask, spd_intf, e2s_qmod_spd_intfc_type[spd_intf],
...
int qtce_phy_interface_config_get(const phymod_phy_access_t* phy, uint32_t flags, phymod_ref_clk_t ref_clock, phymod_phy_inf_config_t* config)
{
    ...
    const phymod_access_t *pm_acc;
    pm_acc = &phy->access;
    PHYMOD_VDBG(DBG_LNK, pm_acc, ("%-22s: p=%p adr=%0x lmask=%0x flags=%x\n",
...
#ifdef VENDOR_BROADCOM 
    PHYMOD_VDBG(QMOD_DBG_LNK, pm_acc, ("%-22s: p=%p adr=%0x lmask=%0x sp_id=0x%0x rate=%0d phy_intf=%0d(%s) intf_md=%0x\n",
...
int qtce_phy_autoneg_ability_set(const phymod_phy_access_t* phy, const phymod_autoneg_ability_t* an_ability)
{
    ...
    const phymod_access_t *pm_acc;
    pm_acc = &phy->access;
    PHYMOD_VDBG(DBG_AN, pm_acc, ("%-22s: p=%p adr=%0x lmask=%0x an_cap=%0x cap=%0x\n",
...
#ifdef VENDOR_BROADCOM
    pm_acc = &phy->access;
    PHYMOD_VDBG(QMOD_DBG_AN, pm_acc, ("%-22s: p=%p adr=%0x lmask=%0x an_cap=%0x bam=%0x\n",
        __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask, an_ability_get_type->an_cap, an_ability_get_type->cl37bam_cap));
#endif
    return PHYMOD_E_NONE;
}
...
int qtce_phy_autoneg_set(const phymod_phy_access_t* phy, const phymod_autoneg_control_t* an)
{
    ...
    const phymod_access_t *pm_acc;
    pm_acc = &phy->access;
    PHYMOD_VDBG(DBG_AN, pm_acc, ("%-22s: p=%p adr=%0x lmask=%0x an_en=%d flag=%0x mode=%x ln=%0d\n",
          __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask, an->enable, an->flags, an->an_mode, an->num_lane_adv));
#endif
    /* next program the tx fir taps and driver current based on the input */
    PHYMOD_IF_ERR_RETURN
        (phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));
...
#ifdef VENDOR_BROADCOM
    PHYMOD_VDBG(DBG_AN, pm_acc, ("%-22s: p=%p adr=%0x lmask=%0x an_en=%d ctrl_port=%0d\n",
         __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask, an->enable, ctrl_port));
#endif  
    return PHYMOD_E_NONE;
...
#ifdef VENDOR_BROADCOM
    PHYMOD_VDBG(QMOD_DBG_AN, pm_acc, ("%-22s: p=%p adr=%0x lmask=%0x an_en=%d an_done=%0x\n",
         __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask, an->enable, *an_done));
#endif
...
int qtce_core_init(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    const phymod_access_t *pm_acc;
    pm_acc = &core->access;
    PHYMOD_VDBG(DBG_CFG, pm_acc, ("%-22s: p=%p adr=%0x lmask=%0x\n",
                __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask));
#endif
...
int qtce_phy_loopback_set(const phymod_phy_access_t* phy, phymod_loopback_mode_t loopback, uint32_t enable)
{
    ...
    PHYMOD_VDBG(DBG_LPK, pm_acc, ("%-22s: p=%p adr=%0x lmask=%0x lpbk=%0d(%s) en=%0d\n",
