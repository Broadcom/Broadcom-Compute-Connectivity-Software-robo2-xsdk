...
#ifdef PHYMOD_VIPER4X_TS28HPM_6MX_1MR_QSGMII_COMBO_SUPPORT
int viper_disable_lmtcal(const PHYMOD_ST *pa)
 {
     RX3_CTL2r_t reg;
     phymod_access_t pa_copy;
     ...
     RX3_CTL2r_CLR(reg);
     READ_RX3_CTL2r(pa, &reg); 

     /* Disable lmtcal " set {lmtcal_en_ovrd, lmtcal_en_ovrd_val} = 2'10} " */
     RX3_CTL2r_LMTCAL_EN_OVRDf_SET      (reg, 1);
     RX3_CTL2r_LMTCAL_EN_OVRD_VALf_SET  (reg, 0);

     WRITE_RX3_CTL2r(&pa_copy, reg);
     return PHYMOD_E_NONE;
 }
#endif
...
#ifdef PHYMOD_VIPER4X_TS28HPM_6MX_1MR_QSGMII_COMBO_SUPPORT
    XGXSCTLr_MODEf_SET      (reg, VIPER_XGXS_MODE_INDLANE_OS4);
#else
...
#ifdef PHYMOD_VIPER4X_TS28HPM_6MX_1MR_QSGMII_COMBO_SUPPORT
 int viper_pll_disable_qsgmii_single_lane (PHYMOD_ST *pa)
 {
    XGXSCTLr_t reg;
...
/*
 * viper_qsgmii_enable
     5'h1F RX_afe Block Address 
        16'h80B0
     5'h19 Enable CDR step-by-1 mode 
        16'h0083
     5'h1F XGXSBlk2 Block Address 
        16'h8100
     5'h1B Enable QSGMII (sets Lane0 to OS1 mode): Assert the following (i.e. 0x810B.[1:0] = {qsgmii_pin_disable, qsgmii_en}
        16'h0003
 */
int viper_qsgmii_enable (const PHYMOD_ST *pa)
{
    RX_AFE_ANARXCTL1Gr_t rx_afe_anarxctl1g;
    RX_AFE_ANARXCTL1Gr_SET(rx_afe_anarxctl1g, 0x0083);
    WRITE_RX_AFE_ANARXCTL1Gr(pa, rx_afe_anarxctl1g);
    
    QSGMII_CTRLREGr_t qsgmii_ctrlreg;
    /*READ_QSGMII_CTRLREGr(pa, qsgmii_ctrlreg);*/
    QSGMII_CTRLREGr_CLR(qsgmii_ctrlreg);
    QSGMII_CTRLREGr_QSGMII_ENf_SET(qsgmii_ctrlreg, 0x1);
    QSGMII_CTRLREGr_QSGMII_PIN_DISABLEf_SET(qsgmii_ctrlreg, 0x1);
    WRITE_QSGMII_CTRLREGr(pa, qsgmii_ctrlreg);

    CTRL_QSGMIIr_t ctrl_qsgmii;
    CTRL_QSGMIIr_CLR(ctrl_qsgmii);
    QSGMIIr_QSGMII_MODE_EN_TXf_SET(ctrl_qsgmii, 0x1);
    QSGMIIr_QSGMII_MODE_EN_RXf_SET(ctrl_qsgmii, 0x1);
    WRITE_CTRL_QSGMIIr(pa, ctrl_qsgmii);
...
/*
 * viper_quad_gmii_enable
     5'h1F XGXSBlk2 Block Address 
        16'h8100
     5'h1B Enable QSGMII (sets Lane0 to OS1 mode): Assert the following (i.e. 0x810B.[1:0] = {qsgmii_pin_disable, qsgmii_en}
        16'h0002
 */
int viper_quad_gmii_enable (const PHYMOD_ST *pa)
{
    QSGMII_CTRLREGr_t qsgmii_ctrlreg;
    /*READ_QSGMII_CTRLREGr(pa, qsgmii_ctrlreg);*/
    QSGMII_CTRLREGr_CLR(qsgmii_ctrlreg);
    QSGMII_CTRLREGr_QSGMII_ENf_SET(qsgmii_ctrlreg, 0x0);
    QSGMII_CTRLREGr_QSGMII_PIN_DISABLEf_SET(qsgmii_ctrlreg, 0x1);
    WRITE_QSGMII_CTRLREGr(pa, qsgmii_ctrlreg);
...
#ifdef PHYMOD_VIPER4X_TS28HPM_6MX_1MR_QSGMII_COMBO_SUPPORT
int viper_pll_enable_qsgmii_single_lane_refclk156(PHYMOD_ST *pa)
{
   return viper_pll_enable_forced_10G(pa);
}

 int viper_pll_afe_config_refclk156_6p25Ghz (const PHYMOD_ST *pa)
 {
    /* PLL AFE Programming */
    PLL_AFE_CTL0r_t pll_afectrl0;
    PLL_AFE_CTL1r_t pll_afectrl1;
    PLL_AFE_CTL2r_t pll_afectrl2;
    PLL_AFE_CTL3r_t pll_afectrl3;
    PLL_AFE_CTL4r_t pll_afectrl4;
    PLL_AFE_CTL5r_t pll_afectrl5;
    PLL_AFE_CTL6r_t pll_afectrl6;
    PLL_AFE_CTL7r_t pll_afectrl7;
    PLL_AFE_CTL8r_t pll_afectrl8;
    PLL2_CTL1r_t pll2_ctl1;

    PLL_AFE_CTL0r_SET(pll_afectrl0, 0x5740);
    PLL_AFE_CTL1r_SET(pll_afectrl1, 0x01d0);
    PLL_AFE_CTL2r_SET(pll_afectrl2, 0x19f8);
    PLL_AFE_CTL3r_SET(pll_afectrl3, 0x2b00);
    PLL_AFE_CTL4r_SET(pll_afectrl4, 0x0021);
    PLL_AFE_CTL5r_SET(pll_afectrl5, 0x0044);
    PLL_AFE_CTL6r_SET(pll_afectrl6, 0x0);
    PLL_AFE_CTL7r_SET(pll_afectrl7, 0x0);
    PLL_AFE_CTL8r_SET(pll_afectrl8, 0x0);

    WRITE_PLL_AFE_CTL0r(pa, pll_afectrl0);
    WRITE_PLL_AFE_CTL1r(pa, pll_afectrl1);
    WRITE_PLL_AFE_CTL2r(pa, pll_afectrl2);
    WRITE_PLL_AFE_CTL3r(pa, pll_afectrl3);
    WRITE_PLL_AFE_CTL4r(pa, pll_afectrl4);
    WRITE_PLL_AFE_CTL5r(pa, pll_afectrl5);
    WRITE_PLL_AFE_CTL6r(pa, pll_afectrl6);
    WRITE_PLL_AFE_CTL7r(pa, pll_afectrl7);
    WRITE_PLL_AFE_CTL8r(pa, pll_afectrl8);

    PLL2_CTL1r_SET(pll2_ctl1, 0x600);
    WRITE_PLL2_CTL1r(pa, pll2_ctl1);

    /* Toggle mmd_resetb */
    TESTMODELANEr_t testmodelane;
    TESTMODELANEr_SET(testmodelane, 0x20);
    WRITE_TESTMODELANEr(pa, testmodelane);        

    /*PLL_AFE_CTL4r_SET(pll_afectrl4, 0x8021);*/
    PLL_AFE_CTL4r_SET(pll_afectrl4, 0x0021);
    WRITE_PLL_AFE_CTL4r(pa, pll_afectrl4);

    /*PLL_AFE_CTL4r_SET(pll_afectrl4, 0x8821);*/
    PLL_AFE_CTL4r_SET(pll_afectrl4, 0x0821);
    WRITE_PLL_AFE_CTL4r(pa, pll_afectrl4);
    return PHYMOD_E_NONE;
 }

int viper_pll_afe_config_refclk156 (const PHYMOD_ST *pa)
{
    /* PLL AFE Programming */
    PLL_AFE_CTL0r_t pll_afectrl0;
    PLL_AFE_CTL1r_t pll_afectrl1;
    ...
    PLL_AFE_CTL2r_SET(pll_afectrl2, 0x19f8);
    PLL_AFE_CTL3r_SET(pll_afectrl3, 0x2b00);
    PLL_AFE_CTL4r_SET(pll_afectrl4, 0x0020);
    PLL_AFE_CTL5r_SET(pll_afectrl5, 0x0044);
    PLL_AFE_CTL6r_SET(pll_afectrl6, 0x0);
    PLL_AFE_CTL7r_SET(pll_afectrl7, 0x0);
    PLL_AFE_CTL8r_SET(pll_afectrl8, 0x0);
    ...
    /*PLL_AFE_CTL4r_SET(pll_afectrl4, 0x8821);*/
    PLL_AFE_CTL4r_SET(pll_afectrl4, 0x0820);
    WRITE_PLL_AFE_CTL4r(pa, pll_afectrl4);
    return PHYMOD_E_NONE;
 }

 /* 
 * viper_sgmii_force_2p5G 
 *
 * Set SGMII Mode   0x8300: 0x0100 (four)
 * Set 10M & Dis AN 0x0000: 0x0100 (one)
 * Set OS 2 Mode    0x834A: 0x0001 (four)
 *
 */
static int viper_sgmii_force_2p5G (PHYMOD_ST *pa)
{
    ...
    MIICTLr_MANUAL_SPEED0f_SET (miictrl, 1);
    MIICTLr_MANUAL_SPEED1f_SET (miictrl, 1);
...
#ifdef PHYMOD_VIPER4X_TS28HPM_6MX_1MR_QSGMII_COMBO_SUPPORT
#ifndef ROBO2_VIPER_CONFIG_1000X_1000
    /* Set os5 mode */
...
#ifndef PHYMOD_VIPER4X_TS28HPM_6MX_1MR_QSGMII_COMBO_SUPPORT
    if (VIPER_CORE_IS_XGXS_10G_028(pa))
#endif
    {
...
int viper_set_spd_intf(PHYMOD_ST *pa, viper_spd_intfc_type_t type)
{
    ...
    switch(type){
        ...
        case VIPER_SPD_1000_SGMII:
            ...
            break; 
#ifdef PHYMOD_VIPER4X_TS28HPM_6MX_1MR_QSGMII_COMBO_SUPPORT
        case VIPER_SPD_2500_SGMII:
            PHYMOD_IF_ERR_RETURN(viper_sgmii_force_2p5G(pa));
            break; 
#endif
        ...
        case VIPER_SPD_10000_CX4:
            ...
            break; 
#ifdef PHYMOD_VIPER4X_TS28HPM_6MX_1MR_QSGMII_COMBO_SUPPORT
        case VIPER_SPD_AN_1G:
            PHYMOD_IF_ERR_RETURN(viper_fiber_AN_1G(pa));
            break; 
        case VIPER_SPD_AN_SGMII_M_100M:
            PHYMOD_IF_ERR_RETURN(viper_sgmii_master_aneg_100M(pa, 0));
            break; 
        case VIPER_SPD_AN_SGMII_M_10M:
...
int viper_actual_speed_get(const PHYMOD_ST* pa, viper_actual_speed_id_t *speed_id)
{
    ...
    /* workaround for not able to read speed properly for SGMII 2.5G */
    DIG_MISC8r_CLR(misc8);
...
#ifdef PHYMOD_VIPER4X_TS28HPM_6MX_1MR_QSGMII_COMBO_SUPPORT
    /* Set bit 12 to 1 as a workaround for lane0 transition to/from 2.5G mode 
       affecting other lanes */
    DIG_MISC1r_SET(misc1, 0xd000);
#else
...
/*
 * viper_disable_carrier_extend
 *
 * enable fast timers 0x8301 : 0x0080
 *
 */
int viper_disable_carrier_extend(PHYMOD_ST *pa)
{
    DIG_CTL1000X2r_t   ctrl;

    READ_DIG_CTL1000X2r (pa, &ctrl);
    DIG_CTL1000X2r_DISABLE_CARRIER_EXTENDf_SET(ctrl, 1);
    MODIFY_DIG_CTL1000X2r (pa, ctrl);
