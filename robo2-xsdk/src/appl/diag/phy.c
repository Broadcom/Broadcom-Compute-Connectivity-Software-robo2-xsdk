...
_phy_diag_loopback(int unit, bcm_pbmp_t pbmp, args_t *args)
{
    ...
    DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
        ...
        if (mode > 0) {
            rv = bcm_port_loopback_set(unit, port, 2); /* PHY */

            if (rv != BCM_E_NONE) break;

            rv = port_diag_ctrl(unit, port, PHY_DIAG_INSTANCE(phy_unit, PHY_DIAG_INTF_LINE, PHY_DIAG_LN_DFLT) ,
                                   PHY_DIAG_CTRL_SET,
        ...
        case 0:
            rv = bcm_port_loopback_set(unit, port, 0);
            ...
            rv = port_diag_ctrl(unit, port, PHY_DIAG_INSTANCE(phy_unit, PHY_DIAG_INTF_SYS, PHY_DIAG_LN_DFLT),
                                   PHY_DIAG_CTRL_SET,
                                   SOC_PHY_CONTROL_LOOPBACK_PMD,
                                   (void *)FALSE);
            if (rv != BCM_E_NONE) break;
