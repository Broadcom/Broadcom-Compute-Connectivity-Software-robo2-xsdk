...
/* PTP SIMulate ... */
#define PTP_SIMULATE_USAGE \
    "PTP SIMulate DMAC=<target> DIP=<target> IP=<start_src_ip> Num=<numslaves> Rate=<rate_per_slave> SIGnalduration=<grant time>\n"

STATIC cmd_result_t
cmd_ptp_simulate(int unit, args_t *a)
{
    int rv;
    ...
    int arg_rate = 0;
    int arg_signal = 150;
    bcm_mac_t arg_dmac = {0};
    bcm_ip_t arg_dip = 0;

    uint8* config;
    uint32 ns_between_dr;
    int32 sync_logintv; /* currently sync and DR rates are equal */

    mos_msg_data_t config_msg;
    int uC = 1;
    int timeout_usec = 200000000;
    uint32 min_version = 0x01000000;
    static int started = 0;
    const int config_msg_size = 32;

    parse_table_init(unit, &pt);
    parse_table_add(&pt, "Num", PQ_INT, (void*)0, &arg_num, NULL);
    parse_table_add(&pt, "IP",   PQ_IP  | PQ_DFL, (void*)0, &arg_ip, NULL);
    parse_table_add(&pt, "Rate", PQ_INT, (void*)0, &arg_rate, NULL);
    parse_table_add(&pt, "SIGnalduration", PQ_INT, (void*)150, &arg_signal, NULL);
    parse_table_add(&pt, "DMAC", PQ_MAC, (void*)arg_dmac, &arg_dmac, NULL);
    parse_table_add(&pt, "DIP", PQ_IP | PQ_DFL, (void*)0, &arg_dip, NULL);

    if (parse_arg_eq(a, &pt) < 0) {
        cli_out("%s: Invalid option %s\n", ARG_CMD(a), ARG_CUR(a));
        cli_out(PTP_SIMULATE_USAGE);
        PTP_CLI_RESULT_RETURN(CMD_FAIL);
    }

    if (!started) {
        cli_out("Starting app\n");
        rv = soc_cmic_uc_appl_init(unit, uC,
                                   MOS_MSG_CLASS_PTPSIM,
                                   timeout_usec,
                                   min_version, min_version, NULL, NULL);
        if (SOC_E_NONE == rv) {
            cli_out("Started\n");
        } else {
            cli_out("App Init failed\n");
            PTP_CLI_RESULT_RETURN(CMD_FAIL);
        }
        started = 1;
    }

    config = soc_cm_salloc(unit, config_msg_size, "ptpsim config");

    /* for convenience: allow negative values to mean log interval, */
    /*   so -5 => 32, etc */
    if (arg_rate < 0) {
        int logIntv = arg_rate;
        sync_logintv = arg_rate;
        arg_rate = 1;
        while (logIntv++ < 0) {
            arg_rate *= 2;
        }
    } else {
        int rate_countdown = arg_rate + arg_rate/2;
        sync_logintv = 1;
        while (rate_countdown > 0) {
            --sync_logintv;
            rate_countdown /= 2;
        }
    }
    ...
    if (arg_num) {
        ns_between_dr = 1000000000 / arg_rate / arg_num;
    } else {
        ns_between_dr = 0;
    }

    _bcm_ptp_uint32_write(config, arg_ip);                 /* base IP */
    _bcm_ptp_uint32_write(config + 4, arg_ip + arg_num);   /* end IP */
    _bcm_ptp_uint32_write(config + 8, ns_between_dr);      /* DelReq gap */
    _bcm_ptp_uint32_write(config + 12, arg_signal);        /* Signal duration */
    _bcm_ptp_uint32_write(config + 16, (uint32)sync_logintv); /* for signal */
    _bcm_ptp_uint32_write(config + 20, arg_dip);           /* target IP */
    sal_memcpy(config + 24, arg_dmac, 6);                  /* target MAC */

    soc_cm_sflush(unit, config, config_msg_size);

    config_msg.s.mclass = MOS_MSG_CLASS_PTPSIM;
    config_msg.s.subclass = MOS_MSG_SUBCLASS_PTPSIM_CONFIG;
    config_msg.s.len = 0;
    config_msg.s.data = bcm_htonl(soc_cm_l2p(unit, (void*)config));

    if (BCM_FAILURE(rv = soc_cmic_uc_msg_send(unit, uC, &config_msg, timeout_usec))) {
        cli_out("Config failed\n");
    }
    soc_cm_sfree(unit, config);
    PTP_CLI_RESULT_RETURN((rv ? CMD_FAIL : CMD_OK));
}
...
#ifdef VENDOR_BROADCOM
#ifdef BCM_CMICM_SUPPORT
    {"SIMulate", cmd_ptp_simulate, PTP_SIMULATE_USAGE, 0},
#endif /* BCM_CMICM_SUPPORT */
#endif /* VENDOR_BROADCOM */
#endif /* __KERNEL__ */
