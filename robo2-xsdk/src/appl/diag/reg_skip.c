...
#ifdef VENDOR_CALHOUN
    case XMAC_LH_HDR_0_SCH_IDLEr:
    case XMAC_LH_HDR_1r:
    case XMAC_LH_HDR_2r:
    case XMAC_LH_HDR_3r:
    case XMAC_SCH_CTRLr:
        /* These are not implemented in Trident */
        goto skip;
#endif /* VENDOR_CALHOUN */
...
/*
  * On emulator setup, there is only half number of xpe/pipe/slice. So some per pipe/xpe/slice
  * registers may not exist. Then, those registers should be skipped.
  * Please refer document MMUuA_sbus_addressing.pdf for detailed information.
  */
int
tomahawk_reg_mask_on_2pipe_setup(int unit, soc_regaddrinfo_t *ainfo, char *msg) {
    soc_reg_t reg;
    int acc_type, base_type;
    int instance;
    int skip = 0;
    /* soc_info_t *si = &SOC_INFO(unit); */
    uint32 flag_invalid = 0, base = 0;
    int blk = 0;
    int gran = 0;
    ...
    gran = soc_regtype_gran_new_bus[reg_type_no];

    if (SOC_REG_IS_ARRAY(unit, reg)) {
        base = ainfo->addr - ainfo->idx * SOC_REG_ELEM_SKIP(unit, reg);
    } else if (SOC_REG_ARRAY(unit, reg)) {
        base = ainfo->addr - ainfo->idx * gran;
        if (SOC_REG_ARRAY2(unit, reg)) {
            base = ainfo->addr - ainfo->idx * gran * 2;
        } else if (SOC_REG_ARRAY4(unit, reg)) {
            base = ainfo->addr - ainfo->idx * gran * 4;
        }
    } else {
        base = ainfo->addr;
    }
    ...
    if (SOC_REG_BLOCK_IN_LIST(unit, reg, SOC_BLK_MMU_XPE)) {
        blk = SOC_BLK_MMU_XPE;
        if (acc_type == _SOC_TH_ACC_TYPE_SINGLE) {
            switch (base_type) {
                case 5: /* XPE */
                    if (instance % 2) {
                        skip = 1;
        ...
        } else if (acc_type == _SOC_TH_ACC_TYPE_DUPLICATE) {
            switch (base_type) {
                case 1: /* EPORT */
                    skip = 0;
                    break;
        ...
        }else if (_SOC_TH_ACC_TYPE_UNIQUE_PIPE1 == acc_type ||
           _SOC_TH_ACC_TYPE_UNIQUE_PIPE3 == acc_type) {
           skip = 1;
        } else if (_SOC_TH_ACC_TYPE_UNIQUE_PIPE0 == acc_type ||
                   _SOC_TH_ACC_TYPE_UNIQUE_PIPE2 == acc_type) {
            skip = 0;
        } else {
            flag_invalid = 1;
        }
    }
    if (SOC_REG_BLOCK_IN_LIST(unit, reg, SOC_BLK_MMU_SC)
#ifdef BCM_TOMAHAWK2_SUPPORT
        ...
        blk = SOC_BLK_MMU_SC;
#ifdef BCM_TOMAHAWK2_SUPPORT
        blk = SOC_REG_BLOCK_IN_LIST(unit, reg, SOC_BLK_MMU_SED)? SOC_BLK_MMU_SED : SOC_BLK_MMU_SC;
#endif
        if (acc_type == _SOC_TH_ACC_TYPE_SINGLE) {
            switch (base_type) {
                case 1: /* EPORT */
    ...
    if (SOC_REG_BLOCK_IN_LIST(unit, reg, SOC_BLK_IPIPE) ||
        SOC_REG_BLOCK_IN_LIST(unit, reg, SOC_BLK_EPIPE)) {
        blk = (SOC_REG_BLOCK_IN_LIST(unit, reg, SOC_BLK_IPIPE))?SOC_BLK_IPIPE : SOC_BLK_EPIPE;
        if (_SOC_TH_ACC_TYPE_UNIQUE_PIPE2 == acc_type ||
            _SOC_TH_ACC_TYPE_UNIQUE_PIPE3 == acc_type) {
            skip = 1;
        } else if (acc_type == _SOC_TH_ACC_TYPE_SINGLE) {
            if (instance >= 2) {
                skip = 1;
            }
        } else if (_SOC_TH_ACC_TYPE_UNIQUE_PIPE0 == acc_type ||
                   _SOC_TH_ACC_TYPE_UNIQUE_PIPE1 == acc_type ||
                   _SOC_TH_ACC_TYPE_DUPLICATE == acc_type ||
                   _SOC_TH_ACC_TYPE_ADDR_SPLIT_SPLIT == acc_type) {
            skip = 0;
        } else {
            flag_invalid = 1;
        }
    ...
    if (msg) {
        if (skip) {
            LOG_CLI((BSL_META_U(unit,
                                "%s: skip reg %s(%d), acc_type is %d, block_type %d, base_type is %d, base_addr is 0x%x\n"),
                                 msg, SOC_REG_NAME(unit, reg), reg, acc_type, blk, base_type, ainfo->addr));
        }
        if (flag_invalid) {
            LOG_CLI((BSL_META_U(unit,
                                "%s: invalid reg %s(%d), acc_type is %d, block_type %d, base_type is %d, base_addr is 0x%x\n"),
                                 msg, SOC_REG_NAME(unit, reg), reg, acc_type, blk, base_type, ainfo->addr));
        }
    }
    return skip;
...
#ifdef VENDOR_BROADCOM
    /* Need not to skip registers */
    if(SOC_IS_TOMAHAWKX(unit) && 
       (soc_property_get(unit, "num_pipe", 4) == 2) &&
       (soc_property_get(unit, "nonexistent_register_mask_enable", 0))) {
        if (tomahawk_reg_mask_on_2pipe_setup(unit, ainfo, NULL)) {
            goto skip;
        }
    }
#endif /*VENDOR_BROADCOM*/
    ...
    if (SOC_IS_APACHE(unit)) {
        switch (ainfo->reg) {
            ...
            case TOP_TAP_CONTROLr:
                ...
                return 1;
#ifdef VENDOR_BROADCOM
            case TOP_SW_BOND_OVERRIDE_REG_0r:
            case TOP_SW_BOND_OVERRIDE_REG_1r:
            case TOP_SW_BOND_OVERRIDE_REG_2r:
            case TOP_SW_BOND_OVERRIDE_REG_3r:
            case TOP_SW_BOND_OVERRIDE_REG_4r:
            case TOP_SW_BOND_OVERRIDE_REG_5r:
            case TOP_SW_BOND_OVERRIDE_REG_6r:
            case TOP_SW_BOND_OVERRIDE_REG_7r:
            case TOP_SW_BOND_OVERRIDE_REG_8r:
            case TOP_SW_BOND_OVERRIDE_REG_9r:
            case TOP_SW_BOND_OVERRIDE_REG_10r:
            case TOP_SW_BOND_OVERRIDE_REG_11r:
            case TOP_SW_BOND_OVERRIDE_REG_12r:
            case TOP_SW_BOND_OVERRIDE_REG_13r:
            case TOP_SW_BOND_OVERRIDE_REG_14r:
            case TOP_SW_BOND_OVERRIDE_REG_15r:
            case TOP_SW_BOND_OVERRIDE_CODE_REGr:
                /* These registers get programmed during otp override */
                return (soc_property_get(unit, "pci_delay_val", 0));
#endif /* VENDOR_BROADCOM */
