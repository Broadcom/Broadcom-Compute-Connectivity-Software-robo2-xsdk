...
#ifdef AVENGER_DEBUG
int
robo2_do_show_counters(int unit, pbmp_t pbmp )
{
    pbmp_t   tpbmp;
    soc_port_t port;
    int rv;
    ...
    if (SOC_PBMP_IS_NULL(pbmp)) {
        BCM_PBMP_ASSIGN(tpbmp, PBMP_ALL(unit));
    } else {
        BCM_PBMP_ASSIGN(tpbmp, pbmp);
    }
    dump_cb_counters(unit);
    PBMP_ITER(tpbmp, port) {
        rv = soc_port_stat_dump(port);
        if (rv < 0) {
            cli_out("%d: Error: Could not get counters for port : %s\n",
                    port, soc_errmsg(rv));
            return CMD_FAIL;
        }
    }
...
#ifdef AVENGER_DEBUG
    return(robo2_do_show_counters(unit, pbmp));
#endif
