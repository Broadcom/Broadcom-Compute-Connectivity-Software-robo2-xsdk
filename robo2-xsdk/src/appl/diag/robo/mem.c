...
#ifdef AVENGER_DEBUG
extern int dump_table(int tid, int idx, int len);
#endif
/*
...
#ifdef AVENGER_DEBUG
cmd_result_t
cmd_robo2_table_dump(int unit, args_t *a)
{
    char *tab;
    int tid, idx, len;
    ...
    tab = ARG_GET(a);
    if (!tab) {
        cli_out("ERROR: Table ID required\n");
        cli_out("Usage: listmem <tableid> <index> <len>\n");
        return CMD_FAIL;
    }
    tid = parse_integer(tab);

    tab = ARG_GET(a);
    if (!tab) {
        cli_out("ERROR: Table index required\n");
        cli_out("Usage: listmem <tableid> <index> <len>\n");
        return CMD_FAIL;
    }
    idx = parse_integer(tab);

    tab = ARG_GET(a);
    if (!tab) {
        cli_out("ERROR: Table entries length required\n");
        cli_out("Usage: listmem <tableid> <index> <len>\n");
        return CMD_FAIL;
    }
    len = parse_integer(tab);

    if(!dump_table(tid, idx, len)) {
        return CMD_FAIL;
    }
    return CMD_OK;
}
...
#ifdef AVENGER_DEBUG
    return (cmd_robo2_table_dump(unit,a));
#endif
