...
#ifdef AVENGER_DEBUG
cmd_result_t
cmd_robo2_reg_get(int unit, args_t *a)
{
    char   *name;
    uint32 regaddr;
    int    rv;
    ...
    regaddr = parse_integer(name);  /* register address */
    rv = avgr_reg_read(unit, regaddr);
    if (rv < 0) {
...
#ifdef AVENGER_DEBUG
    return (cmd_robo2_reg_get(unit, a));
#endif
...
#ifdef AVENGER_DEBUG
cmd_result_t
cmd_robo2_reg_write(int unit, char *regname, args_t *a)
{
    char   *name;
    uint32 regaddr;
    int    val32;
    ...
    regaddr = parse_integer(regname);  /* register address */

    name = ARG_GET(a);
    if (name == NULL) {
        return CMD_USAGE;
    }
    val32   = parse_integer(name);  /* register value */

    rv = avgr_reg_write(unit, regaddr, val32);
    if (rv < 0) {
        cli_out("ERROR: Write reg failed: %s\n", soc_errmsg(rv));
        return CMD_FAIL;
    }
...
#ifdef AVENGER_DEBUG
    return (cmd_robo2_reg_write(unit, name, a));
#endif
