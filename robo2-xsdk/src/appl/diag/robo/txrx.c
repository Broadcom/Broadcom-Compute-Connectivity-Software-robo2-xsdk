 ...
 */
{
    ...
    parse_table_add(&pt, "Opcode",PQ_DFL|PQ_INT, 0,
            &xd->pkt_info.opcode,    NULL);
#endif /* BCM_ROBO2_SUPPORT */
#if defined(BCM_ROBO2_SUPPORT) || defined (BCM_TB_SUPPORT)
    parse_table_add(&pt, "McastGroupID",     PQ_DFL|PQ_INT,     0,
                    &xd->pkt_info.multicast_group, NULL);
#endif /* BCM_ROBO2_SUPPORT || BCM_TB_SUPPORT */
#if defined(BCM_TB_SUPPORT) || defined(BCM_POLAR_SUPPORT)
...
robo_rx_cb_handler(int unit, bcm_pkt_t *info, void *cookie)
{
    ...
    if (SOC_IS_TBX(unit)) {
        ...
        cli_out("Rx Trap data = %d Rx ingress time stamp %d \n",
                info->rx_trap_data, info->rx_timestamp);
    }
