...
bcm_robo_eav_srp_mac_ethertype_set(int unit, bcm_mac_t mac, bcm_port_ethertype_t ethertype)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_EAV_SRP_MAC_ETHERTYPE_SET(unit, mac, ethertype));
        return BCM_E_NONE;
    }
...
bcm_robo_eav_srp_mac_ethertype_get(int unit, bcm_mac_t mac, bcm_port_ethertype_t *ethertype)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_EAV_SRP_MAC_ETHERTYPE_GET(unit, mac, ethertype));
        return BCM_E_NONE;
    }
    BCM_IF_ERROR_RETURN(REG_READ_MULTI_PORT_CTLr(unit, &reg_val));
    ...
    int remapped_pcp)
{
    ...
    rv = DRV_EAV_PCP_MAPPING_SET(unit, type, pcp, remapped_pcp);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_COMMON,
                  (BSL_META_U(unit,
                     "EAV Error: failure in bcm_robo_eav_pcp_mapping_set()\n")));
        return rv;
    }
    return rv;
}
    ...
    int *remapped_pcp)
{
    ...
    rv = DRV_EAV_PCP_MAPPING_GET(unit, type, pcp, remapped_pcp);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_COMMON,
                  (BSL_META_U(unit,
                     "EAV Error: failure in bcm_robo_eav_pcp_mapping_get()\n")));
        return rv;
    }
    return rv;
}
    ...
    uint32 bytes_burst)
{
    ...
    rv = DRV_EAV_BANDWIDTH_SET(unit, port, type, bytes_sec, bytes_burst);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_COMMON,
                  (BSL_META_U(unit,
                     "EAV Error: failure in bcm_robo_eav_bandwidth_set()\n")));
        return rv;
    }
    return rv;
}
    ...
    uint32 *bytes_burst)
{
    ...
    rv = DRV_EAV_BANDWIDTH_GET(unit, port, type, bytes_sec, bytes_burst);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_COMMON,
                  (BSL_META_U(unit,
                     "EAV Error: failure in bcm_robo_eav_bandwidth_get()\n")));
        return rv;
    }
    return rv;
}
