...
extern int cbxi_csd_port_init(void);
...
bcm_custom_field_id_t robo_field_id_g = 0;
...
_bcm_robo_modules_init(int unit)
{
    ...
    if(SOC_IS_AVENGER(unit)) {
       /* Trunk init needs to be called before fsal init until
        * bcm_robo_port_init() is fixed in cbx_fsal_init()
        */
    /* Init modules */
       BCM_IF_ERROR_NOT_UNAVAIL_RETURN(bcm_trunk_init(unit));
       BCM_IF_ERROR_NOT_UNAVAIL_RETURN(bcm_port_init(unit));
       BCM_IF_ERROR_RETURN(DRV_FSAL_INIT(unit));
    } else {
       BCM_IF_ERROR_NOT_UNAVAIL_RETURN(bcm_port_init(unit));
    }
    ...
    field_params.stage = bcmCustomFieldStageIngress;
    BCM_IF_ERROR_NOT_UNAVAIL_RETURN(bcm_custom_field_init(unit, &field_params, &robo_field_id_g));

    BCM_IF_ERROR_NOT_UNAVAIL_RETURN(bcm_mirror_init(unit));
    BCM_IF_ERROR_NOT_UNAVAIL_RETURN(bcm_custom_meter_init(unit));
    BCM_IF_ERROR_NOT_UNAVAIL_RETURN(bcm_custom_cosq_init(unit));
    BCM_IF_ERROR_NOT_UNAVAIL_RETURN(bcm_stat_init(unit));
    BCM_IF_ERROR_NOT_UNAVAIL_RETURN(bcm_custom_auth_params_t_init( unit,
                                                          &auth_params));
    BCM_IF_ERROR_NOT_UNAVAIL_RETURN(bcm_custom_auth_init(unit,
                                                          &auth_params));
    BCM_IF_ERROR_NOT_UNAVAIL_RETURN(bcm_custom_ts_init(unit));

    /* Initialize link scan module after all modules are initializsed */
    BCM_IF_ERROR_NOT_UNAVAIL_RETURN(bcm_linkscan_init(unit));

#ifdef CONFIG_CASCADED_MODE
    sal_printf("cbxi_csd_port_init()\n");
    rv = cbxi_csd_port_init();
    if (rv < 0) {
        LOG_ERROR(BSL_LS_FSAL_COMMON,
            (BSL_META("cbxi_csd_port_init: rv = %d\n"), rv));
    }
#endif /* CONFIG_CASCADE_MODE */
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_PORT_PKT_FLOW_ENABLE(unit));
    }
    return BCM_E_NONE;
}
...
STATIC int
_bcm_robo2_init(int unit)     /* Initialize chip and BCM layer */
{
