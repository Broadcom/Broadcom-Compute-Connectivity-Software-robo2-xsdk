...
bcm_robo_l2_init(int unit)
{
    ...
    if (SOC_IS_AVENGER(unit))
        BCM_IF_ERROR_RETURN(DRV_L2_INIT(unit));
    else {
        /* prevent L2 init process when ARL table still frozen,
         *  - L2_ARLm is LOCK in frozen state
         *  - frozen counter(f->frozen) not clear in frozen state
...
bcm_robo_l2_addr_add(int unit, bcm_l2_addr_t *l2addr)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_L2_ADD(unit, l2addr));
        return BCM_E_NONE;
    } else {
...
bcm_robo_l2_addr_delete(int unit, bcm_mac_t mac, bcm_vlan_t vid)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_L2_REMOVE(unit, &l2addr));
        return BCM_E_NONE;
    }
                ...
                uint32 flags)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_L2_FLUSH_BY_PORT(unit, port));
        return BCM_E_NONE;
    }
...
bcm_robo_l2_addr_delete_by_vlan(int unit, bcm_vlan_t vid, uint32 flags)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        if(!vid) return BCM_E_PARAM; /* Otherwise it removes all VLANS' entries */
        BCM_IF_ERROR_RETURN(DRV_L2_FLUSH_BY_VLAN(unit, vid));
        return BCM_E_NONE;
    }
    else {
        sal_memset(&arl_entry, 0, sizeof (arl_entry));
                    ...
                    bcm_l2_addr_t *l2addr)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        bcm_l2_addr_init(l2addr, mac, vid);
        BCM_IF_ERROR_RETURN(DRV_L2_LOOKUP(unit, l2addr));
        return BCM_E_NONE;
    }
...
bcm_robo_l2_age_timer_set(int unit, int age_seconds)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_L2_AGE_TIMER_SET(unit, age_seconds));
    } else {
...
bcm_robo_l2_age_timer_get(int unit, int *age_seconds)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_L2_AGE_TIMER_GET(unit, (uint32 *)age_seconds));
    }
    else {
...
bcm_robo_l2_clear(int unit)
{
    ...
    L2_INIT(unit);
    if (SOC_IS_AVENGER(unit))
        BCM_IF_ERROR_RETURN(DRV_L2_RESEED(unit));
    else {
...
bcm_robo_l2_traverse(int unit, bcm_l2_traverse_cb trav_fn, void *user_data)
{
    ...
    bcm_l2_addr_t       l2_entry, *l2_ptr = NULL;
    ...
    if (SOC_IS_AVENGER(unit)) {
        int rv;
        while(1) {
            rv = DRV_L2_LOOKUP_NEXT(unit, l2_ptr, &l2_entry);
            if(rv == BCM_E_NOT_FOUND)
                return BCM_E_NONE;
            else if(rv)
                return rv;

            trav_fn(unit, &l2_entry, user_data);
            l2_ptr = &l2_entry;
        }
    }
        ...
        bcm_l2_traverse_cb trav_fn, void *user_data)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_L2_LOOKUP(unit, match_addr));
        trav_fn(unit, match_addr, user_data);
        return BCM_E_NONE;
    }
