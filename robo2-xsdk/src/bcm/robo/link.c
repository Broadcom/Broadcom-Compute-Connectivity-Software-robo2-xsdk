...
_robo_lc_pbm_init(int unit)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        /* Start from all link down */
        BCM_PBMP_CLEAR(lc->lc_pbm_link);
        BCM_PBMP_CLEAR(lc->lc_pbm_link_change);
        /* All ports in SW linkscan */
        BCM_PBMP_ASSIGN(lc->lc_pbm_sw, PBMP_ALL(unit));
    }
...
STATIC void _bcm_robo2_link_scan_callback(uint32 portid, uint32 value)
{
	bcm_port_info_t info;
	int unit, port;
	ls_handler_t *lh;
	...
	if (BCM_GPORT_IS_SET(portid)) {
            unit = 0;
            bcm_robo_port_local_get(unit, portid, &port);
	}

	bcm_port_info_get(unit, port, &info);
	info.linkstatus = value;

        LC_LOCK(unit);

	lc = robo_link_control[unit];
...
int bcm_robo_linkscan_init(int unit)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_LINKSCAN_INIT(unit));
        BCM_IF_ERROR_RETURN(DRV_LINKSCAN_REGISTER(unit,
                                        _bcm_robo2_link_scan_callback));
        BCM_IF_ERROR_RETURN(bcm_robo_linkscan_enable_set(unit, 100000));
    }
...
int bcm_robo_linkscan_detach(int unit)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_LINKSCAN_DETACH(unit));
        BCM_IF_ERROR_RETURN(DRV_LINKSCAN_UNREGISTER(unit,
                                        _bcm_robo2_link_scan_callback));
    }
...
int bcm_robo_linkscan_enable_set(int unit, int us)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_LINKSCAN_ENABLE_SET(unit, us));
        lc->lc_us = us;
        return rv;
    }
...
int bcm_robo_linkscan_enable_get(int unit, int *us)
{
   ...
   if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_LINKSCAN_ENABLE_GET(unit, us));
    } else {
...
{
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_LINKSCAN_DUMP(unit));
        return BCM_E_NONE;
    }
    ls_handler_t *ent;
