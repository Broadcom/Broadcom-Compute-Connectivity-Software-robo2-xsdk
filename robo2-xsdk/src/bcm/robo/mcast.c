                  ...
                  bcm_mcast_addr_t *mcaddr)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        ...
        BCM_IF_ERROR_RETURN(DRV_MCAST_INFO_GET(unit, mcaddr));
        return BCM_E_NONE;
    }
...
int bcm_robo_mcast_addr_add(int unit, bcm_mcast_addr_t *mcaddr)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_MCAST_CREATE(unit, mcaddr));
        return BCM_E_NONE;
    }
...
int bcm_robo_mcast_addr_remove(int unit, bcm_mac_t mac, bcm_vlan_t vid)
{
    ...
    bcm_mcast_addr_init(&mcaddr, mac, vid);
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_MCAST_DESTROY(unit, &mcaddr));
        return BCM_E_NONE;
    }
              ...
              bcm_pbmp_t *allRouterBmp)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_MCAST_ADD(unit, &newmc, srcPort));
        return BCM_E_NONE;
    }
    else {
        /*
               ...
               int srcPort)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_MCAST_REMOVE(unit, &mcaddr, srcPort));
