...
int
_bcm_robo_mirror_port_flags_convert(uint32 flags, uint32 *out_flags)
{
    *out_flags = 0;

    if (flags & BCM_MIRROR_PORT_INGRESS) {
        *out_flags |= DRV_PORT_MIRROR_INGRESS_ENABLE;
    }
    if (flags & BCM_MIRROR_PORT_EGRESS) {
        *out_flags |= DRV_PORT_MIRROR_EGRESS_ENABLE;
    }
...
bcm_robo_mirror_init(int unit)
{
    ...
    if (!SOC_IS_AVENGER(unit)) {
        ...
        BCM_IF_ERROR_RETURN(DRV_MIRROR_INIT(unit));
        return BCM_E_NONE;
    }
}
                              ...
                              uint32 flags, bcm_gport_t mirror_dest) 
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_COMMON,
                (BSL_META_U(unit,
                            "BCM API : bcm_robo_mirror_port_dest_add()..\n")));
    ...
    BCM_IF_ERROR_RETURN
          (_bcm_robo_mirror_port_flags_convert(flags, &out_flags));

    BCM_IF_ERROR_RETURN
          (DRV_MIRROR_PORT_ADD(unit, port, out_flags, mirror_dest));
                                ...
                                uint32 flags, bcm_gport_t mirror_dest) 
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_COMMON,
                (BSL_META_U(unit,
                            "BCM API : bcm_robo_mirror_port_dest_delete()..\n")));
    ...
    BCM_IF_ERROR_RETURN
          (_bcm_robo_mirror_port_flags_convert(flags, &out_flags));
    BCM_IF_ERROR_RETURN
          (DRV_MIRROR_PORT_DELETE(unit, port, out_flags, mirror_dest));
...
{
    drv_mirror_dest_t drv_mirror_dest;

    LOG_VERBOSE(BSL_LS_BCM_COMMON,
                (BSL_META_U(unit,
                           "BCM API : bcm_robo_mirror_destination_create()..\n")));
    ...
    sal_memset(&drv_mirror_dest, 0, sizeof (drv_mirror_dest_t));
    drv_mirror_dest.port = mirror_dest->gport;

    BCM_IF_ERROR_RETURN
          (DRV_MIRROR_DEST_CREATE(unit, &drv_mirror_dest));
    mirror_dest->mirror_dest_id = drv_mirror_dest.mirror_destid;
...
bcm_robo_mirror_destination_destroy(int unit, bcm_gport_t mirror_dest_id) 
{
    LOG_VERBOSE(BSL_LS_BCM_COMMON,
                (BSL_META_U(unit,
                          "BCM API : bcm_robo_mirror_destination_destroy()..\n")));
    ...
    BCM_IF_ERROR_RETURN
          (DRV_MIRROR_DEST_DESTROY(unit, mirror_dest_id));
 ...
 * Purpose:
 *     Add mirroring destination to a vlan.
 * Parameters:
 *      unit            - (IN) BCM device number.
 *      port            - (IN) Port mirrored port.
 *      vlan            - (IN) Mirror destination id.
 *      flags           - (IN) BCM_MIRROR_PORT_XXX flags.
    ...
    bcm_gport_t destid)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_COMMON,
                (BSL_META_U(unit,
                           "BCM API : bcm_robo_mirror_port_vlan_dest_add()..\n")));
    ...
    BCM_IF_ERROR_RETURN
          (_bcm_robo_mirror_port_flags_convert(flags, &out_flags));
    BCM_IF_ERROR_RETURN
          (DRV_MIRROR_VLAN_ADD(unit, vlan, out_flags, destid));
 ...
 * Purpose:
 *     Delete mirroring destination for a vlan.
 * Parameters:
 *      unit            - (IN) BCM device number.
 *      port            - (IN) Port mirrored port.
 *      vlan            - (IN) Mirror destination id.
    ...
    bcm_gport_t destid)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_COMMON,
                (BSL_META_U(unit,
                        "BCM API : bcm_robo_mirror_port_vlan_dest_delete()..\n")));
    ...
    BCM_IF_ERROR_RETURN
          (_bcm_robo_mirror_port_flags_convert(flags, &out_flags));
    BCM_IF_ERROR_RETURN
          (DRV_MIRROR_VLAN_DELETE(unit, vlan, out_flags, destid));
