...
bcm_robo_port_probe(int unit, pbmp_t pbmp, pbmp_t *okay_pbmp)
{
    ...
    {
        if (SOC_IS_AVENGER(unit)) {
            /* Facing traffic issue if phy-id:10 (port12) initialized before
                                    phy-id:9 (port13) of the 10G core */
            if (port == 12) { 
                port12_probe = 1;
                continue;
            }
    ...
    if (SOC_IS_AVENGER(unit)) {
        if (port12_probe) {
            rv = DRV_PORT_STATUS_GET(unit, 12, DRV_PORT_STATUS_PROBE, &okay);
            if (okay) 
            {
                BCM_PBMP_PORT_ADD(*okay_pbmp, 12);
            }
        }
    }
    return rv;
...
bcm_robo_port_init(int unit)
{
    ...
    if(SOC_IS_AVENGER(unit)) {
        if (BCM_PBMP_MEMBER(okay_ports, 12)) {
            if ((rv = bcm_robo_port_settings_init(unit, 12)) < 0) {
                LOG_WARN(BSL_LS_BCM_COMMON,
                     (BSL_META_U(unit,
                                 "Warning: Unit %d Port %s: "
                                 "Failed to configure initial settings: %s\n"),
        ...
        if (BCM_PBMP_MEMBER(okay_ports, 13)) {
            if ((rv = bcm_robo_port_settings_init(unit, 13)) < 0) {
                LOG_WARN(BSL_LS_BCM_COMMON,
                     (BSL_META_U(unit,
                                 "Warning: Unit %d Port %s: "
                                 "Failed to configure initial settings: %s\n"),
            ...
            bcm_port_abil_t *ability_mask)
{
    ...
    if (SOC_IS_AVENGER(unit) && port == CBX_PORT_RGMII) {
        return BCM_E_NONE;
    }
                    ...
                    uint32 phy_reg_addr, uint32 * phy_data)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        return(soc_phyctrl_reg_read(unit, port,
                    ...
                    uint32 phy_reg_addr, uint32 phy_data)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        return(soc_phyctrl_reg_write(unit, port,
                                    flags, phy_reg_addr, phy_data));
    }
...
bcm_robo_port_pfm_set(int unit, bcm_port_t port, int mode)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        ...
        switch (mode) {
            case BCM_PORT_MCAST_FLOOD_UNKNOWN:
                 mode = DRV_PORT_PROP_MCAST_FLOOD_UNKNOWN;
                 break;
            case BCM_PORT_MCAST_FLOOD_NONE:
                 mode = DRV_PORT_PROP_MCAST_FLOOD_NONE;
                 break;
            default:
                 return BCM_E_PARAM;
        }
        return (DRV_PORT_SET(unit, pbmp,
                             DRV_PORT_PROP_MCAST_PFM, mode));
    }
...
bcm_robo_port_pfm_get(int unit, bcm_port_t port, int *mode)
{
    ...
    if(SOC_IS_AVENGER(unit)) {
        ...
        BCM_IF_ERROR_RETURN(DRV_PORT_GET
            (unit, port, DRV_PORT_PROP_MCAST_PFM, &temp));
        switch (temp) {
            case DRV_PORT_PROP_MCAST_FLOOD_UNKNOWN:
                 *mode = BCM_PORT_MCAST_FLOOD_UNKNOWN;
                 break;
            case DRV_PORT_PROP_MCAST_FLOOD_NONE:
...
{
    if (SOC_IS_AVENGER(unit)) {
        return DRV_PORT_PRI_MAPOP_SET(
            unit, port, DRV_PORT_OP_PCP2TC, pkt_pri, cfi, (uint32)internal_pri, (uint32)color);
    } else {
