...
int _bcm_robo_rx_remote_pkt_alloc(int len, bcm_pkt_t **pkt)
{
    uint8 *data;
    int rv;
...
int _bcm_robo_rx_remote_pkt_free(bcm_pkt_t *pkt)
{
    if (pkt == NULL) {
...
int _bcm_robo_pkt_rx_handler(void *buffer, int buffer_size,
                             drv_packet_attr_t *attr,
                             drv_packet_cb_info_t *cb_info)
{
    bcm_pkt_t *rx_pkt;
    int rv;

    if (robo_rx_control.thread_running == TRUE) {
        rv = _bcm_robo_rx_remote_pkt_alloc(buffer_size, &rx_pkt);
        if (rv != BCM_E_NONE) {
            LOG_VERBOSE(BSL_LS_BCM_RX,
                    (BSL_META("RX pkt: remote pkt alloc failed "
                              "for %d bytes\n"),buffer_size));
            return BCM_E_NONE;
        }

        rx_pkt->blk_count = 1;
        sal_memcpy(rx_pkt->_pkt_data.data, buffer, buffer_size);
        rx_pkt->_pkt_data.len = buffer_size;
        rx_pkt->pkt_data = &rx_pkt->_pkt_data;
        rx_pkt->rx_unit = attr->unit;
        rx_pkt->vlan = attr->vlanid;
        rx_pkt->src_port = attr->source_index;
        rx_pkt->dest_port = attr->dest_index;
        rx_pkt->tot_len = buffer_size;
        rx_pkt->pkt_len = buffer_size;
        if (SOC_IS_AVENGER(0)) {
            rx_pkt->rx_port = rx_pkt->src_port;
            rx_pkt->rx_timestamp = attr->ing_ptp_ts;
            rx_pkt->rx_trap_data = attr->trap_data;
        }
        rv = bcm_rx_remote_pkt_enqueue(0, rx_pkt);
        if (rv < 0) {
            LOG_VERBOSE(BSL_LS_BCM_RX,
                    (BSL_META("RX pkt: remote pkt enque failed ")));
            return BCM_E_NONE;
        }
    }
    return BCM_E_NONE;
...
bcm_robo_rx_init(int unit)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        /* pkt rx buffers init */
	bcm_rx_pktlist_count = BCM_RX_PKTLIST_COUNT_DEFAULT;
        ...
        BCM_IF_ERROR_RETURN(DRV_PKT_RX_REGISTER(unit, _bcm_robo_pkt_rx_handler));
    }
    return BCM_E_NONE;
}               
...
robo_rx_thread_pkts_process(int unit, int cos, int count)
{
    ...
    while (pkt_list) {
        ...
        if (SOC_IS_AVENGER(unit)) {
            _bcm_robo_rx_remote_pkt_free(pkt_list);
        }
        pkt_list = next_pkt;
