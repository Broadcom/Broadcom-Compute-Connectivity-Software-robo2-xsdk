...
int bcm_robo_stg_init(int unit)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_STG_INIT(unit));
        return BCM_E_NONE;
    }
...
bcm_robo_stg_vlan_add(int unit, bcm_stg_t stg, bcm_vlan_t vid)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_STG_VLAN_ADD(unit, (int)stg, (uint32)vid));
        return BCM_E_NONE;
    }
...
bcm_robo_stg_vlan_remove(int unit, bcm_stg_t stg, bcm_vlan_t vid)
{
   bcm_stg_info_t  *si = &robo_stg_info[unit];
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_STG_VLAN_REMOVE(unit, (int)stg, (uint32)vid));
        return BCM_E_NONE;
    }
...
bcm_robo_stg_vlan_remove_all(int unit, bcm_stg_t stg)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_STG_VLAN_REMOVE_ALL(unit, (int)stg));
        return BCM_E_NONE;
    }
...
bcm_robo_stg_create(int unit, bcm_stg_t *stg_ptr) /* Generate new ID */
{
   bcm_stg_info_t  *si = &robo_stg_info[unit];
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_STG_CREATE(unit, (int *)stg_ptr));
        return BCM_E_NONE;
    }
    CHECK_INIT(unit, si);
...
bcm_robo_stg_destroy(int unit, bcm_stg_t stg)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_STG_DESTROY(unit, (int)stg));
        return BCM_E_NONE;
    }
			   ...
			   bcm_port_t port, int stp_state)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_STG_STP_SET(unit, (int)stg, (int)port, stp_state));
        return BCM_E_NONE;
    }
			   ...
			   bcm_port_t port, int *stp_state)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_STG_STP_GET(unit, (int)stg, (int)port, stp_state));
        return BCM_E_NONE;
    }
