...
#define BCM_TRUNK_MEMBER_ADD         0x01
#define BCM_TRUNK_MEMBER_DELETE      0x02
 ...
 */
STATIC int
_bcm_robo2_trunk_get(int unit, bcm_trunk_t tid, bcm_trunk_info_t *trunk_info,
    ...
    int *member_count)
{
    ...
    *member_count = size;
    ...
    if (member_max > 0) {
        /* Can't be greater than maximum number of members in provided
         * member_array
         */
...
/*
 * Function:
 *      _bcm_robo2_trunk_psc_bcm2drv_convert
 * Purpose:
 *      Internal trunk BCM psc to DRV psc convert function for Robo2.
 * Note:
 */
STATIC int
_bcm_robo2_trunk_psc_bcm2drv_convert(int unit, bcm_trunk_t tid,
    int  psc_in, int  *psc_out )
{
    if (psc_in <= 0) {
        psc_in = BCM_TRUNK_PSC_DEFAULT;
    }

    *psc_out = 0;

    if (psc_in & ~_BCM_TRUNK_PSC_VALID_VAL) {
        if (psc_in &
            ~(_BCM_TRUNK_PSC_VALID_VAL | _BCM_TRUNK_PSC_ORABLE_VALID_VAL)) {
            return BCM_E_UNAVAIL;
        }
        if (psc_in & BCM_TRUNK_PSC_MACSA) {
            *psc_out |= ( DRV_TRUNK_HASH_FIELD_MACSA );
        }
        if (psc_in & BCM_TRUNK_PSC_MACDA) {
            *psc_out |= ( DRV_TRUNK_HASH_FIELD_MACDA );
        }
        if (psc_in & BCM_TRUNK_PSC_VID) {
            *psc_out |= ( DRV_TRUNK_HASH_FIELD_VLANID );
        }
        if ((psc_in & BCM_TRUNK_PSC_IPMACSA) ||
            (psc_in & BCM_TRUNK_PSC_IPSA)) {
            *psc_out |= ( DRV_TRUNK_HASH_FIELD_IP_MACSA );
        }
        if ((psc_in & BCM_TRUNK_PSC_IPMACDA) ||
            (psc_in & BCM_TRUNK_PSC_IPDA)) {
...
/*
 * Function:
 *      _bcm_robo2_trunk_psc_drv2psc_convert
 * Purpose:
 *      Internal trunk BCM psc to DRV psc  convert function for Robo2.
 * Note:
 */
STATIC int
_bcm_robo2_trunk_psc_drv2bcm_convert(int unit, bcm_trunk_t tid,
    int  psc_in, int  *psc_out )
{
    if ( psc_in <= 0 ) {
        psc_in = BCM_TRUNK_PSC_DEFAULT;
...
bcm_robo_trunk_init(int unit)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        /* Initialize trunk_ctrl_t structure to use TRUNK_INIT() check
         * in all the trunk APIs
         */
        TRUNK_CNTL(unit).ngroups = 1;
        ...
        BCM_IF_ERROR_RETURN(DRV_TRUNK_INIT(unit));
        return BCM_E_NONE;
    }
...
bcm_robo_trunk_create(int unit, uint32 flags, bcm_trunk_t *tid)
{
    ...
    /* Avenger chip */
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_TRUNK_CREATE(unit, flags, tid));
        return BCM_E_NONE;
    }
...
bcm_robo_trunk_psc_set(int unit, bcm_trunk_t tid, int psc)
{
    ...
    /* Avenger chip */
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(_bcm_robo2_trunk_psc_bcm2drv_convert(unit,
                          tid, psc, &psc_out));
        BCM_IF_ERROR_RETURN(DRV_TRUNK_PSC_SET(unit, tid, psc_out));
        return BCM_E_NONE;
    }
...
bcm_robo_trunk_psc_get(int unit, bcm_trunk_t tid, int *psc)
{
    ...
    /* Avenger chip */
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_TRUNK_PSC_GET(unit, tid, &psc_in));
        BCM_IF_ERROR_RETURN(_bcm_robo2_trunk_psc_drv2bcm_convert(unit,
                          tid, psc_in, psc));
        return BCM_E_NONE;
    }
...
bcm_robo_trunk_destroy(int unit, bcm_trunk_t tid)
{
    ...
    /* Avenger chip */
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_TRUNK_DESTROY(unit, tid));
        return BCM_E_NONE;
    }
    ...
    int member_max, bcm_trunk_member_t *member_array, int *member_count)
{
    ...
    /* Avenger chip */
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(_bcm_robo2_trunk_get(unit, tid, &local_trunk_info,
 ...
 *      unit         - RoboSwitch device unit number (driver internal).
 *      tid          - Trunk ID.
 *      member       - (OUT) Place to store returned trunk info.
 * Returns:
 *      BCM_E_NONE    - Success.
 *      BCM_E_XXX
 */
    ...
    bcm_trunk_member_t *member)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_COMMON,
                (BSL_META_U(unit,
                            "BCM API : bcm_robo_trunk_member_add()..\n")));
    /* Avenger chip */
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_TRUNK_MODIFY(unit, tid,
                            member->gport, BCM_TRUNK_MEMBER_ADD));
    } else {
        return BCM_E_UNAVAIL;
    }
    ...
    bcm_trunk_member_t *member)
{
    ...
    LOG_VERBOSE(BSL_LS_BCM_COMMON,
                (BSL_META_U(unit,
                            "BCM API : bcm_robo_trunk_member_delete()..\n")));
    /* Avenger chip */
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_TRUNK_MODIFY(unit, tid,
                            member->gport, BCM_TRUNK_MEMBER_DELETE));
    } else {
        return BCM_E_UNAVAIL;
    }
