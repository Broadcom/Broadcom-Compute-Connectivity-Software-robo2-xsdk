...
int bcm_robo_vlan_init(int unit)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_VLAN_INIT(unit));
    }
   ...
   /*
    ...
    /*
     * Init VLAN related hw reg/mem robo specific
     * Robo2 VLAN will be initialized by FSAL layer
     */    
    if (!SOC_IS_AVENGER(unit))
...
{
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_VLAN_CREATE(unit, (uint32)vid));
    } else {
...
{
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_VLAN_DESTROY (unit, (uint32)vid));
    } else {
...
{
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_VLAN_DESTROY_ALL(unit));
    } else {
                 ...
                 bcm_pbmp_t pbmp, bcm_pbmp_t ubmp)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        ...
        PBMP_ITER(pbmp, port) {
            int flags = 0;
            if (PBMP_MEMBER(ubmp, port)) {
                flags = BCM_VLAN_GPORT_ADD_UNTAGGED;
            }
            BCM_IF_ERROR_RETURN(bcm_robo_vlan_gport_add(unit, vid, port, flags));
        }
        return BCM_E_NONE;
    }
                ...
                bcm_pbmp_t pbmp)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        bcm_port_t port;
        PBMP_ITER(pbmp, port) {
            BCM_IF_ERROR_RETURN(bcm_robo_vlan_gport_delete(unit, vid, port));
		}
        return BCM_E_NONE;
    }
    BCM_IF_ERROR_RETURN(_bcm_robo_pbmp_check(unit, pbmp));
...
bcm_robo_vlan_stg_get(int unit, bcm_vlan_t vid, bcm_stg_t *stg_ptr)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_VLAN_STG_GET(unit, (uint32)vid, stg_ptr));
    } else {
        CHECK_INIT(unit);
        CHECK_VID(unit, vid);
        /* Check if this VLAN is exist */
...
bcm_robo_vlan_stg_set(int unit, bcm_vlan_t vid, bcm_stg_t stg)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_VLAN_STG_SET(unit, (uint32)vid, (uint32)stg));
    } else {
        CHECK_INIT(unit);
        CHECK_VID(unit, vid);
                          ...
                          bcm_vlan_control_vlan_t control)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        int rv;
        uint32 vlan_index, flags, learnmode, mcastid, ucastid;

        vlan_index = vid;
        flags = control.flags;
        learnmode = control.learn_flags;
        mcastid = 0;
        ucastid = 0;

        rv = DRV_VLAN_CONTROL_VLAN_SET(unit, (uint32)vid, vlan_index, flags, learnmode, mcastid, ucastid);
        BCM_IF_ERROR_RETURN(rv);
    } else if (SOC_IS_ROBO53242(unit) || SOC_IS_ROBO53262(unit)) {
                          ...
                          bcm_vlan_control_vlan_t *control)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        int rv;
        uint32 vlan_index, flags, learnmode, mcastid, ucastid;
        rv = DRV_VLAN_CONTROL_VLAN_GET(unit, (uint32)vid, &vlan_index, &flags, &learnmode, &mcastid, &ucastid);
        control->forwarding_vlan = vlan_index;
        control->flags = flags;
        control->learn_flags = learnmode;
                       ...
                       int flags)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_VLAN_PORT_ADD(unit, (uint32)vlan, (int)port, flags));
        return rv;
    }
...
bcm_robo_vlan_gport_delete(int unit, bcm_vlan_t vlan, bcm_gport_t port)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_VLAN_PORT_REMOVE(unit, (uint32)vlan, (int)port));
    } else {
        CHECK_INIT(unit);
        CHECK_VID(unit, vlan);
...
{
    if (SOC_IS_AVENGER(unit)) {
        BCM_IF_ERROR_RETURN(DRV_VLAN_GPORT_GET_ALL(unit, (uint32)vlan, array_max,
