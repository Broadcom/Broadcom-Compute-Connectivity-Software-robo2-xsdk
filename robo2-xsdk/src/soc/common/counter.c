...
STATIC void
_soc_counter_debug_update(int unit, int pipe, soc_counter_evict_t *evict,
                          soc_counter_non_dma_t *ctr_dma, int wrap_around)
{
    soc_counter_control_t *soc_ctr_ctrl = SOC_CTR_CTRL(unit);
    ...
    if ((!evict) || (!ctr_dma)) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                              "counter_debug_update: Invald param.\n")));
        return;
    }

    if (ctr_dma->flags & _SOC_COUNTER_NON_DMA_SUBSET_PARENT) {
        ctrl_info.instance_type = SOC_CTR_INSTANCE_TYPE_POOL;
        ctrl_info.instance = evict->pool_id;

        if (soc_ctr_ctrl->get_child_dma) {
            soc_reg_t child_id = ctr_dma->id;
            rv = soc_ctr_ctrl->get_child_dma(unit, ctr_dma->id, ctrl_info,
                                             &ctr_dma);

            if ((rv != SOC_E_NONE) || (!ctr_dma)) {
                LOG_INFO(BSL_LS_SOC_COUNTER,
                         (BSL_META_U(unit,
                                     "ERR: Unable to find child (Id %d)\n"),
                          child_id));
                return;
            }
        }
    ...
    ctr_dma->counter_eviction_ct[pipe]++;

    if (wrap_around) {
        LOG_INFO(BSL_LS_SOC_COUNTER,
                 (BSL_META_U(unit,
                             "**** Wrap Around: Ctr Id %d, Pipe %d *****\n"),
                  ctr_dma->id, pipe));
        ctr_dma->counter_rollover_evt[pipe]++;
    }
}
...
#ifdef VENDOR_BROADCOM
    wrap_around = soc_mem_field32_get(unit, mem, entry, COUNTER_WRAP_AROUNDf);
    soc_ctr_ctrl->total_eviction_ct++;
    if (wrap_around) {
        soc_ctr_ctrl->counter_rollover_ct++;
    }
    if (soc_ctr_ctrl->counter_debug_lvl) {
        _soc_counter_debug_update(unit, pipe, evict, pkt_ctr_dma, wrap_around);
        if (soc_ctr_ctrl->counter_debug_lvl > 2) {
            LOG_INFO(BSL_LS_SOC_COUNTER,
                     (BSL_META_U(unit,
                                 "(T %u): Ctr Id %d, Mem Id %d(CtrIdx %d),"
                                 "Pipe %d: PktCnt %d, ByteCnt [%u, %u], Wrap %d\n"),
                      sal_time_usecs(), pkt_ctr_id, mem_id, index, pipe, pkt_cnt,
                      fval[1], fval[0], wrap_around));
        }
    }
#endif /* VENDOR_BROADCOM */
}
...
_soc_ctr_evict_fifo_dma_thread(void *unit_vp)
{
...
#ifdef VENDOR_BROADCOM
    stat_fail_ovrflw_mask = 0;
    soc_reg_field_set(unit, stat_reg, &stat_fail_ovrflw_mask,
                      HOSTMEM_OVERFLOWf, 1);
#endif /* VENDOR_BROADCOM */
    ...
    while ((interval = soc->ctr_evict_interval)) {
            ...
            if (sal_sem_take(soc->ctrEvictIntr, interval) < 0) {
                ...
                LOG_DEBUG(BSL_LS_SOC_EVICT,
                            (BSL_META_U(unit,
                             ...
                             soc->ctr_evict_name, soc_mem_fifo_delay_value));
#ifdef VENDOR_BROADCOM
                /* Counter to keep track of number of CMIC interrupts */
                soc_ctr_ctrl->counter_intr_ct++;
                if (soc_ctr_ctrl->counter_debug_lvl > 1) {
                    LOG_INFO(BSL_LS_SOC_COUNTER,
                             (BSL_META_U(unit,
                                         "(T %u): CMIC Intr Rcvd(#%d)\n"),
                                         sal_time_usecs(),
                                         soc_ctr_ctrl->counter_intr_ct));
                }
#endif /* VENDOR_BROADCOM */
...
#ifdef VENDOR_BROADCOM
                    LOG_DEBUG(BSL_LS_SOC_DIAG,
                              (BSL_META_U(unit,
                                          "Hostmem fail mask 0x%x\n"),
                               rval));
                    hostmem_overflow = FALSE;
                    if (rval & stat_fail_ovrflw_mask) {
                        hostmem_overflow = TRUE;
                    }
#endif /* VENDOR_BROADCOM */
                    fail = TRUE;
                }
...
#ifdef VENDOR_BROADCOM
                /* Debug counter to keep track of hostmem processing count */
                soc_ctr_ctrl->counter_hm_process_ct++;
                if (soc_ctr_ctrl->counter_debug_lvl > 1) {
                    LOG_INFO(BSL_LS_SOC_COUNTER,
                             (BSL_META_U(unit,
                                         "(T %u): Process HostMem(#%d): "
                                         "Msg Count %d\n"),
                                         sal_time_usecs(),
                                         soc_ctr_ctrl->counter_hm_process_ct,
                                         count));
        ...
        if (fail) {
#ifdef VENDOR_BROADCOM
            soc_ctr_ctrl->hostmem_err++;
            if (hostmem_overflow) {
                soc_ctr_ctrl->hostmem_overflow_err++;
            }
#endif /* VENDOR_BROADCOM */
...
#ifdef VENDOR_BROADCOM
    if (!soc_ctr_ctrl->slowdma_enabled) {
        LOG_VERBOSE(BSL_LS_SOC_COUNTER,
                 (BSL_META_U(unit,
                                    ...
                                    soc_reg_t ctr_reg)
{
    ...
    if (soc_feature(unit, soc_feature_th_a0_sw_war) &&
        ...
        efp_ctr_rval = 0;
#ifdef VENDOR_BROADCOM
        /* Enable CLR_ON_READ for EFP Counter before SCHAN DMA */
#endif
    ...
    if (soc_feature(unit, soc_feature_th_a0_sw_war) &&
        ...
        soc_reg_t reg = EGR_EFP_CNTR_UPDATE_CONTROLr;
#ifdef VENDOR_BROADCOM
        /* Disable CLR_ON_READ for EFP Counter after SCHAN DMA */
#endif
        efp_ctr_rval = 0;
        rv = soc_reg32_get(unit, reg, REG_PORT_ANY, 0, &efp_ctr_rval);
        if (SOC_FAILURE(rv)) {
...
#ifdef VENDOR_BROADCOM
                if (!(soc->counter_sync_req || soc_ctr_ctrl->slowdma_enabled)) {
                    break;
                }
#endif /* VENDOR_BROADCOM */
...
soc_counter_start(int unit, uint32 flags, int interval, pbmp_t pbmp)
{
    ...
    if (soc_ctr_ctrl) {
        /* Set default value */
        soc_ctr_ctrl->counter_debug_lvl = 0;
        soc_ctr_ctrl->slowdma_enabled = 1;
#ifdef BCM_TOMAHAWK_SUPPORT
        if (soc_feature(unit, soc_feature_counter_eviction)) {
            soc_ctr_ctrl->slowdma_enabled =
                soc_property_get(unit, "ctr_slowdma_enable", 1);
            soc_ctr_ctrl->counter_debug_lvl =
                soc_property_get(unit, "ctr_eviction_debug", 0);
        }
#endif /* BCM_TOMAHAWK_SUPPORT */
    }
#endif /* VENDOR_BROADCOM */
...
#ifdef VENDOR_BROADCOM
/* Handler function for Diag command "show c debug" */
void
soc_counter_debug_show(int unit)
{
    soc_control_t *soc;
    soc_reg_t id;
    soc_counter_non_dma_t *ctr_dma;
    int pipe, count = 0, child_ct = 0;
    soc_counter_control_t *soc_ctr_ctrl = SOC_CTR_CTRL(unit);

    soc = SOC_CONTROL(unit);
    cli_out("Counter Debug Info:\n");
    cli_out("*******************\n");

    if (soc_ctr_ctrl->counter_debug_lvl) {
        cli_out("Wrap Ct:\n");
        for (id = SOC_COUNTER_NON_DMA_START; id < SOC_COUNTER_NON_DMA_END; id++) {
            count = 0;
            ctr_dma = &soc->counter_non_dma[id - SOC_COUNTER_NON_DMA_START];

            if ((!ctr_dma) ||
                (!(ctr_dma->flags & _SOC_COUNTER_NON_DMA_DO_DMA)) ||
                (!(ctr_dma->flags & _SOC_COUNTER_NON_DMA_CTR_EVICT))) {
                continue;
            }

            LOG_INFO(BSL_LS_SOC_COUNTER,
                     (BSL_META_U(unit, "  Id: %d (Flags: 0x%x)\n"),
                                 ctr_dma->id, ctr_dma->flags));

            if (ctr_dma->flags & _SOC_COUNTER_NON_DMA_SUBSET_PARENT) {
                child_ct = ctr_dma->extra_ctr_ct << 1;
            ...
            do {
                cli_out("    Ctr Name: %s (Flags: 0x%x)\n",
                        ctr_dma->cname, ctr_dma->flags);

                for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
                    if (ctr_dma->counter_eviction_ct[pipe] || 
                        ctr_dma->counter_rollover_evt[pipe]) {
                        cli_out("+      Pipe %d: Eviction Ct %d, Wrap Ct %d\n",
                                 pipe, ctr_dma->counter_eviction_ct[pipe],
                                 ctr_dma->counter_rollover_evt[pipe]);
                    } else {
                        LOG_INFO(BSL_LS_SOC_COUNTER,
                                 (BSL_META_U(unit,
                                             "       Pipe %d: Eviction Ct %d, "
                                             "Wrap Ct %d\n"),
                                  pipe, 0, 0));
                    }
                }
                count += 2; /* Skipping Byte Counter */
                ctr_dma += 2;
            } while ((count < child_ct) && ctr_dma);
        }
    }
    cli_out("Total Wrap Ct (All pools): %d\n", soc_ctr_ctrl->counter_rollover_ct);
    cli_out("Total Eviction Ct (All pools): %d\n",
            soc_ctr_ctrl->total_eviction_ct);
    cli_out("Total CMIC Intr Ct: %d\n", soc_ctr_ctrl->counter_intr_ct);
    cli_out("Total HostMem Process Ct: %d\n", soc_ctr_ctrl->counter_hm_process_ct);
    ...
    cli_out("  Overflow Error Count : %d\n", soc_ctr_ctrl->hostmem_overflow_err);
    LOG_INFO(BSL_LS_SOC_COUNTER,
             (BSL_META_U(unit,
                         "\nDebug Level: %d\n\n"),
              soc_ctr_ctrl->counter_debug_lvl));
...
/* Handler function for Diag command "clear c debug" */
void
soc_counter_debug_clear(int unit)
{
    soc_control_t *soc;
    soc_reg_t id;
    soc_counter_non_dma_t *ctr_dma;
    ...
    for (id = SOC_COUNTER_NON_DMA_START; id < SOC_COUNTER_NON_DMA_END; id++) {
        ...
        do {
            for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
                ctr_dma->counter_eviction_ct[pipe] = 0;
                ctr_dma->counter_rollover_evt[pipe] = 0;
            }
            count += 2; /* Skipping Byte Counter */
            ctr_dma += 2;
        } while ((count < child_ct) && ctr_dma);
    }
    soc_ctr_ctrl->counter_rollover_ct = 0;
    soc_ctr_ctrl->total_eviction_ct = 0;
    soc_ctr_ctrl->counter_hm_process_ct = 0;
    soc_ctr_ctrl->counter_intr_ct = 0;
    soc_ctr_ctrl->hostmem_err = 0;
    soc_ctr_ctrl->hostmem_overflow_err = 0;
