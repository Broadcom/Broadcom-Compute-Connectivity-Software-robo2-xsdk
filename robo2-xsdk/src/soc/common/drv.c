    ...
    if (SOC_FUNCTIONS(unit)) {
        if (SOC_FUNCTIONS(unit)->soc_misc_init) {
            ...
            if (SOC_IS_XGS(unit)) {
                ...
                /* Refer to SDK-56268 for more info on the h/w issue */
#endif
...
char *
soc_property_robo2_port_get_str(int unit, soc_port_t port,
                          const char *name)
{
    char        *s, *p;
...
#ifdef CONFIG_CASCADED_MODE
    return(soc_property_robo2_port_get_str(0,port,name));
#endif /* CONFIG_CASCADE_MODE */
