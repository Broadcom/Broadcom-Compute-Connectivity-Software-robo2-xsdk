...
_soc_mem_scan_thread(void *unit_vp)
{
    ...
    /* JIRA TH-4364
     * "OBM queue bffer" is exception - because for this mem SBE correction
     * also has problem. Thus SBE for this mem can lead to operation failure.
     * And also because we have problem in reporing SBE (no interrupts)
     * as described in TH-4360, DE recommends that we keep SBE reporting enabled
     * and then poll it.
     * For this mem, DE asserts that DBE reporting will not be missed even if we
     * have SBE followed by DBE.
     */
