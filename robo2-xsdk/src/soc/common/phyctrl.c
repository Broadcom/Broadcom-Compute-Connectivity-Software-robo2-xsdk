    ...
    if (SOC_IS_SBX(unit)) {
     ...
     if (SOC_IS_ROBO(unit)) {
        if(SOC_IS_AVENGER(unit)) {
           /* cascaded to be handled */
#ifdef CONFIG_CASCADED_MODE
           if ((port < 12) ||((port>15) && (port< 28)))  {
#else
           if (port < 12) {
#endif /* CONFIG_CASCADE_MODE */
              int_pc.read  = soc_robo2_miim_int_read;
              int_pc.write = soc_robo2_miim_int_write;

              ext_pc.read  = soc_robo2_miim_int_read;
              ext_pc.write = soc_robo2_miim_int_write;

#ifdef CONFIG_CASCADED_MODE
           } else if ((port == 12 || port == 13) || (port == 28) || (port ==29)) {
#else
           } else if (port == 12 || port == 13) {
#endif /* CONFIG_CASCADE_MODE */
              int_pc.read  = soc_robo2_pmi_int_read;
              int_pc.write = soc_robo2_pmi_int_write;

              ext_pc.read  = soc_robo2_pmi_int_read;
              ext_pc.write = soc_robo2_pmi_int_write;
           }
        } else {
