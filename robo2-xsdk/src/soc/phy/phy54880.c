...
_phy_54880_br_ability_advert_std_set(int unit, soc_port_t port, soc_port_ability_t *ability)
{
    ...
    /* Turning off as these port speeds have been removed for now
     * could be turned on again (if needed) after the extended port 
     * speed bitmap is added 
     *
    if (ability->speed_full_duplex & SOC_PA_SPEED_20MB)  {
        adv_ability |= LONGREACH_ABILITY_20M;
    }
    if (ability->speed_full_duplex & SOC_PA_SPEED_25MB)  {
        adv_ability |= LONGREACH_ABILITY_25M;
    }
    if (ability->speed_full_duplex & SOC_PA_SPEED_33MB)  {
        adv_ability |= LONGREACH_ABILITY_33M;
    }
    if (ability->speed_full_duplex & SOC_PA_SPEED_50MB)  {
        adv_ability |= LONGREACH_ABILITY_50M;
    }*/
#endif
...
_phy_54880_br_ability_advert_std_get(int unit, soc_port_t port, soc_port_ability_t *ability)
{
     ...
     * speed bitmap is added 
     *
    if (adv_ability & LONGREACH_ABILITY_20M) {
      ability->speed_full_duplex |= SOC_PA_SPEED_20MB;
    }
    if (adv_ability & LONGREACH_ABILITY_25M) {
      ability->speed_full_duplex |= SOC_PA_SPEED_25MB;
    }
    if (adv_ability & LONGREACH_ABILITY_33M) {
      ability->speed_full_duplex |= SOC_PA_SPEED_33MB;
    }
    if (adv_ability & LONGREACH_ABILITY_50M) {
      ability->speed_full_duplex |= SOC_PA_SPEED_50MB;
    }*/
#endif
...
_phy_54880_br_ability_remote_std_get(int unit, soc_port_t port, soc_port_ability_t *ability)
{
     ...
     * speed bitmap is added 
     *
    if (rem_ability & LONGREACH_ABILITY_20M) {
      ability->speed_full_duplex |= SOC_PA_SPEED_20MB;
    }
    if (rem_ability & LONGREACH_ABILITY_25M) {
      ability->speed_full_duplex |= SOC_PA_SPEED_25MB;
    }
    if (rem_ability & LONGREACH_ABILITY_33M) {
      ability->speed_full_duplex |= SOC_PA_SPEED_33MB;
    }
    if (rem_ability & LONGREACH_ABILITY_50M) {
      ability->speed_full_duplex |= SOC_PA_SPEED_50MB;
    }*/
#endif
...
_phy_54880_br_ability_local_get(int unit, soc_port_t port, soc_port_ability_t *ability)
{
     ...
     */
    if (PHY_IS_BCM54881(pc)) {
        ability->speed_full_duplex  = SOC_PA_SPEED_100MB | /*SOC_PA_SPEED_50MB |
                                      SOC_PA_SPEED_33MB | SOC_PA_SPEED_25MB |
                                      SOC_PA_SPEED_20MB |*/ SOC_PA_SPEED_10MB;
    } else {
        ability->speed_full_duplex  = SOC_PA_SPEED_100MB |SOC_PA_SPEED_10MB; 
    }
#else
