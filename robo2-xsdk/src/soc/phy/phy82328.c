...
#ifdef VENDOR_GAMMA
#define PHY82328_GP_REG_1_SPD_20G                ((MGT_TOP_PMD_USR_GP_REGISTER_1_SPEED_100G_MASK) | \
...
#ifdef VENDOR_GAMMA
    #define PHY82328_20G_PORT_MODE(_pc)  \
    (SOC_INFO((_pc)->unit).port_num_lanes[(_pc)->port] == 2)
#endif
...
#ifdef VENDOR_GAMMA
    #define RMT_PHY_CAP(_pc)		(((phy82328_dev_desc_t *)((_pc) + 1))->rmt_phy_cap)
    #define RMT_PHY_MODE(_pc)		(((phy82328_dev_desc_t *)((_pc) + 1))->phy_mode)
#endif
...
#ifdef VENDOR_GAMMA
    SOC_PORT_IF_XFI2,                   /* PHY_SYSTEM_INTERFACE_XFI2    */
    SOC_PORT_IF_SR2,                    /* PHY_SYSTEM_INTERFACE_SR2     */
    SOC_PORT_IF_CR2,                    /* PHY_SYSTEM_INTERFACE_CR2     */
    SOC_PORT_IF_KR2                     /* PHY_SYSTEM_INTERFACE_KR2     */
#endif        
...
#ifdef VENDOR_GAMMA
STATIC int
_phy_82328_intf_type_20G(soc_port_if_t intf_type);
#endif
...
int32 phy82328_intf_side_reg_select(int unit, soc_port_t port, uint16 intf)
{
    phy_ctrl_t *pc;
#ifdef VENDOR_GAMMA
    uint16 phy_id = 0, i=0, num_lanes = 0;
#endif
    pc = EXT_PHY_SW_STATE(unit, port);

#ifdef VENDOR_GAMMA
    phy_id = pc->phy_id;
    num_lanes = PHY82328_20G_PORT_MODE(pc) ? PHY82328_20G_NUM_LANES : 1;
    for (i = 0; i < num_lanes; i++)
    {
        if (num_lanes > 1) {
            pc->phy_id = (phy_id & (~0x1)) + i;
        }
#endif            
        SOC_IF_ERROR_RETURN
...
#ifdef VENDOR_GAMMA
    if (line_intf->speed == PHY82328_SPD_20G) {
        sys_intf->speed = PHY82328_SPD_20G;
        line_intf->type = SOC_PORT_IF_SR2;
        if (!(_phy_82328_intf_type_20G(sys_intf->type))) {
            /* system side interface is not compatible so overwrite with default */
            LOG_ERROR(BSL_LS_SOC_PHY,
                      (BSL_META_U(unit,
                                  "PHY82328 incompatible 20G system side interface, "
                                  "using default: u=%d p=%d\n"), unit, port));
            sys_intf->type = SOC_PORT_IF_XFI2;
        }
        if (RMT_PHY_CAP(pc)) {
            line_intf->type = SOC_PORT_IF_KR2;
        }
    }
#endif    
    else { /* 10G/1G */
    ...
    *speed = 0;
    ...
    case PHY82328_GP_REG_1_SPD_10G:
        *speed = PHY82328_SPD_10G;
        break;
#ifdef VENDOR_GAMMA        
    case PHY82328_GP_REG_1_SPD_20G:
        *speed = PHY82328_SPD_20G;
        break;
#endif    
...
#ifdef VENDOR_GAMMA    
    else if (PHY82328_20G_PORT_MODE(pc)) {
        line_intf->speed = PHY82328_SPD_20G;
    }
#endif    
    else {
        line_intf->speed = PHY82328_SPD_10G;
...
#ifdef VENDOR_GAMMA
        else if ( line_intf->speed == PHY82328_SPD_20G) {
            line_intf->type = SOC_PORT_IF_SR2;
            sys_intf->type = SOC_PORT_IF_XFI2;
            if (RMT_PHY_CAP(pc)) {
                line_intf->type = SOC_PORT_IF_KR2;
...
_phy_82328_init_pass1(int32 unit, soc_port_t port)
{
    ...
    SOC_IF_ERROR_RETURN(phy_82328_control_set(unit, port,SOC_PHY_CONTROL_PORT_OFFSET,
                                              offset));
#ifdef VENDOR_GAMMA
    RMT_PHY_CAP(pc) = soc_property_port_get(unit, port, "RMT_PHY_CAP", FALSE);
    if (RMT_PHY_CAP(pc)) {
        LOG_CLI((BSL_META_U(unit,
                            "Local PHY works only when SW Link scan is ON,"
                 " port %d\n"),port));
        SOC_IF_ERROR_RETURN(PHY_CONTROL_SET(int_pc->pd, unit, port,
                    SOC_PHY_CONTROL_TX_LANE_SQUELCH, 1));
    }
    RMT_PHY_MODE(pc) = SOC_PORT_MEDIUM_COPPER;
#endif
...
#ifdef VENDOR_GAMMA
    if (RMT_PHY_CAP(pc)) {
        if (LINE_INTF(pc).speed == PHY82328_SPD_40G) {
            max_lane = PHY82328_NUM_LANES;
        } else if (LINE_INTF(pc).speed == PHY82328_SPD_20G) {
            max_lane = 2;
        } else {
            max_lane = 1;
        }
...
#ifdef VENDOR_GAMMA
STATIC int 
_phy_82328_rmt_phy_cap_update(int unit, soc_port_t port) 
{
 	phy_ctrl_t *pc, *int_pc;
	uint16 rmt_phy_ablity;
    phy82328_intf_cfg_t *line_intf, *sys_intf;
    int32 speed = 0, intf = 0;
    soc_port_if_t new_type;
    ...
    SOC_IF_ERROR_RETURN
			(READ_PHY82328_MMF_AN_REG(unit, pc, PHY82328_RMT_PHY_ABLITY_REG,
                                      &rmt_phy_ablity));
    if (rmt_phy_ablity & PHY82328_RMT_PHY_ABLITY_VALID_MASK) {
        if (rmt_phy_ablity & PHY82328_RMT_PHY_ABLITY_KR_MASK) {
            speed = PHY82328_SPD_10G;
            intf =  SOC_PORT_IF_KR;
            sys_intf->type =  SOC_PORT_IF_XFI;
        } else if(rmt_phy_ablity & PHY82328_RMT_PHY_ABLITY_KR2_MASK) {
            speed = PHY82328_SPD_20G;
            intf = SOC_PORT_IF_KR2;
            sys_intf->type =  SOC_PORT_IF_XFI2;
        } else if(rmt_phy_ablity & PHY82328_RMT_PHY_ABLITY_KR4_MASK) {
            speed = PHY82328_SPD_40G;
            intf = SOC_PORT_IF_KR4;
            sys_intf->type =  SOC_PORT_IF_XLAUI;
        }
        if ((line_intf->speed != speed) || (line_intf->type != intf)) {
            line_intf->speed = speed;
            sys_intf->speed = speed;	
            line_intf->type = intf;
            LOG_CLI((BSL_META_U(unit,
                                "Setting speed:%d and interface:%s\n"), 
                     speed, phy82328_intf_names[line_intf->type]));
            
            if (speed == PHY82328_SPD_10G) {
                new_type = SOC_PORT_IF_XFI;
            } else if (speed == PHY82328_SPD_20G) {
                new_type = SOC_PORT_IF_KR;
            } else if (speed == PHY82328_SPD_40G) {
                new_type = SOC_PORT_IF_KR4;
            } else {
                new_type = sys_intf->type;
            }
            LOG_CLI((BSL_META_U(unit,
                "NEW internal interface:%s\n"), phy82328_intf_names[new_type]));
            (void) PHY_INTERFACE_SET(int_pc->pd, unit, port, new_type);
            (void) PHY_SPEED_SET(int_pc->pd, unit, port, sys_intf->speed);
        }
        SOC_IF_ERROR_RETURN(PHY_CONTROL_SET(int_pc->pd, unit, port,
                    SOC_PHY_CONTROL_TX_LANE_SQUELCH, 0));
    } else {
        SOC_IF_ERROR_RETURN(PHY_CONTROL_SET(int_pc->pd, unit, port,
                    SOC_PHY_CONTROL_TX_LANE_SQUELCH, 1));
    }
...
phy_82328_link_get(int32 unit, soc_port_t port, int32 *link)
{
    ...
    if (RMT_PHY_CAP(pc)) {
        SOC_IF_ERROR_RETURN(
                _phy_82328_rmt_phy_cap_update(unit, port));
    }
#endif    
...
#ifdef VENDOR_GAMMA
    uint16 num_lanes = 0, phy_id = 0, i =0;
#endif
    phy_ctrl_t  *pc = EXT_PHY_SW_STATE(unit, port);
...
_phy_82328_intf_speed_reg_get(int32 unit, soc_port_t port, int32 speed, uint16 *reg_data, uint16 *reg_mask)
{
    ...
    *reg_mask = 0;		
    switch (speed) {
        ...
        break;
#ifdef VENDOR_GAMMA
    case PHY82328_SPD_20G:
		*reg_data = PHY82328_GP_REG_1_SPD_20G;
		break;
#endif             
        default :
            LOG_ERROR(BSL_LS_SOC_PHY,
...
#ifdef VENDOR_GAMMA
    if (RMT_PHY_CAP(pc)) {
        *reg_data |= (MGT_TOP_PMD_USR_GP_REGISTER_1_LINE_TYPE_MASK |
                      MGT_TOP_PMD_USR_GP_REGISTER_1_LINE_CU_TYPE_MASK);
        *reg_mask |= (MGT_TOP_PMD_USR_GP_REGISTER_1_LINE_TYPE_MASK |
                      MGT_TOP_PMD_USR_GP_REGISTER_1_LINE_CU_TYPE_MASK);
    }
#endif
    if (side == PHY82328_INTF_SIDE_LINE) {
...
#ifdef VENDOR_GAMMA            
            if (speed == PHY82328_SPD_20G) {
                new_type = SOC_PORT_IF_KR;
                (void) PHY_INTERFACE_SET(int_pc->pd, unit, port, new_type);
                (void) PHY_SPEED_SET(int_pc->pd, unit, port, speed);
            } else if (RMT_PHY_CAP(pc)) {
                if (speed == PHY82328_SPD_10G) {
                   new_type = SOC_PORT_IF_XFI;
                } else if (speed == PHY82328_SPD_40G) {
                   new_type = SOC_PORT_IF_KR4;
...
#ifdef VENDOR_GAMMA
            case PHY82328_SPD_20G:
            if (line_intf->speed <= 10000) {
                /* 
                 * If changing speed from lesser that 10000 to 20000, set line interface to SR2
                 */
				if (!_phy_82328_intf_type_20G(line_intf->type)) {
					line_intf->type = SOC_PORT_IF_SR2;
				}
				/*
...
_phy_82328_polarity_flip_tx(int32 unit, soc_port_t port, phy82328_intf_side_t side, uint16 cfg_pol)
{
    ...
    if ((PHY82328_SINGLE_PORT_MODE(pc))) {
        ...
        phy_id = pc->phy_id;
        if (cfg_pol != PHY82328_POL_DND) {
            for (lane = 0; lane < PHY82328_20G_NUM_LANES; lane++) {
	            /* Check for all lanes or whether this lane has polarity flipped */
                if ((cfg_pol & POL_CONFIG_LANE_MASK(lane)) == POL_CONFIG_LANE_MASK(lane)) {
				    flip = 1;
                } else {
                ...
                LOG_INFO(BSL_LS_SOC_PHY,
                         (BSL_META_U(unit,
                                     "20G 82328 tx polarity flip=%d: u=%d p=%d" 
                                     "lane=%d\n"), flip, unit, port, lane));
    ...
    if ((PHY82328_SINGLE_PORT_MODE(pc))) {
        ...
        if (cfg_pol != PHY82328_POL_DND) {
            for (lane = 0; lane < PHY82328_20G_NUM_LANES; lane++) {
                ...
                LOG_INFO(BSL_LS_SOC_PHY,
                         (BSL_META_U(unit,
                                     "20G 82328 Rx polarity flip=%d: u=%d p=%d" 
                                     "lane=%d\n"), flip, unit, port, lane));
...
_phy_82328_remote_loopback_set(int32 unit,soc_port_t port,int32 enable)
{
    ...
    for (i = 0; i < num_lanes; i++) {
        if (num_lanes > 1) {
            pc->phy_id = (phy_id & (~0x1)) + i; /* Port 1 and 0 */
        }
#endif
...
phy_82328_lb_get(int32 unit, soc_port_t port, int32 *enable)
{
    ...
    phy_ctrl_t  *pc = EXT_PHY_SW_STATE(unit, port);
#ifdef VENDOR_GAMMA
    uint16 phy_id = 0, i=0, num_lanes = 1;
#endif
								   ...
								   phy82328_intf_side_t side, uint32 *value)
{
...
#ifdef VENDOR_GAMMA
    if (PHY82328_20G_PORT_MODE(pc) && (lane_idx > 1)) {
        return SOC_E_PARAM;
    }
#endif
	 ...
	 */
#ifdef VENDOR_GAMMA
	if (PHY82328_20G_PORT_MODE(pc) && (lane == 1)) {
        phy_id = pc->phy_id;
        pc->phy_id = (phy_id & (~0x1)) + lane;
    }
#endif     
								   ...
								   uint32 value)
{
...
#ifdef VENDOR_GAMMA    
    if (PHY82328_20G_PORT_MODE(pc) && (lane > 1) && (lane != PHY82328_ALL_LANES)) {
        return SOC_E_PARAM;
    }
#endif    
	...
	if (PHY82328_SINGLE_PORT_MODE(pc)) {
        ...
        phy_id = pc->phy_id;
        lane_start = (lane == PHY82328_ALL_LANES) ? 0 : lane;
		lane_end   = (lane == PHY82328_ALL_LANES) ? 2 : lane + 1;
    } 
#endif
    else {
...
#ifdef VENDOR_GAMMA
        if (PHY82328_20G_PORT_MODE(pc)) {
            pc->phy_id = (phyid & (~0x1)) + lane_idx; /* Port 1 and 0 */
        }
#endif
...
#ifdef VENDOR_GAMMA
    LOG_CLI((BSL_META_U(unit,
                        "AOC : %d RPHY cap: %d\n"), RMT_PHY_MODE(pc), RMT_PHY_CAP(pc)));
#endif
...
#ifdef VENDOR_GAMMA
    pa_speed |= PHY82328_SINGLE_PORT_MODE(pc) ? 0: SOC_PA_SPEED_20GB;
#endif        
...
#ifdef VENDOR_GAMMA
    if (PHY82328_20G_PORT_MODE(pc) && (lane != 0)) {
        if (lane > 1) {
            return SOC_E_PARAM;
        }
        phy_id = pc->phy_id;
                            ...
                            soc_port_ability_t *ability)
{
	...
	line_intf = &(LINE_INTF(pc));
#ifdef VENDOR_GAMMA
    if (line_intf->type != SOC_PORT_IF_KR2)
    {
#endif /* VENDOR_GAMMA */
    ...
    if (an && an_done) {
        ...
        if (RMT_PHY_CAP(pc)) {
            SOC_IF_ERROR_RETURN
              (READ_PHY82328_MMF_AN_REG(unit, pc, 
                              PHY82328_RMT_PHY_ABLITY_REG, &pause_adv));
                LOG_CLI((BSL_META_U(unit,
                                    "pause adv:%x\n"), pause_adv));
            if (pause_adv & PHY82328_RMT_PHY_ABLITY_VALID_MASK) {
                pause_adv &= 0x3800;
                /* Move bit 11,12 and 13 to 10,11,12 */
                pause_adv >>= 1;
            }
 ...
 */
STATIC int
phy82328_medium_config_set(int unit, soc_port_t port,
                           ...
                           soc_phy_config_t  *cfg)
{
    ...
    switch (medium) {
        case SOC_PORT_MEDIUM_COPPER:
            ...
            LOG_INFO(BSL_LS_SOC_PHY,
                     (BSL_META_U(unit,
                                 "Configuring G28 in Copper mode\n")));
            break;
        case SOC_PORT_MEDIUM_FIBER:
            RMT_PHY_MODE(pc) = SOC_PORT_MEDIUM_FIBER;
            chan_data = (1 << 12);
            LOG_INFO(BSL_LS_SOC_PHY,
                     (BSL_META_U(unit,
                                 "Configuring G28 in AOC mode\n")));
            break;
        default:
            return SOC_E_PARAM;
    }
    SOC_IF_ERROR_RETURN(PHY_ENABLE_GET(int_pc->pd, unit, port, &enable));
    if (enable == 1) {
        /* Disable INternal Serdes*/
        SOC_IF_ERROR_RETURN(PHY_ENABLE_SET(int_pc->pd, unit, port, 0));
    }

    /* Configure 82328 to CU/AOC mode */
    chan_mask = (1 << 12);
    SOC_IF_ERROR_RETURN(
        MODIFY_PHY82328_MMF_PMA_PMD_REG(unit, pc, MGT_TOP_GP_REGISTER_0,
                                            chan_data, chan_mask));
    SOC_IF_ERROR_RETURN (
        MODIFY_PHY82328_MMF_PMA_PMD_REG(unit, pc, MGT_TOP_GP_REGISTER_1,
        MGT_TOP_PMD_USR_GP_REGISTER_1_FINISH_CHANGE_MASK,
...
STATIC int
phy82328_medium_config_get(int unit, soc_port_t port,
                           soc_port_medium_t  medium,
                           soc_phy_config_t  *cfg)
{
    return SOC_E_NONE;
 ...
 */
STATIC int
phy82328_medium_get(int unit, soc_port_t port, soc_port_medium_t *medium)
{
    phy_ctrl_t       *pc;
...
#ifdef VENDOR_GAMMA    
    phy82328_medium_config_set,  /* phy_medium_config_set        */
    phy82328_medium_config_get,  /* phy_medium_config_get        */
    phy82328_medium_get,         /* phy_medium_get               */
#else
