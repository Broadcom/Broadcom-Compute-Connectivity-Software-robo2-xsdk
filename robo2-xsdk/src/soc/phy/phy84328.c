...
#ifdef VENDOR_GAMMA
    #define PHY84328_20G_PORT_MODE(_pc)  \
...
#ifdef VENDOR_GAMMA            
    #define PHY84328_DEV1_GP_REGISTER_1_SPEED_20G			((1 << PHY84328_DEV1_GP_REGISTER_1_SPEED_100G_SHIFT) |  \
...
#ifdef VENDOR_GAMMA
    num_lanes = (PHY84328_20G_PORT_MODE(pc)) ? 2 : 1;
    phy_id = pc->phy_id;
#endif    
...
#ifdef VENDOR_GAMMA
STATIC int
_phy_84328_intf_type_20000(soc_port_if_t intf_type)
{
	int rv;
...
#ifdef VENDOR_GAMMA
    case 20000:
		*reg_data = PHY84328_DEV1_GP_REGISTER_1_SPEED_20G;
		break;
#endif        
	case 1000:
    case 100:
...
#ifdef VENDOR_GAMMA    
    else if (PHY84328_20G_PORT_MODE(pc)) {
		int	lane, phy_id = 0;
        phy_id = pc->phy_id;
    ...
    *speed = 0;
    ...
    case PHY84328_DEV1_GP_REGISTER_1_SPEED_10G | PHY84328_DEV1_GP_REGISTER_1_SPEED_1G:
        ...
        *speed = 1000;
        break;
#ifdef VENDOR_GAMMA    
    case PHY84328_DEV1_GP_REGISTER_1_SPEED_20G:
        *speed = 20000;
        break;
#endif        
    default:
...
#ifdef VENDOR_GAMMA
        if (!PHY84328_SINGLE_PORT_MODE(pc) && PHY84328_20G_PORT_MODE(pc)){
								   ...
								   uint32 value)
{
...
#ifdef VENDOR_GAMMA    
    if (PHY84328_20G_PORT_MODE(pc) && (lane > 1) && (lane != PHY84328_ALL_LANES)) {
        return SOC_E_PARAM;
    }
#endif	
	...
	if (PHY84328_SINGLE_PORT_MODE(pc)) {
        ...
        phy_id = pc->phy_id;
        lane_start = (lane == PHY84328_ALL_LANES) ? 0 : lane;
		lane_end   = (lane == PHY84328_ALL_LANES) ? 2 : lane + 1;
    } 
#endif    
    else {
								   ...
								   phy84328_intf_side_t side, uint32 *value)
{
	...
	*value = 0;
#ifdef VENDOR_GAMMA
	if (PHY84328_20G_PORT_MODE(pc) && (lane == 1)) {
        phy_id = pc->phy_id;
        pc->phy_id = (phy_id & (~0x1)) + lane;
    }
#endif    
	...
	if (PHY84328_SINGLE_PORT_MODE(pc)) {
        ...
        if (PHY84328_20G_PORT_MODE(pc)) {
            if (lane == PHY84328_ALL_LANES) {
                forced = ((data & (0x3 << (pc->phy_id & 0x03))) == 
                        (0x3 << (pc->phy_id & 0x03))) ? 1 : 0;
            }
        }
#endif        
	}
								 ...
								 phy84328_intf_side_t side, uint32 value)
{
	...
	if (PHY84328_SINGLE_PORT_MODE(pc)) {
        ...
        phy_id = pc->phy_id;
		lane_start = (lane == PHY84328_ALL_LANES) ? 0 : lane;
		lane_end   = (lane == PHY84328_ALL_LANES) ? 2 : (lane_start + 1);
    }
#endif    
    else {
		/* Make sure specified lane matches lane for the channel */
								 ...
								 phy84328_intf_side_t side, uint32 *value)
{
    ...
    if (PHY84328_20G_PORT_MODE(pc) && lane == 1) {
        phy_id = pc->phy_id;
        pc->phy_id = (phy_id & (~0x1)) + lane; /* Port 1 and 0 */
    }
#endif    
...
#ifdef VENDOR_GAMMA
            if (PHY84328_20G_PORT_MODE(pc) && (lane != lane_start)) {
                pc->phy_id = (phy_id & (~0x1)) + (lane & 0x1);
            }
#endif            
...
#ifdef VENDOR_GAMMA
    if (PHY84328_20G_PORT_MODE(pc) && (lane != 0)) {
        if (lane > 1) {
            return SOC_E_PARAM;
        }
        phy_id = pc->phy_id;
    ...
    if (PHY84328_SINGLE_PORT_MODE(pc)) {
        ...
        case 20000:
            if (line_intf->speed <= 10000) {
                 ...
                 */
				if (!_phy_84328_intf_type_20000(line_intf->type)) {
				 ...
				 */
				sys_intf->type = CFG_SYS_INTF(pc);
				if (!_phy_84328_intf_type_20000(sys_intf->type)) {
