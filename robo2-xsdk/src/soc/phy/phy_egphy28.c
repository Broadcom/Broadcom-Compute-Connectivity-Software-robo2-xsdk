...
_phy_egphy28_reset_setup(int unit, soc_port_t port)
{
    ...
    /* Avenger workaround to bring GPHY amplitude within IEEE Specs */
    SOC_IF_ERROR_RETURN(WRITE_PHY_REG(unit, pc, 0x15, 0));
    /* AFE_TX_CONFIG_1 1g AB symmetry Iq  */
    SOC_IF_ERROR_RETURN(WRITE_PHY_REG(unit, pc, 0x17, 0x403A));
    SOC_IF_ERROR_RETURN(WRITE_PHY_REG(unit, pc, 0x15, 0xBCC));
    /* Register: AFE_TX_IQ_RX_LP LPF BW  */
    SOC_IF_ERROR_RETURN(WRITE_PHY_REG(unit, pc, 0x17, 0x39));
    SOC_IF_ERROR_RETURN(WRITE_PHY_REG(unit, pc, 0X15, 0X233f));
    /* afe_tempsen_others  RCAL +6LSB to make impedance from 112 to 100ohm*/
    SOC_IF_ERROR_RETURN(WRITE_PHY_REG(unit, pc, 0x17, 0x3B));
    SOC_IF_ERROR_RETURN(WRITE_PHY_REG(unit, pc, 0x15, 0xAD40));
    /* DSP_TAP10 since rcal make R smaller, make master current -4% */
    SOC_IF_ERROR_RETURN(WRITE_PHY_REG(unit, pc, 0x17, 0xA));
    SOC_IF_ERROR_RETURN(WRITE_PHY_REG(unit, pc, 0x15, 0x91B));
...
phy_egphy28_ability_local_get(int unit, soc_port_t port, soc_port_ability_t *ability)
{
    ...
    if (SOC_IS_AVENGER(unit)) {
        if(soc_robo2_is_1g_disabled(unit, port)) {
