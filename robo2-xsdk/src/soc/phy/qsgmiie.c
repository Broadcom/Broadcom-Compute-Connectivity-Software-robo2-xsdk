...
typedef struct {
    ...
    /* the following fields are under a VENDOR_BROADCOM conditional compile in tscmod.c */
    int los_usec;                   /* "qsgmiie_los_usec" */
    int an_ctl;                     /* "qsgmiie_diag_an_ctl" */
    int an_link_fail_nocl72_wait_timer;
                                    /* "qsgmiie_diag_an_link_fail_nocl72_wait_timer" */
    int pll_vco_step;               /* "qsgmiie_diag_pll_vco_step" */
    int ability_mask;               /* "tsc_diag_ability_mask" */ 
    int diag_regs;                  /* "qsgmiie_diag_regs" */
    int linkup_bert;                /* "qsgmiie_linkup_bert" */
    int fault_disable;              /* "qsgmiie_fault_disable" */
    int svk_id;                     /* "qsgmiie_svk_id" */
    int diag_verb;                  /* "qsgmiie_diag_verb" */

    /* the following fields are used in tscmod.c, but may not be needed for qsgmiie */
    int pll_divider;                /* spn_XGXS_PHY_PLL_DIVIDER */
    int sgmii_mstr;                 /* spn_SERDES_SGMII_MASTER */
    int pdetect10g;                 /* spn_XGXS_PDETECT_10G */
    int cl37an;                     /* spn_PHY_AN_C37 */
    int an_cl72;                    /* spn_PHY_AN_C72 */
    int forced_init_cl72;           /* spn_PORT_INIT_CL72 */
    int cidL;
    int cidH;
#endif
#endif
...
/* mark it for the assertion of lane_num and chip_num is assigned */
        ...
        */
        ...
        pCfg->lane0_rst          = soc_property_get(unit, spn_SERDES_LANE0_RESET, TRUE);
        pCfg->auto_medium        = soc_property_port_get(unit, port, spn_SERDES_AUTOMEDIUM, FALSE);
        pCfg->rxaui              = soc_property_port_get(unit, port, spn_SERDES_2WIRE_XAUI, FALSE);
        pCfg->cx42hg             = soc_property_port_get(unit, port, spn_CX4_TO_HIGIG, FALSE);
        pCfg->lane_mode          = soc_property_port_get(unit, port, spn_PHY_HL65_1LANE_MODE, 0);

        
        pCfg->los_usec 
             = soc_property_port_get(unit, port, "tsc_los_usec", 75000); 
        pCfg->an_ctl 
          = soc_property_port_get(unit, port, "tsc_diag_an_ctl", 0);
        pCfg->an_link_fail_nocl72_wait_timer
          = soc_property_port_get(unit, port, "tsc_diag_an_link_fail_nocl72_wait_timer", 0) ;
        pCfg->pll_vco_step 
          = soc_property_port_get(unit, port, "tsc_diag_pll_vco_step", 0) ;
        pCfg->ability_mask  
          = soc_property_port_get(unit, port, "tsc_diag_ability_mask", 0);
        pCfg->diag_regs 
          = soc_property_port_get(unit, port, "tsc_diag_regs", 1);
        pCfg->linkup_bert
          = soc_property_port_get(unit, port, "tsc_linkup_bert", 0);
        pCfg->fault_disable
          = soc_property_port_get(unit, port, "tsc_fault_disable", 0);
        pCfg->svk_id 
          = soc_property_port_get(unit, port, "tsc_svk_id", 0); 

        
        pCfg->pll_divider        = soc_property_port_get(unit, port, spn_XGXS_PHY_PLL_DIVIDER, 0);
        pCfg->sgmii_mstr         = soc_property_port_get(unit, port, spn_SERDES_SGMII_MASTER, FALSE);
        pCfg->pdetect10g         = soc_property_port_get(unit, port, spn_XGXS_PDETECT_10G, FALSE);

        pCfg->cl37an             = soc_property_port_get(unit, port, spn_PHY_AN_C37, FALSE);
        pCfg->an_cl72            = soc_property_port_get(unit, port, spn_PHY_AN_C72, TRUE);

        
        pCfg->diag_verb
          = soc_property_port_get(unit, port, "tsc_diag_verb", 0);
        pCfg->diag_port 
          = soc_property_port_get(unit, port, "tsc_diag_mport", pCfg->diag_port);
        if((pCfg->diag_port&(1<<(port-32)))&&(port>=32)&&(port<64)) 
          tsc->verbosity = pCfg->diag_verb ;
       
        pCfg->diag_port = 0 ; 
        pCfg->diag_port 
          = soc_property_port_get(unit, port, "tsc_diag_hport", pCfg->diag_port);
        if((pCfg->diag_port&(1<<(port-64)))&&(port>=64)&&(port<96)) 
          tsc->verbosity = pCfg->diag_verb ;
       
        pCfg->diag_port = 0 ; 
        pCfg->diag_port 
          = soc_property_port_get(unit, port, "tsc_diag_sport", pCfg->diag_port);
        if((pCfg->diag_port&(1<<(port-96)))&&(port>=96)) 
          tsc->verbosity = pCfg->diag_verb ;

        pCfg->diag_port = 0 ; 
        pCfg->diag_port 
          = soc_property_port_get(unit, port, "tsc_diag_port", pCfg->diag_port);
        if((pCfg->diag_port&(1<<port))&&(port<32)) tsc->verbosity = pCfg->diag_verb ;

        if(pCfg->linkup_bert &(1<<port)) {
