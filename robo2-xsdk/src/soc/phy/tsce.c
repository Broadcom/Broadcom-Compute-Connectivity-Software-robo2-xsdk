...
_tsce_reg_read(void* user_acc, uint32_t core_addr, uint32_t reg_addr, uint32_t *val)
{
    ...
    *val = data16;
	...
	PHYMOD_VDBG(TEMOD_DBG_REGACC, pm_acc,("%-22s: core_addr: 0x%08x reg_addr: 0x%08x, data: 0x%04x\n", __func__, core_addr, reg_addr, *val ));
#endif
    return rv;
}
...
_tsce_reg_write(void* user_acc, uint32_t core_addr, uint32_t reg_addr, uint32_t val)
{
    ...
    if (mask) {
        if (core->wrmask) {
            ...
            pm_core = &core->pm_core;
            pm_acc = &pm_core->access;
            PHYMOD_VDBG(TEMOD_DBG_REGACC, pm_acc,
                        ("%-22s: core_addr: 0x%08x reg_addr: 0x%08x, data: 0x%04x\n", __func__, core_addr, reg_addr, val ));
#endif
...
#ifdef VENDOR_BROADCOM
    pm_acc = &phy->access;
    PHYMOD_VDBG(DBG_SPD, pm_acc, ("%-22s: p=%p adr=%0x lmask=%0x if=%0d(%s) soc_if=%0d tx_inx=%0d\n",
       __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask, interface_config->interface_type, 
          phymod_interface_t_mapping[interface_config->interface_type].key, speed_config->line_interface, *tx_index));
#endif
    return SOC_E_NONE;
}
                 ...
                 phymod_phy_init_config_t  *pm_phy_init_config)
{
    ...
    */
    ...
    speed_config->speed  = soc_property_port_get(unit, port, spn_PORT_INIT_SPEED, 10312);
...
phy_tsce_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    SOC_IF_ERROR_RETURN
        (phymod_phy_interface_config_set(&phy->pm_phy,
                                          PHYMOD_INTF_F_SPEED_MODIFY, &interface_config));
    /* record success */
...
#ifdef VENDOR_BROADCOM
    pm_phy = &phy->pm_phy;
    pm_acc = &pm_phy->access;
    PHYMOD_VDBG(DBG_SPD, pm_acc, ("%-22s: p=%p adr=%0x lmask=%0x if=%0d(%s) pif=%0d\n",
      __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask, interface_config.interface_type, 
      phymod_interface_t_mapping[interface_config.interface_type].key, *pif));
#endif
    return (SOC_E_NONE);
...
phy_tsce_probe(int unit, phy_ctrl_t *pc)
{
   ...
   if (SOC_IS_ARDON(unit)) {
    ...
    if (SOC_IS_ARDON(unit)) {
        ...
        phy_type = phymodDispatchTypeEaglePhy2;
