...
viper_speed_config_t def_speed_config;
...
STATIC int viper_power_set(soc_phymod_ctrl_t *pmc, uint32 value);
...
viper_speed_to_interface_config_get(const phymod_phy_access_t *phy, viper_speed_config_t* speed_config, phymod_phy_inf_config_t* interface_config, TXDRV_INXS_t* tx_index)
{
    ...
    *tx_index = TXDRV_DFT_INX;
        ...
        case 1000:
            if (speed_config->fiber_pref) { 
                ...
                *tx_index = TXDRV_DFT_INX;
                ...
                interface_config->interface_type = phymodInterfaceSGMII;
             	if(SOC_IS_AVENGER(0)) {
                    if(speed_config->line_interface == SOC_PA_INTF_SGMII) {
                        interface_config->interface_type = phymodInterfaceSGMII;
                        interface_config->interface_modes = 1;
                    } else if(speed_config->line_interface) /* QSGMII*/
                        interface_config->interface_type = phymodInterface1000X;
                }
    ...
    if(SOC_IS_AVENGER(0) && speed_config->an) {
        interface_config->interface_modes = 1;
        *tx_index = TXDRV_AN_INX;
    }

#ifdef VENDOR_BROADCOM
    pm_acc = &phy->access;
    PHYMOD_VDBG(DBG_SPD, pm_acc, ("%-22s: p=%p adr=%0x lmask=%0x if=%0d(%s) tx_inx=%0d\n",
       __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask, interface_config->interface_type, 
       phymod_interface_t_mapping[interface_config->interface_type].key, *tx_index));
#endif
    return SOC_E_NONE;
}
...
#ifdef VENDOR_BROADCOM
    phymod_dbg_mask = soc_property_port_get(unit, port, "phymod_dbg_mask", phymod_dbg_mask);
    phymod_dbg_addr = soc_property_port_get(unit, port, "phymod_dbg_addr", phymod_dbg_addr);
    phymod_dbg_lane = soc_property_port_get(unit, port, "phymod_dbg_lane", phymod_dbg_lane);
#endif
...
#ifdef CONFIG_QSGMII
    /* For QSGMII */
    if(SOC_IS_AVENGER(unit)) {
        pm_phy_init_config->polarity.rx_polarity = 0;
        pm_phy_init_config->polarity.tx_polarity = 0;
        pm_phy_init_config->tx[0].pre = -1;
        pm_phy_init_config->tx[1].pre = -1;
        pm_phy_init_config->tx[2].pre = -1;
        pm_phy_init_config->tx[3].pre = -1;
        pm_phy_init_config->tx[0].amp = -1;
        pm_phy_init_config->tx[1].amp = -1;
        pm_phy_init_config->tx[2].amp = -1;
        pm_phy_init_config->tx[3].amp = -1;
        if (speed_config->speed == 1000 && speed_config->line_interface) {
            int phyid;
            uint16 phy_rd_data;
            /*QSGMII 1G*/
            for (phyid=16 ;phyid <= 19;phyid++) {
                /* Enable RDB*/
                pc->write(0,phyid , 0x17 , 0xF7E);
                pc->write(0,phyid , 0x15 , 0x0);
                /* Target the REG with RBD address 0x2A (Legacy address 0x18 Shadow 0x2) ,*/
                pc->write(0,phyid , 0x1E , 0x2A);
                pc->read(0, phyid, 0x1f, &phy_rd_data);
                phy_rd_data = phy_rd_data & 0xFFDF;
                pc->write(0,phyid, 0x1F , phy_rd_data);
                pc->read(0, phyid, 0x1f, &phy_rd_data);
            }

            /* 5G enable in External phy*/
            /* Set AER as QSGMII[0]*/
            pc->write(0,0x18 ,0x1F ,0xFFD0);
            pc->write(0,0x18 ,0x1E ,0x0);
            /* Broadcast to all lanes*/
            pc->write(0,0x18 ,0x1E ,0x1F);
            /* Write reg 0x8000, 0x10*/
            pc->write(0,0x18 ,0x1F ,0x8000);
            pc->write(0,0x18 ,0x10 ,0x202F);
            /* Write reg 0x8000 0x1E*/
            pc->write(0,0x18 ,0x1E ,0x1);
            /* Write reg 0x0000 0x0*/
            pc->write(0,0x18 ,0x1F ,0x0);
            pc->write(0,0x18 ,0x0  ,0xA040);
            /* Write reg 0x8000 0x10*/
            pc->write(0,0x18 ,0x1F ,0x8000);
            pc->write(0,0x18 ,0x10 ,0x62F);
            /* Write reg 0x8300 0x18*/
            pc->write(0,0x18 ,0x1F ,0x8300);
            pc->write(0,0x18 ,0x18 ,0x5800);
            /* Write reg 0x8000 0x10*/
            pc->write(0,0x18 ,0x1F ,0x8000);
            pc->write(0,0x18 ,0x10 ,0x263F);
            /* Write reg 0x8010 0x16*/
            pc->write(0,0x18 ,0x1F ,0x8010);
            pc->write(0,0x18 ,0x16 ,0x303);
            /* Enable Auto Neg*/
            pc->write(0, 11, 0xd, 0x0);
            pc->write(0, 11, 0xe, 0x800d);
            pc->write(0, 11, 0xd, 0x4000);
            pc->write(0, 11, 0xe, 0x800f);
            pc->write(0,0xb,0x1F, 0x0);
            pc->write(0,0xb,0x0 ,0x1140);
            pc->write(0, 11, 0xd, 0x0);
            pc->write(0, 11, 0xe, 0x800d);
            pc->write(0, 11, 0xd, 0x4000);
            pc->write(0, 11, 0xe, 0x800f);
            pc->write(0,0xc,0x1F, 0x0);
            pc->write(0,0xc,0x0 ,0x1140);
            pc->write(0, 11, 0xd, 0x0);
            pc->write(0, 11, 0xe, 0x800d);
            pc->write(0, 11, 0xd, 0x4000);
            pc->write(0, 11, 0xe, 0x800f);
            pc->write(0,0xd,0x1F, 0x0);
            pc->write(0,0xd,0x0 ,0x1140);
            pc->write(0, 11, 0xd, 0x0);
            pc->write(0, 11, 0xe, 0x800d);
            pc->write(0, 11, 0xd, 0x4000);
            pc->write(0, 11, 0xe, 0x800f);
            pc->write(0,0xe,0x0 ,0x1140);
        }
        pm_phy_init_config->an_en = FALSE;
    }
#endif
...
phy_viper_init(int unit, soc_port_t port)
{
    ...
    if(SOC_IS_AVENGER(unit)) {
        def_pc = INT_PHY_SW_STATE(unit, SOC_PORT_MAX(unit, ge) - 1);
        if(port == SOC_PORT_MAX(unit, ge))
            pc->driver_data = def_pc->driver_data;
    }
    ...
    for (idx = 0; idx < pmc->num_phys; idx++) {
        phy = pmc->phy[idx];
        core = phy->core;
        phy->pm_phy.access.lane_mask = 1 << (pc->lane_num);
        ...
        if(!SOC_IS_AVENGER(unit) || !viper_inited) {
    ...
    if (!(SOC_IS_AVENGER(0) && speed_config->speed == 2500)) {
    ...
    if ((interface_config.interface_type == phymodInterfaceSGMII) && interface_config.data_rate <=1000) {
        interface_config.interface_modes = 1;
    }
    SOC_IF_ERROR_RETURN
...
phy_viper_an_set(int unit, soc_port_t port, int enable)
{
    ...
    if(SOC_IS_AVENGER(unit) && pCfg->speed_config.fiber_pref)
        return SOC_E_NONE;
...
phy_viper_interface_set(int unit, soc_port_t port, soc_port_if_t pif)
{
    ...
    if (pCfg->speed_config.line_interface == SOC_PA_INTF_SGMII) {
                   ...
                   phymod_bus_t *core_bus, uint32 core_addr)
{
    ...
    pm_core->type = phymodDispatchTypeViper;
   ...
   speed_config->line_interface = 0;
   if(speed_config->line_interface)
       PHYMOD_ACC_LANE_MASK(pm_acc) = 0xf; /*QSGMII*/
   else
#endif
       PHYMOD_ACC_LANE_MASK(pm_acc) = 0x1; /*SGMII*/
    }
    return;
}
...
phy_viper_probe(int unit, phy_ctrl_t *pc)
{
    ...
    if(SOC_IS_SABER2(unit)) {
        ...
        pc->chip_num = SOC_BLOCK_NUMBER(unit, SOC_PORT_BLOCK(unit, phy_port));
    } else if (SOC_IS_AVENGER(unit)) {
        pc->lane_num = phy_port - 8;
    }
    ...
    for (idx = 0; idx < pmc->num_phys ; idx++) {
        ...
        if(SOC_IS_AVENGER(unit)) {
           ...
           phymod_access_t_init(pm_acc);
           PHYMOD_ACC_USER_ACC(pm_acc) = core;
           PHYMOD_ACC_BUS(pm_acc) = &core->pm_bus;
           PHYMOD_ACC_ADDR(pm_acc) = 11;
           PHYMOD_ACC_LANE_MASK(pm_acc) = 0xf;
           PHYMOD_ACC_F_CLAUSE45_CLR(pm_acc);
           rv = phymod_core_probe(&(pm_core->access), &phy_type, (int *)&found);
           if ( found == 1 ) {
        	   LOG_INFO(BSL_LS_SOC_PHY, (BSL_META_U(pc->unit, "_VIPER CORE found!\n")));
           } else {
        	   LOG_INFO(BSL_LS_SOC_PHY, (BSL_META_U(pc->unit, "_ERROR: Viper Core Not Found!\n")));
           }
           sal_memcpy(&phy->pm_phy.access, &pm_core->access, sizeof(*pm_acc));
        }
#endif
    }
