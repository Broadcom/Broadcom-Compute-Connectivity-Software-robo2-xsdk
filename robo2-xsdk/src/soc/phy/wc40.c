...
_wc40_soft_an_cl73kr2_custom(int unit, soc_port_t port, int an)
{
    ...
    for (lane = lane_start; lane <= lane_end; lane++) {
        SOC_IF_ERROR_RETURN
            (WC40_REG_WRITE(unit,pc,ln_access[lane], 0x8436, 0xA157));
        SOC_IF_ERROR_RETURN
            (WC40_REG_WRITE(unit,pc,ln_access[lane], 0x8437, 0x7537));
        SOC_IF_ERROR_RETURN
            (WC40_REG_WRITE(unit,pc,ln_access[lane], 0x8438, 0xCBE2));
        SOC_IF_ERROR_RETURN
            (WC40_REG_WRITE(unit,pc,ln_access[lane], 0x8439, 0xA157));
        SOC_IF_ERROR_RETURN
            (WC40_REG_WRITE(unit,pc,ln_access[lane], 0x843a, 0x7537));
        SOC_IF_ERROR_RETURN
            (WC40_REG_WRITE(unit,pc,ln_access[lane], 0x843b, 0xCBE2));
    }
...
{
#ifdef VENDOR_GAMMA
#define WC40_HP_OUI0        0x321
#define WC40_HP_OUI1        0x454
#define WC40_HP_ETA0_KR2    0x157
#define WC40_HP_ETA1_KR2    0x620
    ...
    if (data16 & (1 << 6)) {
        ...
        LOG_VERBOSE(BSL_LS_SOC_PHY,
                    (BSL_META_U(unit,
                                "KR2 CUSTOM autoneg wait page_rx bit set: u=%d p=%d kr2_state:0x%x,nxp0:0x%x, nxp1:0x%x,nxp2:0x%x\n"), 
                     unit, port,DEV_CTRL_PTR(pc)->soft_kr2.state,lp_nxp0,lp_nxp1,lp_nxp2));
        ...
        if((DEV_CFG_PTR(pc)->cl73an == WC40_CL73_KR2_CUSTOM_WAR)
        ...
        if (DEV_CTRL_PTR(pc)->soft_kr2.state == WC40_SOFT_KR2_STATE_INIT) {
            ...
            if ( (lp_nxp0 & AN_IEEE1BLK_AN_LP_XNP_ABILITY0_NP_MASK)
                 ...
                 && (lp_nxp1 == WC40_HP_OUI0) 
                 && (lp_nxp2 == WC40_HP_OUI1)) {
                
                ld_nxp1 = DEV_CTRL_PTR(pc)->soft_kr2.ld_kr2? WC40_HP_ETA1_KR2 :0;
                SOC_IF_ERROR_RETURN
                    (WRITE_WC40_AN_IEEE1BLK_AN_XNP_TRANSMIT1r(unit, pc, 0x00, ld_nxp1));
                ld_nxp2 = 0x00;    
                SOC_IF_ERROR_RETURN
                    (WRITE_WC40_AN_IEEE1BLK_AN_XNP_TRANSMIT2r(unit, pc, 0x00, ld_nxp2));
                ld_nxp0 = WC40_HP_ETA0_KR2 | AN_IEEE1BLK_AN_XNP_TRANSMIT0_NP_MASK;
            ...
            if ( (lp_nxp0_msg == WC40_HP_ETA0_KR2) && (lp_nxp2 == 0)) {
                /* KR2 match */
                if (lp_nxp1 == WC40_HP_ETA1_KR2) {
        ...
        LOG_VERBOSE(BSL_LS_SOC_PHY,
                    (BSL_META_U(unit,
                                "KR2 CUSTOM autoneg tx page: u=%d p=%d kr2_state:0x%x,tx_nxp0:0x%x,tx_nxp1:0x%x, tx_nxp2:0x%x\n"), 
        ...
        if(SOFT_KR2_TIMER_VALID(pc)
           && (DEV_CFG_PTR(pc)->cl73an == WC40_CL73_KR2_CUSTOM_WAR)) {
            if(SOFT_KR2_CHECK_TIMEOUT(pc)) {
                ...
                LOG_VERBOSE(BSL_LS_SOC_PHY,
                            (BSL_META_U(unit,
                                        "KR2 CUSTOM (WAR) autoneg no page rcvd: u=%d p=%d kr2_state:0x%x\n"), 
