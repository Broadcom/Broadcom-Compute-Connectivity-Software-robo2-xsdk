...
uint32 GetBoardDACvalue(int unit)
{
    ...
    sal_printf("GetBoardDACvalue: PVT_1V_0 : 0x%x\n", VFB_pin_voltage);
    board_dac_code = (1425U - VFB_pin_voltage)*10000U/11938U;
#else
    VFB_pin_voltage = reg*8779U/10240U;
    sal_printf("GetBoardDACvalue: PVT_1p8V : 0x%x\n", VFB_pin_voltage);
...
void AvsSetDAC(int unit, uint32 dac_code)
{
    ...
    sal_printf ("dwl......avs.c:AvsSetDac=%d, cur_dac=%d delta=%d \n",
...
#define MIN_VOLTAGE_STEP_SIZE INT(.00088)
        ...
        if ((cur_dac_code > BCHP_AVS_DAC_CODE_max)
            || (cur_dac_code < BCHP_AVS_DAC_CODE_min)) {
             sal_printf("FindNominal_Voltage Break Iteration[0x%x]"
                        "voltage[0x%x]\n", i, voltage);
             sal_printf("FindNominal_Voltage Break cur_dac_code[0x%x]\n", cur_dac_code);
             cur_dac_code = board_dac_code; /* return this instead */
             break;
        }

        sal_printf("FindNominal_Voltage Iteration[0x%x],  voltage[0x%x],"
                   " step_size[%d], cur_dac[0x%x]\n",
...
uint32 avenger_static_avs(int unit)
{
    ...
    sal_printf("AVS Nominal Value from OTP: 0x%x\n", avs_otp_value);
#endif

    sal_printf("Nominal_Voltage 0x%x\n", avs_nominal_voltage);
    ...
    sal_printf("Board DAC Code 0x%x\n", board_dac_code);
...
#define MIN_VOLTAGE_STEP_SIZE INT(.00088)
        ...
        if ((cur_dac_code > BCHP_AVS_DAC_CODE_max)
            || (cur_dac_code < BCHP_AVS_DAC_CODE_min)) {
             sal_printf("SetAvs_Voltage Break Iteration[0x%x]"
                        " voltage[0x%x]\n", i, voltage);
             sal_printf("SetAvs_Voltage Break cur_dac_code[0x%x]\n",
                        cur_dac_code);
             cur_dac_code = board_dac_code; /* return this instead */
             break;
