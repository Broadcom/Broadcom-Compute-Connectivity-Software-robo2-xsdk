...
STATIC int
cbxi_l2_arl_counter_update(int unit, uint16_t dpg, int count, uint32_t flags)
{
    uint32_t reg_val;
    int      age_time = 0;
...
#ifdef MANAGED_MODE
    /* Match valid entries on MAC + VLAN */
    flags = (CBXI_L2_MAC_MASK | CBXI_L2_VLAN_MASK |
                                CBXI_L2_VALID_MASK);
    rv = cbxi_robo2_port_validate(l2entry->params.portid, &port, &lpg, &unit);
    if (CBX_FAILURE(rv)) {
         /* Provide a valid dummy port */
        l2entry->params.portid = CBX_PORT_ECPU;
...
#ifdef MANAGED_MODE
     /* This is port id is not used in L2 addr querry, but requires a valid port */
    l2entry->params.portid = CBX_PORT_ECPU;
    l2entry->params.dest_type = cbxL2AddrDestDPG;
#endif
