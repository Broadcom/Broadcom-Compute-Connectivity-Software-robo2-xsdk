...
int drv_avgr_pkt_tx(int unit, bcm_pkt_t **pkt, int count)
{
    ...
    for (num = 0; num < count; num++) {
        BCM_PBMP_ASSIGN(tx_pbmp, pkt[num]->tx_pbmp);
        PBMP_ITER(tx_pbmp, port) {
            cbx_packet_attr_t_init(&attr);
            if (pkt[num]->opcode == BCM_PKT_OPCODE_MC ) {
                attr.flags = CBX_PACKET_TX_FWD_MCAST;
                attr.mcastid = pkt[num]->multicast_group;
            } else if (pkt[num]->opcode == BCM_PKT_OPCODE_UC ) {
                attr.flags = CBX_PACKET_TX_FWD_UCAST;
            } else if (pkt[num]->opcode == BCM_PKT_OPCODE_SWITCH ) {
                attr.flags = CBX_PACKET_TX_FWD_SWITCH;
            } else {
                attr.flags = CBX_PACKET_TX_FWD_RAW;
            }
            attr.dest_index = port;
            attr.vlanid = pkt[num]->vlan;
            cbx_pktio_packet_send(pkt[num]->pkt_data->data,
                                  pkt[num]->pkt_data->len, &attr);
        }
    }
...
{
    cbx_pktio_rx_cb_handle rx_cb_handle = NULL;
    cbx_packet_cb_info_t recv_info;
    int ret;

    /* Register with Pkt I/O for send/receive */
    sal_memset(&recv_info, 0, sizeof(cbx_packet_cb_info_t));
    recv_info.flags = CBX_PKTIO_CALLBACK_ALL;
    recv_info.name = "RX";
    recv_info.priority = 1;
    recv_info.cookie = NULL;
    if ((ret = cbx_pktio_register(recv_info, (cbx_packet_io_callback_t)cb, &rx_cb_handle))
        != CBX_E_NONE) {
        sal_printf("SDK packet register failed %d \n", ret);
        return -1;
    }
    return 0;
}
