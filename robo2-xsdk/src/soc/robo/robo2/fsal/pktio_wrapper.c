...
void packetio_dump_stats(void)
{
    ...
    sal_printf("Packetio Tx Ok   : %lu \n", value);
    for (value = 0, i = 0; i < MAX_PKTIO_SOCKETS; i++)
        value += _sys_sockets[i].tx_drop;
    sal_printf("Packetio Tx Drop : %lu \n", value);
    for (value = 0, i = 0; i < MAX_PKTIO_SOCKETS; i++)
        value += _sys_sockets[i].tx_err;
    sal_printf("Packetio Tx Error: %lu \n", value);
    for (value = 0, i = 0; i < MAX_PKTIO_SOCKETS; i++)
        value += _sys_sockets[i].rx_cnt;
    sal_printf("Packetio Rx Ok   : %lu \n", value);
    for (value = 0, i = 0; i < MAX_PKTIO_SOCKETS; i++)
        value += _sys_sockets[i].rx_drop;
    sal_printf("Packetio Rx Drop : %lu \n", value);
    for (value = 0, i = 0; i < MAX_PKTIO_SOCKETS; i++)
        value += _sys_sockets[i].rx_err;
    sal_printf("Packetio Rx Error: %lu \n", value);
}
