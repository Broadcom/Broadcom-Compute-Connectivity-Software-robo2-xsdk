...
/**
 * Function:
 *   cbxi_port_priority_info_set
 * Purpose :
 *   This routine is used to set the priority parameters of a Port in PGT.
 *
 * @param unit     device Identifier - PRIMARY OR SECONDARY
 * @param flags    PORT flags
 * @parm  pgt_t    Port parameters, including:
 * - port type
 * - drop tagged packets
...
int
cbxi_port_priority_info_set( int unit,
                             uint16 flags,
                             pgt_t *pgt_entry )
{

    if (flags & CBX_PORT_TRUST_MODE_UNTRUSTED) {
        ...
        pgt_entry->arb6 = 3;
        pgt_entry->arb7 = 3;
    } else if (flags & CBX_PORT_TRUST_MODE_DOT1P) {
        pgt_entry->src0 = 9;  /* IP_DSCP V4 or V6  */
        pgt_entry->src1 = 1;  /* C-tag */
        pgt_entry->src2 = 2;  /* S-tag */
        pgt_entry->arb0 = 3;
        ...
        pgt_entry->arb6 = 1;
        pgt_entry->arb7 = 1;
    } else if (flags & CBX_PORT_TRUST_MODE_IPDSCP) {
        pgt_entry->src0 = 9;  /* IP_DSCP V4 or V6  */
        pgt_entry->src1 = 1;  /* C-tag */
        pgt_entry->src2 = 2;  /* S-tag */
        pgt_entry->arb0 = 3;
...
cbxi_port_info_set( int unit,
    ...
    if ((robo->rev_id == BCM53162_B1_REV_ID) ||
        (robo->rev_id == BCM53158_B1_REV_ID)) {
        pgt_entry.ipv6_en = 1;
        pgt_entry.llc_en = 1;
    } else {
        pgt_entry.ipv6_en = 0;
...
#ifdef BCM_ROBO2_DEFAULT_PORT_PRIORITY
    pgt_entry.src0 = 9;  /* IP_DSCP V4 or V6  */
    pgt_entry.src1 = 1;  /* C-tag */
    pgt_entry.src2 = 2;  /* S-tag */
    pgt_entry.arb0 = 3;
    ...
    pgt_entry.arb7 = 1;
#else
    CBX_IF_ERROR_RETURN(cbxi_port_priority_info_set(unit,
                                                    port_params->flags,
                                                    &pgt_entry));
#endif /* BCM_ROBO2_PORT_PRIORITY */
    ...
    if((IS_LOCAL_CPU_PORT(unit, port)) ||
       (IS_EXTERNAL_CPU_PORT(unit, port)))
        ...
        /* Enable soft tag for user configured etype packets */
        pgt_entry.soft_tag_en |= 1 << SOFT_TAG_USER_CFG;
#ifdef CONFIG_PTP
        /*********** PTP changes *****************/
        pgt_entry.metadata_len = 0x2;
        pgt_entry.tlv_parse_en = 1;
    ...
    if (port_params->flags & CBX_PORT_SA_UNKNOWN_CPUCOPY) {
        pglct_entry.copy_unknown_smac = 1;
    } else {
        pglct_entry.copy_unknown_smac = 0;
    }

    if (port_params->flags & CBX_PORT_SA_MOVE_CPUCOPY) {
        pglct_entry.copy_update = 1;
    } else if (port_params->flags & CBX_PORT_SA_MOVE_DROP) {
        pglct_entry.copy_update = 0;
    } else {
        pglct_entry.copy_update = 0;
    }
...
cbxi_port_speed_set(int unit,  cbx_port_t port, int speed ) {
    ...
    if(!(IS_EXTERNAL_CPU_PORT(unit, port))) {
        ...
        /* Disable MAC */
#if defined(BCM_ROBO2_SUPPORT)
        rv = MAC_ENABLE_SET(SOC_ROBO_PORT_MAC_DRIVER(unit, port),
#else
 ...
 */
int
cbxi_port_admin_status_set(int unit, cbx_port_t port, int value) {
    int rv = CBX_E_NONE;
    int en = 0;
...
#if defined(BCM_ROBO2_SUPPORT)
    rv = MAC_ENABLE_GET(SOC_ROBO_PORT_MAC_DRIVER(unit, port_out),
#else
...
cbxi_port_info_get(int unit, cbx_port_t port_out,
    ...
    if (pglct_entry.copy_unknown_smac== 1) {
    ...
    } else if (pgt_entry.arb3 == 1 ) {
        port_params->flags |= CBX_PORT_TRUST_MODE_DOT1P;
    } else if (pgt_entry.arb3 == 0) {
        port_params->flags |= CBX_PORT_TRUST_MODE_IPDSCP;
    } else {
        port_params->flags &= ~( CBX_PORT_TRUST_MODE_UNTRUSTED |
                                CBX_PORT_TRUST_MODE_DOT1P |
                                CBX_PORT_TRUST_MODE_IPDSCP );
    }
...
int
avng_pm_tsc_clk_mux_select_set(int unit, uint8_t port1, uint32_t speed1,
                               uint8_t vco_0, uint32_t vco_1, uint32 *skip_vco_change) {
...
#if defined(BCM_ROBO2_SUPPORT)
    rv = MAC_ENABLE_SET(SOC_ROBO_PORT_MAC_DRIVER(unit, port_out),
#else
        rv = MAC_ENABLE_SET(port_info[unit][port_out].p_mac,
#endif
                                unit, port_out, TRUE);
...
cbx_port_phy_get( cbx_portid_t portid,
   ...
   /* validate the port index passed */
    ...
    if (CBX_FAILURE(rv)) {
        ...
        return (rv == SOC_E_PARAM) ? CBX_E_PARAM : CBX_E_FAIL;
    }
...
cbx_port_attribute_get(cbx_portid_t       port,
   ...
   /* validate the port index passed */
    ...
    if(port > 7 && port < 14 && rv == CBX_E_UNAVAIL)
        rv = CBX_E_NONE;
    if (CBX_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_FSAL_PORT,
...
/* MAC and PHY stub functions */
int avng_phy_duplex_get(int unit, uint8_t port, uint32_t *value)
{
    int rv = CBX_E_NONE;
    rv = soc_phyctrl_duplex_get(unit, port, (int *)value);
    return rv;
}
int avng_phy_duplex_set(int unit, uint8_t port, uint32_t value)
{
    int rv = CBX_E_NONE;
    rv = soc_phyctrl_duplex_set(unit, port, value);
    return rv;
}

int avng_phy_speed_set(int unit, uint8_t port, uint32_t value)
{
    uint32 speed_1 = 0;
    uint32 speed_0 = 0;
    uint8_t vco_5_0 = 0;
    ...
    if ((port == 12) || (port == 13)) {
        rv = avng_phy_speed_get(unit, 12, &speed_0);
        if (rv == CBX_E_NONE) {
        ...
        rv = avng_phy_speed_get(unit, 13, &speed_1);
        if (rv == CBX_E_NONE) {
        ...
        if (speed == 10001) {
            ...
            rv = avng_pm_tsc_clk_mux_select_set(unit, port, speed, vco_5_0,
                                                vco_5_1, &skip_vco_change);
        }
        if (rv == CBX_E_NONE) {
            rv = avng_portmacro_speed_set(unit, port, speed);
        }
    }
    if (rv == CBX_E_NONE) {
        rv = soc_phyctrl_speed_set(unit, port, (int)value);
    }
    return rv;
}

int avng_phy_speed_get(int unit, uint8_t port, uint32_t *value)
{
    int rv = CBX_E_NONE;
    rv = soc_phyctrl_speed_get(unit, port, (int *)value);
    return rv;
}
int avng_phy_link_get(int unit, uint8_t port, uint32_t *up)
{
    int rv = CBX_E_NONE;
    ...
    if (IS_GE_PORT(unit, port) && ext_pc != NULL){
         rv = PHY_LINK_GET(ext_pc->pd, unit, port, (int *)up);
    }
    return rv;
}
int avng_phy_loopback_get(int unit, uint8_t port, uint32_t *status)
{
    int rv = CBX_E_NONE;
    rv = soc_phyctrl_loopback_get(unit, port, (int *) status);
    return rv;
}
int avng_phy_loopback_set(int unit, uint8_t port, uint32_t enable)
{
    int rv = CBX_E_NONE;
    rv = soc_phyctrl_loopback_set(unit, port, enable, TRUE);
    return rv;
}
int avng_phy_autoneg_set(int unit, uint8_t port, uint32_t an)
{
    int rv = CBX_E_NONE;

    /* set PHY auto-negotiation OFF */
    rv = soc_phyctrl_auto_negotiate_set(unit, port, (an ? TRUE : FALSE));

    return rv;
}
int avng_phy_autoneg_get(int unit, uint8_t port, uint32_t *an)
{
    int rv = CBX_E_NONE, done = 0, autoneg = 0;
    rv = soc_phyctrl_auto_negotiate_get(unit, port, &autoneg, &done);
            *an = (autoneg) ? DRV_PORT_STATUS_AUTONEG_ENABLE :
                        DRV_PORT_STATUS_AUTONEG_DISABLED;
    return CBX_E_NONE;
}
int avng_phy_reg_read(uint8_t unit, uint8_t port, uint32_t reg_addr,
                              uint32_t *p_value)
{
    int rv = CBX_E_NONE;
    uint32 flags = 0;

    rv = soc_phyctrl_reg_read(unit, port, flags, reg_addr, p_value);

#if 0 /* Invoke appropriate call for PHY access */
    phy_ctrl_t *ext_pc = NULL;
    if (IS_GE_PORT(unit, port)){
        ext_pc = EXT_PHY_SW_STATE(unit, port);
    }
    /* Section to prevent the runpacket issue on the GE port */
    if (IS_GE_PORT(unit, port) && ext_pc != NULL){
       rv = PHY_REG_READ(ext_pc->pd, unit, port, 0, reg_addr, p_value);
    }
#endif

    return rv;
}
int avng_phy_reg_write(uint8_t unit, uint8_t port, uint32_t reg_addr,
                               uint32_t value)
{
    int rv = CBX_E_NONE;
    uint32 flags = 0;

    rv = soc_phyctrl_reg_write(unit, port, flags, reg_addr, value);
    ...
    /* Section to prevent the runpacket issue on the GE port */
    if (IS_GE_PORT(unit, port) && ext_pc != NULL){
        rv = PHY_REG_WRITE(ext_pc->pd, unit, port, 0, reg_addr, value);
    }
#endif
    return rv;
}
int avng_phy_eee_set(int unit, uint8_t port, uint32_t mode)
{
    return CBX_E_NONE;
}
int avng_phy_eee_get(int unit, uint8_t port, uint32_t *mode)
{
    return CBX_E_NONE;
}
#ifndef BCM_ROBO2_SUPPORT
static int soc_phy_probe(void)
{
    return CBX_E_NONE;
}

int soc_mac_init(void)
{
    return CBX_E_NONE;
}
#endif
...
int mac_max_frame_size_get(int unit, cbx_port_t port, uint32_t *size)
{
    int rv = CBX_E_NONE;
    mac_driver_t *p_mac = NULL;
    p_mac = SOC_ROBO_PORT_MAC_DRIVER(unit, port);
