...
/* PTP clock mode */
#ifdef CONFIG_ONE_STEP_CLK
cbx_ptp_clk_mode_t ptp_clock_mode = cbxPtpClkOneStep;
#else
...
static int cbxi_ptp_init(int unit)
{
    ...
    if (ptp_clock_mode == cbxPtpClkOneStep) {
        cbx_ptp_clock_mode_set(ptp_clock_mode);
    }
...
int cbxi_ptp_port_admin_set(uint8 unit, cbx_portid_t portid_in,
    ...
    if ((admin == CBX_PTP_PORT_ENABLE)&&(ptp_clock_mode == cbxPtpClkOneStep)) {
        ...
        CBX_IF_ERROR_RETURN(cbxi_ptp_lin_source_release(unit,
                      ptp_lin_id[unit][port].eth_linid, port, CBXI_PTP_VSI, 1));
        ...
        CBX_IF_ERROR_RETURN(cbxi_ptp_lin_source_release(unit,
                     ptp_lin_id[unit][port].udp_linid, port, CBXI_PTP_VSI, 2));
        CBX_IF_ERROR_RETURN(cbxi_lin_slot_release(
                                           ptp_lin_id[unit][port].udp_linid));
...
int cbx_ptp_port_admin_set(cbx_portid_t portid, uint8_t admin) {
    ...
    if ((admin && CBX_PBMP_MEMBER(ptp_enable_pbmp[unit], port)) ||
        (!admin && !CBX_PBMP_MEMBER(ptp_enable_pbmp[unit], port))) {
        sal_printf("Admin state is already set to the given value\n");
        return CBX_E_EXISTS;
    }
...
/**
 * cbxi_ptp_port_admin_get
 * Gets the configured admin state of PTP port
 *
 * @param unit      (IN)  Unit number
 * @param portid    (IN)  Port identifier
...
int cbx_ptp_port_admin_get(cbx_portid_t portid, uint8_t *admin) {
    ...
    cbxi_ptp_port_admin_get(unit, port, &admin_temp);
    *admin = admin_temp;
    return CBX_E_NONE;
}
