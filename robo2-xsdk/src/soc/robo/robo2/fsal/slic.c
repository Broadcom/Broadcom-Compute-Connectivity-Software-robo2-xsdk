...
int cbxi_soft_tag_init(int unit)
{
    ...
    /* User confgired STAG */
    CBX_IF_ERROR_RETURN(soc_robo2_stt_get(unit, SOFT_TAG_USER_CFG, &ent_stt));
    ent_stt.etype     = 0xFFFF; /* Ethertype gets updated by user configuration */
    ent_stt.ntype     = CBXI_NT_UTAG0;
    ent_stt.ekey_type = ETYPE_USER_ETYPE8; /* Etype field for SLIC in SLICMAP table */
    ent_stt.length    = 0;
    CBX_IF_ERROR_RETURN(soc_robo2_stt_set(
                        unit, SOFT_TAG_USER_CFG, &ent_stt, &status));
...
int cbxi_default_rule_init(int unit, cbxi_slic_rule_id_t rule_idx)
{
    ...
    case SLIC_RSVD_MAC_2:
        ...
        slic_rule.dmac_lo_mask = 0xFF2E;
        slic_rule.mask_flags |= ROBO2_SLIC_MASK_DMAC;
        slicid = SLICID_LLDP; /* Flood to all ports */
        slic_trap = SLIC_TRAP_GROUP1; /* TRAP2CP */
        break;
    case SLIC_SRP:
        slic_rule.etype = ETYPE_USER_ETYPE8;
        slic_rule.mask_flags |= ROBO2_SLIC_MASK_ETYPE;
        slic_rule.pg_map = 0x3FFF;
        slicid = SLICID_SRP;
        break;
...
/*
 * Function:
 *  cbxi_soft_tag_user_etype_set
 * Purpose:
 *  Set the ether type for the user configurable soft tag
 *
 * @return return code
 * @retval 0 success.
 */
int cbxi_soft_tag_user_etype_set(int unit, uint16 etype)
{
    stt_t ent_stt;
    uint32_t status = 0;
...
/*
 * Function:
 *  cbxi_soft_tag_user_etype_get
 * Purpose:
 *  Get the ether type for the user configurable soft tag
 *
 * @return return code
 * @retval 0 success.
 */
int cbxi_soft_tag_user_etype_get(int unit, uint16 *etype)
{
    stt_t ent_stt;
...
/*
 * Function:
 *  cbxi_slic_rule_dmac_set
 * Purpose:
 *  Set the destination MAC for the given slic rule number
 *
 * @return return code
 * @retval 0 success.
 */
int cbxi_slic_rule_dmac_set(int unit, cbxi_slic_rule_id_t rule_number,
                            uint8 *mac)
{
    cbxi_slic_rule_t slic_rule;

    CBX_IF_ERROR_RETURN(cbxi_slicmap_rule_get(unit, rule_number, &slic_rule));

    slic_rule.dmac_hi_32   = (mac[0] << 24) | (mac[1] << 16) |
                             (mac[2] << 8) | mac[3];
    slic_rule.dmac_hi_mask = 0xFFFFFFFF;
    slic_rule.dmac_lo_16   = (mac[4] << 8) | mac[5];
    slic_rule.dmac_lo_mask = 0xFFFF;
    slic_rule.mask_flags |= ROBO2_SLIC_MASK_DMAC;

    CBX_IF_ERROR_RETURN(cbxi_slicmap_rule_set(unit, rule_number, slic_rule));
    return CBX_E_NONE;
}

/*
 * Function:
 *  cbxi_slic_rule_dmac_get
 * Purpose:
 *  Get the destination MAC for the given slic rule number
 *
 * @return return code
 * @retval 0 success.
 */
int cbxi_slic_rule_dmac_get(int unit,
                cbxi_slic_rule_id_t rule_number, uint8 *mac)
{
    cbxi_slic_rule_t slic_rule;

    CBX_IF_ERROR_RETURN(cbxi_slicmap_rule_get(unit, rule_number, &slic_rule));

    mac[0] = (slic_rule.dmac_hi_32 & 0xff000000) >> 24;
    mac[1] = (slic_rule.dmac_hi_32 & 0x00ff0000) >> 16;
    mac[2] = (slic_rule.dmac_hi_32 & 0x0000ff00) >> 8;
    mac[3] = (slic_rule.dmac_hi_32 & 0x000000ff);
    mac[4] = (slic_rule.dmac_lo_16 & 0xff00) >> 8;
    mac[5] = (slic_rule.dmac_lo_16 & 0x00ff);
