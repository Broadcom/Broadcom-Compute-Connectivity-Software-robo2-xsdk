...
int cbx_stat_init(void)
{
    ...
    CBX_IF_ERROR_RETURN(robo2_counter_reset(unit));

    if (SOC_IS_CASCADED(CBX_AVENGER_PRIMARY)){
        CBX_IF_ERROR_RETURN(robo2_counter_reset(CBX_AVENGER_SECONDARY));
    }

    CBX_IF_ERROR_RETURN(robo2_counter_thread_set(unit,
                      ...
                      uint64_t *value)
{
    ...
    CBX_IF_ERROR_RETURN(robo2_counter_get
...
int cbx_port_stat_clear(cbx_portid_t portid)
{
     ...
     */
    CBX_IF_ERROR_RETURN(robo2_counter_thread_set
                (unit, COUNTER_THREAD_SYNC, flags,
                interval, pbm));


    COMPILER_64_ZERO(val64);
    CBX_IF_ERROR_RETURN(soc_robo2_counter_set_by_port
                (unit, port_out, val64));
