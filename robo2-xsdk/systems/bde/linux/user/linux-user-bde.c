...
#endif
#else /* !KEYSTONE */
#if (defined(IPROC_CMICD) && defined(BCM_STARFIGHTER3_SUPPORT)) || defined(BCM_ROBO2_SUPPORT)
#include <linux/types.h>
#include <linux/spi/spidev.h>
#include <sal/core/libc.h>
#include <sal/appl/config.h>
...
#define USER_SPI_DEVICE_NAME        "/dev/spidev32765"
...
{ 
    ...
    if ((_devfd = open(LUBDE_DEVICE_NAME,
        ...
        if (linux24) {
            ...
            /* Pass the spi device id and rev id */
            sal_snprintf(kernel_bde, ARRAY_SIZE(kernel_bde),"%s %s=%d %s=%d",
                "/sbin/insmod linux-kernel-bde.ko",
                "spi_devid", _sys_devs[0].spi_devid,
                "spi_revid", _sys_devs[0].spi_revid);
 ...
 * Parameters
 *    dev - device number
 *    status - (out) last read status value
 * Returns:
 *    0 - Success
 *   -1 - Failed
 */
int spi_wait_for_read(int dev, uint16 *status)
{
    int ret, i;
 ...
 *   -1 - Failed
 */
int spi_wait_for_write(int dev, uint16 *status)
{
    int ret, i;
    int fd = _sys_devs[dev].spi_fd;
    uint8  tx[1];
 ...
 *   -1 - Failed
 */
static int spi_dev_read(int dev, uint32 reg, uint8 *buf, int len)
{
    int ret;
    int fd = _sys_devs[dev].spi_fd;
    uint8 tx[5];
    ...
    if (ret < 0) {
        printf("Failed Read Transaction status(0x%x)\n", status);
        return -1;
    }
 ...
 *   -1 - Failed
 */
int spi_dev_write(int dev, uint32 reg, uint8 *buf, int len)
{
    int ret;
    int fd = _sys_devs[dev].spi_fd;
    uint8 tx[32];
...
int spi_reset(int dev, int resetall)
{
    int ret;
    uint8 tx[1];
    struct spi_ioc_transfer tr[2];
...
int spi_status_get(int dev, uint16 *status)
{
    int ret;
    uint8 tx[1];
    uint8 rx[2];
 ...
 *   -1 - Failed
 */
static int _sys_spi_dev_init(int dev, int fd)
{
    uint8 mode;
    uint32 speed;
    ...
    if (getenv("ROBO2_SPI_MODE") != NULL) {
        mode = atoi(getenv("ROBO2_SPI_MODE"));
    }
 ...
 *    void
 */
static void _sys_spi_dev_deinit(int dev)
{
    int fd;
 ...
 */
static int _sys_spi_dev_check(int dev, uint32 chipid, uint32 revid)
{
    switch (chipid) {
...
static int linux_spidev_setup()
{
    ...
    uint32 spifreq = USER_SPI_FREQ_DEFAULT;
    int devid = 0;

    sal_snprintf(spi_device_name, ARRAY_SIZE(spi_device_name),
                        "%s.%s", USER_SPI_DEVICE_NAME, USER_SPI_INTF_DEFAULT);
    ...
    if (fd < 0) {
        ...
        if (_sys_spi_dev_init(devid, fd) == 0) {
            ...
            if (rv < 0) {
                 ...
                 printf("SPI: ChipID read failed\n");
                _sys_spi_dev_deinit(devid);
            } else {
                /* Read successful and check if it is a valid SPI device */
#ifdef BE_HOST
                chipid = soc_htonl(chipid);
                revid = soc_htonl(revid);
#endif
                printf("SPI: ChipID reg 0x%x\n", chipid);
                chipid = chipid & 0xffffff;
                printf("SPI: ChipID BCM%d RevID %d\n", chipid, revid);
                rv = _sys_spi_dev_check(devid, chipid, revid);
                if (rv != 0) {
                   /* Not a recognized chip */
                    printf("SPI:Unrecognized Chip\n");
                   _sys_spi_dev_deinit(devid);
                }
                _sys_devs[devid].spi_devid = chipid;
                _sys_devs[devid].spi_revid = revid;
 ...
 *   -1 - Failure
 */
int soc_spi_terminate(int unit)
{
    if (unit == 0) {
 ...
 *   -1 - Failure
 */
int _sys_spi_read(int unit, uint32 reg, uint8 *buffer, int len)
{
    int ret;
 ...
 *   -1 - Failure
 */
int _sys_spi_write(int unit, uint32 reg, uint8 *buffer, int len)
{
    int ret;
